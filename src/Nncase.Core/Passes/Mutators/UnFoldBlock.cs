    ...
    protected override Expr RewriteLeafBlock(Block expr)
    {
        if (expr.Predicate is TensorConst tc && tc.Value.ToScalar<bool>() is var predicate)
        {
            if (predicate)
            {
                ...
                {
                    var lets = expr.AllocBuffers.ToArray().Select(b =>
                    {
                        var globalOffset = new RankedShape(Enumerable.Repeat<Dimension>(0L, b.Rank).ToArray());
                        return (T.Let(out var v, IR.F.Buffer.AllocateBufferView(b, globalOffset), b.Name), v);
                    }).ToArray();
