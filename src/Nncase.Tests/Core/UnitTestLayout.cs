...
public sealed class UnitTestLayout
{
    ...
    {
        var placement = new Placement([1, 2, 8, 4, 4], "cdyxb", "cdbbb");
        var distType = new DistributedType(new TensorType(DataTypes.Float32, new[] { 2048, 1024 }), new SBP[] { SBP.SContiguous([1, 3]), SBP.B }, placement);
    ...
    public void TestCoorinateWithDifferentLayout()
    {
        ...
        var distTypeA = new DistributedType(tensorType, new SBP[] { SBP.SContiguous([1, 3]), SBP.B }, placement);
        var distTypeB = new DistributedType(tensorType, new SBP[] { SBP.B, SBP.SContiguous([2]) }, placement);
        ...
        {
            return axisPolicies.OfType<SBPSplit>().Select(x => x.HierarchyAxes).SelectMany(x => x).Select(x => longs[x]).ToArray();
        }
