...
public class UnitTestGraphPartition : TestClassBase
{
    ...
    {
        var input = new Var("input", new TensorType(DataTypes.Float32, new int[] { 1, 32, 32 }));
        var main = new Function("main", IR.F.Math.Abs(IR.F.Distributed.Boxing(input, new DistributedType(input.CheckedTensorType, new[] { SBP.B, SBP.B, SBP.B }, new(new[] { 1 }, "b", "b")))), input);
    ...
    public async Task TestLineDiffModuleC2I()
    {
        ...
        var unary = IR.F.Distributed.Boxing(input, new DistributedType(ttype, new[] { SBP.B, SBP.B, SBP.B }, new(new[] { 1 }, "b", "b")));
    ...
    public async Task TestLineDiffModuleI2C()
    {
        var ttype = new TensorType(DataTypes.Float32, new int[] { 1, 32, 32 });
        var input = new Var("input", ttype);
        var unary = IR.F.Distributed.Boxing(IR.F.Math.Abs(input), new DistributedType(ttype, new[] { SBP.B, SBP.B, SBP.B }, new(new[] { 1 }, "b", "b")));
    ...
    public async Task TestYSameModuleC()
    {
        var input1 = new Var("input1", new TensorType(DataTypes.Float32, new int[] { 1, 32, 32 }));
        var input2 = new Var("input2", new TensorType(DataTypes.Float32, new int[] { 1, 32, 32 }));
        var v_0 = IR.F.Distributed.Boxing(input1, new DistributedType(input1.CheckedTensorType, new[] { SBP.B, SBP.B, SBP.B }, new(new[] { 1 }, "b", "b")));
        var v_1 = IR.F.Distributed.Boxing(input2, new DistributedType(input2.CheckedTensorType, new[] { SBP.B, SBP.B, SBP.B }, new(new[] { 1 }, "b", "b")));
        var v_2 = IR.F.Math.Binary(BinaryOp.Add, v_0, v_1);
        var main = new Function("main", v_2, input1, input2);
    ...
    {
        var input = new Var("input", new TensorType(DataTypes.Float32, new int[] { 1, 32, 32 }));
        var v_0 = IR.F.Distributed.Boxing(input, new DistributedType(input.CheckedTensorType, new[] { SBP.B, SBP.B, SBP.B }, new(new[] { 1 }, "b", "b")));
        var v_1 = IR.F.Math.Binary(BinaryOp.Add, v_0, v_0);
        var main = new Function("main", v_1, input);
        Assert.True(CompilerServices.InferenceType(main));
    ...
    public async Task TestCircle2DiffModule()
    {
        var ttype = new TensorType(DataTypes.Float32, new int[] { 1, 32, 32 });
        var input = new Var("input", ttype);
        var v_0 = IR.F.Distributed.Boxing(input, new DistributedType(ttype, new[] { SBP.B, SBP.B, SBP.B }, new(new[] { 1 }, "b", "b")));
        var v_1 = IR.F.Math.Cos(IR.F.Distributed.Boxing(v_0, ttype));
        var v_2 = IR.F.Math.Sin(v_0);
        var v_3 = IR.F.Math.Add(IR.F.Distributed.Boxing(v_1, new DistributedType(ttype, new[] { SBP.B, SBP.B, SBP.B }, new(new[] { 1 }, "b", "b"))), v_2);
    ...
    public async Task TestCircle5SameModule()
    {
        var input = new Var("input", new TensorType(DataTypes.Float32, new int[] { 1, 32, 32 }));
        var v_0 = IR.F.Distributed.Boxing(input, new DistributedType(input.CheckedTensorType, new[] { SBP.B, SBP.B, SBP.B }, new(new[] { 1 }, "b", "b")));
        var v_1 = IR.F.Distributed.Boxing(input, new DistributedType(input.CheckedTensorType, new[] { SBP.B, SBP.B, SBP.B }, new(new[] { 1 }, "b", "b")));
        var v_2 = IR.F.Math.Binary(BinaryOp.Add, v_0, v_1);
    ...
    public async Task TestConcat4SameModule()
    {
        ...
        var input = new Var("input", ttype);
        var v0 = IR.F.Distributed.Boxing(input, new DistributedType(ttype, new[] { SBP.B, SBP.B, SBP.B }, new Placement(new[] { 1 }, "b", "b")));
