...
internal sealed class ILPrintVisitor : ExprFunctor<string, string>
{
    ...
    protected override string VisitBuffer(TIR.Buffer expr)
    {
        ...
        var distributedStorage = expr.DistributedStorageKind == TIR.DistributedBufferStorageKind.CompactLocal
            ? string.Empty
            : $", distributed_storage={expr.DistributedStorageKind}";
        _writer.WInd().WriteLine($"{name} = buffer({type}{storageEncoding}{distributedStorage})");
        return name;
    }
    ...
    protected override string VisitGrid(IR.Affine.Grid expr)
    {
        ...
        using (IndentScope())
        {
            _writer.WInd().WriteLine($"Domain: [{string.Join(", ", expr.DomainBounds.ToArray().Select(Visit))}]");
