...
PYBIND11_MODULE(_nncase, m) {
    ...
    py::class_<shape_bucket_options>(m, "ShapeBucketOptions")
        ...
        .def_property(
            "segment_ranges",
            py::overload_cast<>(&shape_bucket_options::segment_ranges),
            py::overload_cast<std::map<std::string, std::vector<int>>>(
                &shape_bucket_options::segment_ranges));
    ...
    py::class_<pyntt_target_options, cpu_target_options>(m, "PyNTTTargetOptions")
      .def(py::init())
      .def_property(
        "Backend",
        []() {},
