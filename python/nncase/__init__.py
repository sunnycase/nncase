...
import _nncase
from _nncase import RuntimeTensor, RTValue, TensorDesc, Simulator, NTTTargetOptions, PyNTTTargetOptions, NocArchitecture, HuggingFaceOptions, SamplerLogprobsMode, MemoryAccessArchitecture, PagedKVCacheDimKind, AttentionConfig, PagedAttentionConfig, PagedAttentionKVCache, PagedAttentionScheduler
...
class Compiler:
    ...
    def gencode_tobytes(self) -> bytes:
        print("WARNING: The gencode_tobytes may not meet the requirements of model alignment, use gencode instead.")
        code = io.BytesIO()
        self.gencode(code)
...
def check_target(target: str):
    def test_target(target: str):
        return target in ["cpu", "k510", "k230", "xpu", "cuda", "pyntt"]
