...
WARP_SPECIALIZATION_AUXILIARY_CONSUMER_WARPS = 4
WARP_SPECIALIZATION_AUXILIARY_CONSUMER_REGISTERS = 160
...
PACKED_GEMV_TRANSIENT_SCALAR_REGISTER_GROUPS = 4
PACKED_GEMV_TRANSIENT_VECTOR_COUNT = 7
BLOCK_FP8_MMA_ACTIVATION_QUANTIZATION_BLOCK_K = 512
...
def _validate_launch(launch: Any, path: str) -> None:
    ...
    for index, descriptor in enumerate(descriptors):
        ...
        owner_indexing = descriptor["owner_indexing"]
        if owner_indexing is not None:
            owner_indexing = _require_exact_object(
                owner_indexing,
                f"{descriptor_path}.owner_indexing",
                {"kind", "stride_bytes"},
            )
            owner_indexing_kind = _require_string(
                owner_indexing["kind"],
                f"{descriptor_path}.owner_indexing.kind",
                nonempty=True,
            )
            owner_stride_bytes = _require_int(
                owner_indexing["stride_bytes"],
                f"{descriptor_path}.owner_indexing.stride_bytes",
                minimum=0,
            )
            if owner_indexing_kind == "fixed":
                if owner_stride_bytes == 0:
                    raise ValueError(
                        f"{descriptor_path}.owner_indexing fixed stride must be positive."
                    )
            elif owner_indexing_kind == "source_pool":
                if owner_stride_bytes != 0:
                    raise ValueError(
                        f"{descriptor_path}.owner_indexing source_pool must not "
                        "carry a fixed stride."
                    )
            else:
                raise ValueError(
                    f"{descriptor_path}.owner_indexing.kind must be 'fixed' or "
                    f"'source_pool', got {owner_indexing_kind!r}."
                )
        _require_string(
...
def _annotate_barrier_synchronized_helpers(kernel: dict[str, Any]) -> None:
    """Mark helper calls whose remote writes complete before their next use."""

    helper_models: dict[str, list[dict[str, Any]]] = {}
    sources: list[str] = []
    ...
    def walk(value: Any) -> None:
        if isinstance(value, dict):
            ...
            if (
                ...
                and function_name
                and isinstance(value.get("MicroKernel"), dict)
            ):
                helper_models.setdefault(function_name, []).append(value)
            for key, child in value.items():
                if (
                    isinstance(child, str)
                    and (key == "body_source" or key.endswith("BodySource"))
                    and child.strip()
                ):
    ...
    def is_full_grid_barrier(statement: ast.stmt) -> bool:
        if not isinstance(statement, ast.Expr) or not isinstance(
            statement.value, ast.Call
        ):
            return False
        ...
        return (
            ...
            and call.func.attr == "distributed_barrier"
            and isinstance(call.func.value, ast.Name)
            and call.func.value.id == "tle"
            and len(call.args) == 1
            and isinstance(call.args[0], ast.Name)
            and call.args[0].id == "PYNTT_GRID_MESH"
            and not call.keywords
        )

    walk(kernel)
    synchronized_consumers: set[str] = set()
    for source in sources:
        statements = ast.parse(source).body
        for current, following in zip(statements, statements[1:]):
            call_name = direct_call_name(current)
            if call_name is not None and is_full_grid_barrier(following):
                synchronized_consumers.add(call_name)

    for function_name, models in helper_models.items():
        if f"{function_name}__consumer" not in synchronized_consumers:
            continue
        for model in models:
            model["_BarrierSynchronizedCooperativeNTiles"] = True
...
def _kernel_backend_config(kernel: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "uniform_registers": uniform_register_limit,
        "producer_registers": producer_registers,
        "auxiliary_consumer_warps": WARP_SPECIALIZATION_AUXILIARY_CONSUMER_WARPS,
        "auxiliary_consumer_registers": WARP_SPECIALIZATION_AUXILIARY_CONSUMER_REGISTERS,
...
def _pipeline_helper_descriptor_specs(
    ...
    if template in (
        ...
        descriptor_specs = (
            (
                lambda current_model, backing:
                _n_major_k_packed_gemv_host_descriptor_spec(
                    current_model,
                    backing,
                    variant=str(current_model["MicroKernel"]["Variant"]),
                )
            )
        ...
        template in (
            ...
            "triton/kernels/qkv_parallel_linear/mma_fp8_smem_pipeline.py.jinja",
        )
    ):
        descriptor_names = (model.get("WeightDescriptorName"),)
        descriptor_specs = (
            _packed_qkv_mma_fp8_host_descriptor_spec
            if template.endswith("/mma_fp8_smem_pipeline.py.jinja")
            else _packed_qkv_gemv_host_descriptor_spec,
        )
    elif (
        template in (
            ...
            "triton/kernels/matmul_glu/mma_block_fp8_smem_pipeline.py.jinja",
            "triton/kernels/matmul_glu/mma_n_major_block_fp8_smem_pipeline.py.jinja",
        )
    ):
        ...
        descriptor_specs = tuple(
            lambda current_model, backing, current_weight=weight:
                _gated_delta_net_projection_host_descriptor_spec(
                    current_model, backing, current_weight
                )
            for weight in ("B", "A")
        )
        ...
        descriptor_names = (
            ...
            model.get("RhsScaleDescriptorName"),
        )
        descriptor_specs = (
            ...
            lambda current_model, backing: _nvfp4_n_major_host_descriptor_spec(
                current_model,
                backing,
                pointer_key="RhsScale",
            ),
        )
    elif template == "triton/kernels/nvfp4_matmul_glu/mma_tma_smem_pipeline.py.jinja":
        descriptor_names = tuple(
            model.get(f"{prefix}Weight{payload}DescriptorName")
            for prefix in ("Gate", "Up")
            for payload in ("Packed", "Scale")
        )
        descriptor_specs = tuple(
            lambda current_model, backing, current_prefix=prefix, current_payload=payload:
                _nvfp4_n_major_host_descriptor_spec(
                    current_model,
                    backing,
                    pointer_key=f"{current_prefix}Weight{current_payload}",
                )
            for prefix in ("Gate", "Up")
            for payload in ("Packed", "Scale")
        )
...
def _gated_delta_net_projection_host_descriptor_spec(
    ...
    if weight not in ("B", "A"):
        raise ValueError(
            "PyNTT GatedDeltaNet projection weight role must be 'B' or 'A', "
            f"got {weight!r}."
        )
    owner_indexed = model.get(f"{weight}WeightDescriptorOwnerIndexed")
    ...
    owner_indexing = backing["owner_indexing"]
    if owner_indexed != (owner_indexing is not None):
        raise ValueError(
            "PyNTT GatedDeltaNet projection descriptor addressing disagrees "
            f"with its backing: owner_indexed={owner_indexed}, "
            f"owner_indexing={owner_indexing!r}."
        )
    ...
    if owner_indexed:
        ...
        owner_count = _product_int(
            [
                _require_int(
                    extent,
                    f"{pointer_name} hierarchy extent",
                    minimum=1,
                )
...
def _matmul_glu_cooperative_n_tile_plan(
    model: dict[str, Any], block_n: int
) -> dict[str, int | str] | None:
    """Plan balanced N tiles when a following full-grid barrier closes writes."""

    if not model.get("_BarrierSynchronizedCooperativeNTiles"):
        return None
    raw_microkernel = model.get("MicroKernel")
    parameters = (
        raw_microkernel.get("Parameters")
        if isinstance(raw_microkernel, dict)
        else None
    )
    if (
        not isinstance(parameters, dict)
        or not isinstance(raw_microkernel.get("AuxiliaryConsumer"), dict)
        or bool(model.get("EmitPartialResults"))
        or bool(model.get("HasGateBias"))
        or bool(model.get("HasUpBias"))
    ):
        return None

    n_lane = int(model.get("NVectorLaneCount", 0))
    output = model.get("Output")
    weights = tuple(model.get(f"{prefix}Weight") for prefix in ("Gate", "Up"))
    if (
        n_lane <= 0
        or not isinstance(output, dict)
        or any(not isinstance(weight, dict) for weight in weights)
        or output.get("DistributedStorageKind") != "CanonicalGlobal"
        or any(
            weight.get("DistributedStorageKind") != "CanonicalGlobal"
            for weight in weights
        )
    ):
        return None

    hierarchy = tuple(int(value) for value in output.get("Hierarchy", ()))
    if len(hierarchy) < 2 or any(value <= 0 for value in hierarchy):
        return None
    expected_axes = list(range(len(hierarchy)))

    def has_uniform_contiguous_n_shard(
        pointer: dict[str, Any], axis: int
    ) -> bool:
        shard_axes = pointer.get("ShardAxes")
        if not isinstance(shard_axes, list) or axis >= len(shard_axes):
            return False
        stages = _shard_axis_stages(shard_axes[axis])
        return (
            pointer.get("Hierarchy") == list(hierarchy)
            and len(stages) == 1
            and (
                stages[0].get("Distribution") == "Contiguous"
                or (
                    stages[0].get("Distribution") == "BlockCyclic"
                    and int(stages[0].get("BlockSize", 0)) > 0
                )
            )
            and stages[0].get("HierarchyAxes") == expected_axes
            and all(
                not _shard_axis_stages(shard_axis)
                for index, shard_axis in enumerate(shard_axes)
                if index != axis
            )
        )

    if not has_uniform_contiguous_n_shard(output, 1) or any(
        not has_uniform_contiguous_n_shard(weight, 0) for weight in weights
    ):
        return None
    if any(
        any(_fixed(offset) != 0 for offset in pointer.get("GlobalOffsets", ()))
        for pointer in (output, *weights)
    ) or any(
        model.get(f"{prefix}WeightDescriptorOriginElements") != "0"
        for prefix in ("Gate", "Up")
    ):
        return None

    output_shape = model.get("OutputShape")
    output_global_shape = output.get("GlobalShape")
    weight_global_shapes = tuple(weight.get("GlobalShape") for weight in weights)
    if (
        not isinstance(output_shape, list)
        or len(output_shape) != 2
        or not isinstance(output_global_shape, list)
        or len(output_global_shape) != 2
        or any(
            not isinstance(shape, list) or len(shape) != 2
            for shape in weight_global_shapes
        )
    ):
        return None

    local_outer_n = _max_value(output_shape[-1])
    global_output_outer_n = _fixed(output_global_shape[-1])
    global_weight_ns = tuple(_fixed(shape[0]) for shape in weight_global_shapes)
    if (
        local_outer_n is None
        or global_output_outer_n is None
        or any(extent is None for extent in global_weight_ns)
    ):
        return None
    local_n = local_outer_n * n_lane
    global_n = global_output_outer_n * n_lane
    owner_count = prod(hierarchy)
    if (
        global_n <= 0
        or global_weight_ns != (global_n, global_n)
        or local_n * owner_count != global_n
    ):
        return None

    group_size = hierarchy[-1]
    group_n = local_n * group_size
    if group_n % block_n != 0:
        return None
    group_tiles = group_n // block_n
    local_tiles = (local_n + block_n - 1) // block_n
    if group_tiles >= local_tiles * group_size:
        return None
    base_tiles, extra_tiles = divmod(group_tiles, group_size)
    tiles_per_owner = base_tiles + int(extra_tiles != 0)
    active_n = f"({base_tiles * block_n}"
    if extra_tiles:
        active_n += (
            f" + ((shard_index % {group_size}) < {extra_tiles}).to(tl.int32)"
            f" * {block_n}"
        )
    active_n += ")"
    global_output_n = (
        f"((((shard_index // {group_size}) * {group_tiles} + "
        f"n_tile * {group_size} + (shard_index % {group_size})) * "
        f"{block_n}) + local_n)"
    )
    return {
        ...
        "tiles_per_owner": tiles_per_owner,
        "block_n": block_n,
        "global_n": global_n,
        "runtime_active_n": active_n,
        "global_output_n": global_output_n,
    }


def _cooperative_n_major_k_packed_descriptor_spec(
    spec: dict[str, Any],
    backing: dict[str, Any],
    ...
    if not entries:
        raise ValueError("PyNTT cooperative N tiling requires descriptor entries.")
    prototype = entries[0]
    prototype_shape = tuple(int(value) for value in prototype["shape"])
    if len(prototype_shape) < 2:
        raise ValueError(
            "PyNTT cooperative N tiling requires an N-major descriptor."
        )

    logical_strides = tuple(int(value) for value in backing["logical_strides"])
    vector_lanes = tuple(int(value) for value in backing["vector_lane_shape"])
    item_size = TMA_DTYPE_ITEM_SIZES[backing["scalar_dtype"]]
    scalar_row_stride = logical_strides[0] * prod(vector_lanes)
    logical_shape = tuple(int(value) for value in backing["logical_shape"])
    if len(logical_shape) != 2 or len(logical_strides) != 2:
        raise ValueError(
            "PyNTT cooperative N tiling requires a rank-2 logical backing."
        )
    block_n = int(plan["block_n"])
    source_block_shape = tuple(int(value) for value in spec["block_shape"])
    source_block_elements = prod(source_block_shape)
    if source_block_elements % block_n != 0:
        raise ValueError(
            "PyNTT cooperative N descriptor block does not contain an "
            f"integral K extent: shape={source_block_shape}, block_n={block_n}."
        )
    transfer_k = source_block_elements // block_n
    scalar_k = logical_shape[1] * prod(vector_lanes)
    innermost_k = source_block_shape[-1]
    if (
        transfer_k <= 0
        or scalar_k <= 0
        or scalar_k % innermost_k != 0
        or transfer_k % innermost_k != 0
    ):
        raise ValueError(
            "PyNTT cooperative N descriptor cannot canonicalize its K "
            f"geometry: transfer_k={transfer_k}, scalar_k={scalar_k}, "
            f"innermost_k={innermost_k}."
        )
    transfer_outer = transfer_k // innermost_k
    if transfer_outer > 1:
        block_shape = (transfer_outer, block_n, innermost_k)
        entry_shape = (scalar_k // innermost_k, block_n, innermost_k)
        entry_strides = (innermost_k, scalar_row_stride, 1)
    else:
        block_shape = (block_n, transfer_k)
        entry_shape = (block_n, scalar_k)
        entry_strides = (scalar_row_stride, 1)
    owner_count = int(plan["owner_count"])
    group_size = int(plan["group_size"])
    group_tiles = int(plan["group_tiles"])
    tiles_per_owner = int(plan["tiles_per_owner"])
    rewritten = []
    for owner in range(owner_count):
        group_index, group_lane = divmod(owner, group_size)
        for wave in range(tiles_per_owner):
            group_tile = wave * group_size + group_lane
            entry = dict(prototype)
            if group_tile < group_tiles:
                global_tile = group_index * group_tiles + group_tile
                entry["offset_bytes"] = (
                    int(backing["offset_bytes"])
                    + global_tile * block_n * scalar_row_stride * item_size
                )
                entry["shape"] = entry_shape
            else:
                entry["offset_bytes"] = int(backing["offset_bytes"])
                padding_shape = list(entry_shape)
                padding_shape[-2] = 1
                entry["shape"] = tuple(padding_shape)
            entry["strides"] = entry_strides
            entry["source_shape_axes"] = tuple(() for _ in entry_shape)
...
def _n_major_k_packed_gemv_descriptor_transfer_plan(
    pointer: dict[str, Any],
    logical_block_n: int,
) -> dict[str, Any]:
    """Decompose one logical N tile into contiguous TMA rectangles."""

    logical_plan = _n_major_k_packed_gemv_descriptor_n_plan(
        pointer, logical_block_n
    )
    block_size = logical_plan["block_size"]
    descriptor_block_n = (
        min(logical_block_n, int(block_size))
        if logical_plan["is_block_cyclic"] and block_size != 1
        else logical_block_n
    )
    descriptor_plan = _n_major_k_packed_gemv_descriptor_n_plan(
        pointer, descriptor_block_n
    )
    return {
        **descriptor_plan,
        "logical_block_n": logical_block_n,
        "descriptor_block_n": descriptor_block_n,
        "descriptors_per_logical_tile": (
            logical_block_n + descriptor_block_n - 1
        )
        // descriptor_block_n,
    }
...
def _n_major_k_packed_gemv_host_descriptor_spec(
    ...
    backing: dict[str, Any],
    *,
    family: str = "triton.matmul",
    variant: str = "mma_block_fp8_smem_pipeline",
    pointer_key: str = "Rhs",
    output_n_vector_lane_key: str = "OutputNVectorLaneCount",
    required_workspace_names: tuple[str, ...] | None = None,
    output_outer_n: int | None = None,
    output_n_lanes: int | None = None,
) -> dict[str, Any]:
    ...
    is_block_fp8_variant = variant in (
        "mma_block_fp8_smem_pipeline",
        "mma_n_major_block_fp8_smem_pipeline",
    )
    reduction_group = (
        _require_int(model.get("WeightBlockK"), "WeightBlockK", minimum=1)
        if is_block_fp8_variant
        else transfer_block_k
    )
    logical_shape = tuple(int(value) for value in backing["logical_shape"])
    logical_strides = _normalize_singleton_strides(
        logical_shape,
    ...
    if (
        ...
        or block_k % transfer_block_k != 0
        or not (
            reduction_group % transfer_block_k == 0
            or transfer_block_k % reduction_group == 0
        )
    ):
        raise ValueError(
            "PyNTT N-major K-packed GEMV descriptor tile is incompatible with "
            f"block_n={logical_block_n}, block_k={block_k}, "
            f"transfer_block_k={transfer_block_k}, k_atom={k_atom}."
        )
    k_outer_extent = transfer_block_k // k_atom
    reduction_groups_per_transfer = transfer_block_k // reduction_group

    pointer = model[pointer_key]
    n_plan = _n_major_k_packed_gemv_descriptor_transfer_plan(
        pointer, logical_block_n
    )
    descriptor_block_n = int(n_plan["descriptor_block_n"])
    descriptors_per_logical_tile = int(
        n_plan["descriptors_per_logical_tile"]
    )
    descriptor_block_shape = (
        (reduction_groups_per_transfer,)
        + tuple(n_plan["block_shape"])
        + (reduction_group,)
        if reduction_groups_per_transfer > 1
        else tuple(n_plan["block_shape"]) + (transfer_block_k,)
    )
    ...
    owner_entries: list[list[dict[str, Any] | None]] = []
    ...
    for linear_owner in range(owner_count):
        ...
        n_entry = _tma_descriptor_table_axis_entry(
            ...
            0,
            owner,
            tile_extent=descriptor_block_n,
            context="N-major K-packed GEMV descriptor N",
        )
        ...
        maximum_tiles_per_owner = max(
            maximum_tiles_per_owner, logical_tile_count
        )
        ...
        entries_for_owner = []
        for tile_index in range(logical_tile_count):
            for descriptor_index in range(descriptors_per_logical_tile):
                local_n_offset = (
                    tile_index * logical_block_n
                    + descriptor_index * descriptor_block_n
                )
                if local_n_offset >= local_n_extent:
                    entries_for_owner.append(None)
                    continue
                ...
                if len(raw_coordinates) != len(n_entry["stride_multipliers"]):
                    raise ValueError(
                        ...
                        f"differ: {raw_coordinates}/"
                        f"{n_entry['stride_multipliers']}."
                    )
                ...
                tile_n_extent = min(
                    descriptor_block_n, local_n_extent - local_n_offset
                )
                if n_entry["is_block_cyclic"] and n_entry["block_size"] != 1:
                    block_size = int(n_entry["block_size"])
                    if descriptor_block_n <= block_size:
                ...
                if reduction_groups_per_transfer > 1:
                    if local_k_extent % reduction_group != 0:
                        raise ValueError(
                            "PyNTT merged N-major K-packed GEMV descriptor K "
                            f"extent {local_k_extent} is not divisible by "
                            f"the reduction group {reduction_group}."
                        )
                    descriptor_shape = (
                        (local_k_extent // reduction_group,)
                        + descriptor_n_shape
                        + (reduction_group,)
                    )
                    descriptor_strides = (
                        (reduction_group,) + descriptor_n_strides + (1,)
                    )
                else:
                ...
                entries_for_owner.append(
                    {
                        "owner_index": _tma_physical_owner_index(
                            backing, linear_owner
                        ),
                        "offset_bytes": int(backing["offset_bytes"])
                        + (
    ...
    if output_outer_n is None or output_outer_n <= 0:
        raise ValueError(
            "PyNTT N-major K-packed GEMV requires a bounded local output N axis."
        )
    if output_n_lanes is None:
        output_n_lanes = int(model[output_n_vector_lane_key])
    if output_n_lanes <= 0:
        raise ValueError(
            "PyNTT N-major K-packed GEMV requires positive output N lanes."
        )
    ...
    first_entry = next(
        ...
        for entry in entries_for_owner
    ...
    expected_entries_per_owner = (
        maximum_tiles_per_owner * descriptors_per_logical_tile
    )
    entries = tuple(
        padding_entry if entry is None else entry
        for entries_for_owner in owner_entries
        for entry in (
            entries_for_owner
            + [None] * (expected_entries_per_owner - len(entries_for_owner))
        )
    )
    ...
    cooperative_plan = (
        _matmul_glu_cooperative_n_tile_plan(model, logical_block_n)
        if family == "triton.matmul_glu"
        and variant == "mma_n_major_block_fp8_smem_pipeline"
        else None
    )
    return (
        _cooperative_n_major_k_packed_descriptor_spec(
            spec, backing, cooperative_plan
        )
...
def _tma_owner_backing_base_elements(
    ...
    *,
    ...
    if storage_kind == "CompactPerOwner" or (
        storage_kind == "CompactLocal" and backing["owner_indexing"] is not None
    ):
        return 0
    if storage_kind not in ("CompactLocal", "CanonicalGlobal"):
        raise ValueError(
...
def _tma_physical_owner_index(
    backing: dict[str, Any], logical_owner: int
) -> int:
    """Map a logical descriptor-table owner to its physical backing owner."""

    return logical_owner if backing["owner_indexing"] is not None else 0
...
def _nvfp4_n_major_host_descriptor_spec(
    ...
    *,
    ...
    """Build TMA views over one N-major NVFP4 weight or scale payload."""
    ...
    vector_lane_shape = tuple(int(value) for value in backing["vector_lane_shape"])
    is_scale = pointer_key.endswith("Scale")
    expected_dtype = "float8e4m3fn" if is_scale else "uint8"
    expected_vector_lane_shape = () if is_scale else (2, 16)
    scalar_lanes_per_logical_element = (
        1
        if is_scale
        else _product_int(list(expected_vector_lane_shape))
    )
    expected_local_k_outer = (
        context["fixed_k"] // context["group_size"]
        if is_scale
        else context["fixed_k"] // 2 // scalar_lanes_per_logical_element
    )
    if (
        ...
        raise ValueError(
            "PyNTT NVFP4 TMA requires an N-major weight/scale backing with a "
    ...
    transfer_k_outer = (
        context["scale_block_k"]
        if is_scale
        else context["block_k"] // 2 // scalar_lanes_per_logical_element
    )
    n_plan = _tma_canonical_axis_plan(
        pointer,
        0,
        tile_extent=block_n,
        context=f"NVFP4 {pointer_key} TMA descriptor N",
    )
    if is_scale:
        k_payload_plan = None
        ...
        descriptor_block_shape = context["scale_tma_block_shape"]
    else:
        k_payload_plan = _tma_packed_atom_axis_plan(
            pointer,
            1,
            tile_extent=transfer_k_outer,
            atom_extent=scalar_lanes_per_logical_element,
    ...
    owner_count = _product_int([int(value) for value in hierarchy])
    owner_entries: list[dict[str, Any] | None] = []
    maximum_tiles_per_owner = 0
    ...
    for linear_owner in range(owner_count):
        ...
        descriptor_n_shape = tuple(
            int(n_entry["descriptor_shape"][dimension])
            for dimension in n_plan["retained_dimensions"]
        )
        descriptor_row_stride = (
        ...
        if is_scale:
            retained_k_dimensions = tuple(k_plan["retained_dimensions"])
            descriptor_k_shape = tuple(
                int(k_entry["descriptor_shape"][dimension])
                for dimension in retained_k_dimensions
            )
            descriptor_k_strides = tuple(
                int(k_entry["stride_multipliers"][dimension])
                * logical_strides[1]
                for dimension in retained_k_dimensions
            )
        else:
            descriptor_k_shape, descriptor_k_strides = _tma_packed_atom_entry(
                k_entry, k_payload_plan
            )
        descriptor_shape = descriptor_n_shape + descriptor_k_shape
        owner_entries.append(
            {
                "owner_index": _tma_physical_owner_index(backing, linear_owner),
                "offset_bytes": int(backing["offset_bytes"])
                + base_scalar_elements * item_size,
                "shape": descriptor_shape,
                "strides": descriptor_n_strides + descriptor_k_strides,
    ...
    first_entry = next(entry for entry in owner_entries if entry is not None)
    padding_entry = {
        **first_entry,
    ...
    entries = tuple(
        entry if entry is not None else padding_entry for entry in owner_entries
    )
    return {
        "kind": "table",
        "name": backing["name"],
...
def _packed_qkv_gemv_host_descriptor_spec(
    ...
    return _k_major_gemv_host_descriptor_spec(
        ...
        expected_vector_lane_shape=_packed_gemv_vector_lane_shape(model),
        transpose_kn=True,
    )


def _packed_qkv_mma_fp8_host_descriptor_spec(
    model: dict[str, Any],
    backing: dict[str, Any],
) -> dict[str, Any]:
    output_n_lanes = _require_int(
        model.get("NVectorLaneCount"), "NVectorLaneCount", minimum=1
    )
    total_n = sum(_packed_qkv_fixed_projection_ns(model).values())
    if total_n % output_n_lanes != 0:
        raise ValueError(
            "PyNTT fused QKV projection capacity is not output-lane aligned: "
            f"total_n={total_n}, lanes={output_n_lanes}."
        )
    return _n_major_k_packed_gemv_host_descriptor_spec(
        model,
        backing,
        family="triton.qkv_parallel_linear",
        variant="mma_fp8_smem_pipeline",
        pointer_key="Weight",
        output_outer_n=total_n // output_n_lanes,
        output_n_lanes=output_n_lanes,
    )
...
def _packed_matmul_glu_gemv_host_descriptor_spec(
    ...
    raw_parameters = model["MicroKernel"].get("Parameters")
    required_workspace_names = None
    if (
        isinstance(raw_parameters, dict)
        and raw_parameters.get("split_projection_channels") == 1
    ):
        required_workspace_names = (
            "gate_rhs_stage",
            "up_rhs_stage",
            "lhs_quantized",
            "lhs_scale",
        ) + (("lhs_stage",) if "lhs_stage_extent" in raw_parameters else ())
    microkernel = _microkernel_context(
        model,
        "triton.matmul_glu",
        str(model["MicroKernel"]["Variant"]),
    ...
    if microkernel["variant"] == "mma_n_major_block_fp8_smem_pipeline":
        return _n_major_k_packed_gemv_host_descriptor_spec(
            model,
            backing,
            family="triton.matmul_glu",
            variant="mma_n_major_block_fp8_smem_pipeline",
            pointer_key=f"{prefix}Weight",
            output_n_vector_lane_key="NVectorLaneCount",
            required_workspace_names=required_workspace_names,
        )
    if microkernel["parameters"].get("owner_major_weight") == 1:
        return _owner_major_k_major_gemv_host_descriptor_spec(
            model,
            backing,
            block_n=microkernel["parameters"]["block_n"],
            block_k=microkernel["parameters"]["block_k"],
...
def _owner_major_k_major_gemv_host_descriptor_spec(
    model: dict[str, Any],
    backing: dict[str, Any],
    *,
    block_n: int,
    ...
    """Describe an offline owner-major copy of a balanced K-major weight."""

    canonical = _k_major_gemv_host_descriptor_spec(
        model,
        backing,
        block_n=block_n,
        block_k=block_k,
        pointer=pointer,
        expected_vector_lane_shape=expected_vector_lane_shape,
    )
    entries = tuple(canonical["entries"])
    if not entries:
        raise ValueError("PyNTT owner-major GEMV requires descriptor entries.")
    local_shape = tuple(entries[0]["shape"])
    if any(tuple(entry["shape"]) != local_shape for entry in entries):
        raise ValueError(
            "PyNTT owner-major GEMV requires one fixed local shape per owner."
        )
    item_size = TMA_DTYPE_ITEM_SIZES[backing["scalar_dtype"]]
    owner_stride_elements = _product_int(list(local_shape))
    local_strides = []
    stride = 1
    for extent in reversed(local_shape):
        local_strides.append(stride)
        stride *= int(extent)
    local_strides.reverse()
    owner_entries = tuple(
        {
            **entry,
            "offset_bytes": int(backing["offset_bytes"])
            + owner * owner_stride_elements * item_size,
            "strides": tuple(local_strides),
            "source_shape_axes": tuple(() for _ in local_shape),
        }
        for owner, entry in enumerate(entries)
    )
    return {**canonical, "entries": owner_entries}
...
def _packed_gemv_vector_lane_shape(
    model: dict[str, Any],
    *,
    prefix: str = "",
) -> tuple[int, int, int]:
    """Read the packed weight ABI from the compiler-emitted model contract."""

    return (
        _require_int(
            model.get(f"{prefix}NVectorLaneCount"),
            f"{prefix}NVectorLaneCount",
            minimum=1,
        ),
        _require_int(
            model.get(f"{prefix}KPackLaneCount"),
            f"{prefix}KPackLaneCount",
            minimum=1,
        ),
        _require_int(
            model.get(f"{prefix}KVectorLaneCount"),
            f"{prefix}KVectorLaneCount",
            minimum=1,
        ),
    )


def _packed_gemv_tma_payload_shape(
    n_lane: int,
    k_pack: int,
    k_lane: int,
    scalar_dtype: str,
) -> tuple[int, int]:
    """Split one packed weight element into an outer axis and <=128B payload."""

    item_size = TMA_DTYPE_ITEM_SIZES.get(scalar_dtype)
    if item_size is None:
        raise ValueError(
            "PyNTT packed GEMV TMA does not support scalar dtype "
            f"{scalar_dtype!r}."
        )
    total_lanes = n_lane * k_pack * k_lane
    maximum_contiguous_lanes = 128 // item_size
    contiguous_lanes = min(total_lanes, maximum_contiguous_lanes)
    if contiguous_lanes <= 0 or total_lanes % contiguous_lanes != 0:
        raise ValueError(
            "PyNTT packed GEMV vector payload cannot be decomposed into a "
            f"128-byte TMA atom: lanes=({n_lane},{k_pack},{k_lane}), "
            f"dtype={scalar_dtype!r}."
        )
    return total_lanes // contiguous_lanes, contiguous_lanes
...
def _k_major_gemv_host_descriptor_spec(
    ...
    *,
    ...
    payload_outer_extent, contiguous_extent = _packed_gemv_tma_payload_shape(
        n_lane,
        k_pack,
        k_lane,
    ...
    ) + (payload_outer_extent, contiguous_extent)
    ...
    for linear_owner in range(owner_count):
        ...
        ) + (payload_outer_extent, descriptor_contiguous_extent)
    ...
    *,
    ...
    if kernel_config is not None:
        ...
        if model.get("AuxiliaryConsumerFunctionName"):
            model["AuxiliaryConsumerWarps"] = kernel_config[
                "auxiliary_consumer_warps"
            ]
            model["AuxiliaryConsumerRegisters"] = kernel_config[
...
def _make_env() -> Environment:
    ...
    env.globals.update(
        ...
        load_zero_literal=_load_zero_literal,
        ...
        packed_block_fp8_mma_matmul_glu_gemv_pipeline_context=(
            _packed_block_fp8_mma_matmul_glu_gemv_pipeline_template_context
        ),
        packed_block_fp8_mma_n_major_matmul_glu_gemv_pipeline_context=(
            _packed_block_fp8_mma_n_major_matmul_glu_gemv_pipeline_template_context
        ),
        ...
        packed_fp8_qkv_mma_gemv_pipeline_context=(
            _packed_fp8_qkv_mma_gemv_pipeline_template_context
        ),
        ...
        pointer_local_vector_to_global_scalar_coordinate=(
        ...
        slice_context=_slice_template_context,
...
def _load_zero_literal(dtype: Any) -> str:
    """Return a scalar zero Triton can cast to the masked-load element type."""

    dtype_name = str(dtype)
    if dtype_name in {"float8e4m3fn", "float8e5m2"}:
        return "0.0"
    if dtype_name in {
        "bool",
        "int8",
        ...
        return "0"
    raise ValueError(f"Unsupported PyNTT masked-load dtype {dtype_name!r}.")
...
def _slice_template_context(model: dict[str, Any]) -> dict[str, Any]:
    """Build one lane-preserving logical Slice access plan."""

    input_shape = model["InputShape"]
    output_shape = model["OutputShape"]
    input_strides = model["InputStrides"]
    output_strides = model["OutputStrides"]
    starts = tuple(int(value) for value in model["Starts"])
    slice_strides = tuple(int(value) for value in model["Strides"])
    rank = len(input_shape)
    if not (
        len(output_shape)
        == len(input_strides)
        ...
        raise ValueError(
            "PyNTT Slice requires equal input/output/parameter ranks: "
            f"input={rank}, output={len(output_shape)}, "
            f"input_strides={len(input_strides)}, "
            f"output_strides={len(output_strides)}, starts={len(starts)}, "
            f"slice_strides={len(slice_strides)}"
        )
    if any(stride == 0 for stride in slice_strides):
        raise ValueError(f"PyNTT Slice strides must be non-zero, got {slice_strides}.")

    input_lanes = _validate_coordinate_lane_shape(
        model["InputVectorLaneShape"], "PyNTT Slice input"
    )
    output_lanes = _validate_coordinate_lane_shape(
        model["OutputVectorLaneShape"], "PyNTT Slice output"
    )
    if input_lanes != output_lanes:
        raise ValueError(
            "PyNTT Slice must preserve vector lanes: "
            f"input={input_lanes}, output={output_lanes}"
        )
    ...
    input_coordinates = tuple(
        _add_coordinate(starts[axis], coordinate)
        if slice_strides[axis] == 1
        else f"({starts[axis]}) + ({coordinate}) * {slice_strides[axis]}"
        for axis, coordinate in enumerate(ctx["tensor_coordinates"])
    )
    ctx["input_access"] = _tensor_access(
        input_coordinates,
...
def _qkv_input_active_shape(model: dict[str, Any]) -> list[Any]:
    storage_shape = _require_list(model.get("InputShape"), "InputShape")
    active_shape = _require_list(model.get("InputActiveShape"), "InputActiveShape")
    if len(active_shape) != len(storage_shape):
        raise ValueError(
            "PyNTT QKV input active/storage ranks differ: "
            f"active={len(active_shape)}, storage={len(storage_shape)}."
        )
    for axis, (active_extent, storage_extent) in enumerate(
        zip(active_shape, storage_shape, strict=True)
    ):
        active_min = _min_value(active_extent)
        active_max = _max_value(active_extent)
        storage_max = _max_value(storage_extent)
        if active_min is None or active_max is None or storage_max is None:
            raise ValueError(
                "PyNTT QKV input extents require bounded active and storage ranges: "
                f"axis={axis}, active={_dim(active_extent)}, "
                f"storage={_dim(storage_extent)}."
            )
        if active_min < 0 or active_max > storage_max:
            raise ValueError(
                "PyNTT QKV input active extent exceeds its storage capacity: "
                f"axis={axis}, active=[{active_min},{active_max}], "
                f"storage_max={storage_max}."
            )
    return active_shape
...
def _microkernel_context(
    ...
    *,
    ...
    if has_complete_consumer_lhs_stage and not (
        ...
        and (
            variant in simt_staged_variants
            or variant
            in (
                "mma_block_fp8_smem_pipeline",
                "mma_n_major_block_fp8_smem_pipeline",
        ...
        raise ValueError(
            ...
            "triton.matmul or triton.matmul_glu with a SIMT or MMA "
    ...
    if required_offsets is None:
        required_offsets = {
            ...
            "mma_fp8_smem_pipeline": ("rhs_stage",),
            "mma_block_fp8_smem_pipeline": (
                "rhs_stage",
                "lhs_quantized",
                "lhs_scale",
            ) + (("lhs_stage",) if has_complete_consumer_lhs_stage else ()),
            "mma_n_major_block_fp8_smem_pipeline": (
                "rhs_stage",
                "lhs_quantized",
                "lhs_scale",
    ...
    if variant in (
    ...
    elif family == "triton.gated_delta_net" and variant == "recurrent_core":
        expected_consumer_workspaces = ("projection_stage",)
    elif variant == "mma_smem_pipeline" or (
        variant in simt_staged_variants
        and (
            family == "triton.qkv_parallel_linear"
            or has_complete_consumer_lhs_stage
        ...
        expected_consumer_workspaces = ("lhs_stage",)
    else:
        expected_consumer_workspaces = ()
    ...
    raw_auxiliary_consumer = microkernel.get("AuxiliaryConsumer")
    auxiliary_consumer = None
    if raw_auxiliary_consumer is not None:
        auxiliary_consumer = _require_exact_object(
            raw_auxiliary_consumer,
            "microkernel.AuxiliaryConsumer",
            {"TransferPipelineChannelNames", "SharedWorkspaceNames"},
        )
        auxiliary_channel_names = tuple(
            _require_string_list(
                auxiliary_consumer["TransferPipelineChannelNames"],
                "microkernel.AuxiliaryConsumer.TransferPipelineChannelNames",
            )
        )
        auxiliary_workspace_names = tuple(
            _require_string_list(
                auxiliary_consumer["SharedWorkspaceNames"],
                "microkernel.AuxiliaryConsumer.SharedWorkspaceNames",
            )
        )
        if not auxiliary_channel_names or len(set(auxiliary_channel_names)) != len(
            auxiliary_channel_names
        ):
            raise ValueError(
                "microkernel.AuxiliaryConsumer transfer channels must be "
                "non-empty and unique."
            )
        if len(set(auxiliary_workspace_names)) != len(auxiliary_workspace_names):
            raise ValueError(
                "microkernel.AuxiliaryConsumer Shared workspaces must be unique."
            )
        raw_channels = microkernel.get("TransferPipelineChannels")
        channel_names = {
            channel.get("Name")
            for channel in raw_channels
            if isinstance(raw_channels, list) and isinstance(channel, dict)
        } if isinstance(raw_channels, list) else set()
        if not set(auxiliary_channel_names).issubset(channel_names):
            raise ValueError(
                "microkernel.AuxiliaryConsumer references unknown transfer channels."
            )
        if not set(auxiliary_workspace_names).issubset(consumer_workspaces):
            raise ValueError(
                "microkernel.AuxiliaryConsumer references Shared workspaces not "
                "owned by the consumer."
            )
        auxiliary_consumer = {
            "transfer_pipeline_channel_names": auxiliary_channel_names,
            "shared_workspace_names": auxiliary_workspace_names,
        }
    ...
    return {
        ...
        "auxiliary_consumer": auxiliary_consumer,
    }
...
def _nvfp4_projection_template_context(
    ...
    *,
    ...
    weight_scale_pointer_keys: tuple[str, ...],
) -> dict[str, Any]:
    """Validate the selected decode NVFP4 projection's physical storage ABI."""

    projection_count = len(packed_weight_pointer_keys)
    if projection_count < 1 or len(weight_scale_pointer_keys) != projection_count:
        raise ValueError(
            "PyNTT NVFP4 projection requires matching non-empty packed-weight "
            "and weight-scale pointer sets."
        )
    ...
    microkernel = _microkernel_context(
        ...
        expected_family,
        "mma_tma_smem_pipeline",
        required_workspace_names=("packed_weight_stage", "weight_scale_stage"),
    )
    ...
    if num_stages % projection_count != 0 or num_stages // projection_count < 2:
        raise ValueError(
            "PyNTT NVFP4 TMA pipeline requires at least two complete logical "
            f"stages, got physical_stages={num_stages}, "
            f"slots_per_logical_stage={projection_count}."
        )
    expected_workspace_shapes = {
        "packed_weight_stage": (num_stages, block_n, block_k // 2),
        "weight_scale_stage": (num_stages, block_n, block_k // group_size),
    }
    ...
    scale_plans = []
    for pointer_key in weight_scale_pointer_keys:
        pointer = model.get(pointer_key)
        if not isinstance(pointer, dict):
            raise ValueError(
                f"PyNTT NVFP4 {pointer_key} pointer metadata must be an object."
        ...
        if len(tma_block_shape) > 5:
            raise ValueError(
                "PyNTT NVFP4 scale TMA descriptor exceeds the hardware rank-5 limit: "
                f"pointer={pointer_key}, block_shape={tma_block_shape}."
            )
        if _product_int(list(tma_block_shape)) != block_n * scale_block_k:
            raise ValueError(
                "PyNTT NVFP4 scale TMA block does not cover the selected Shared tile: "
                f"pointer={pointer_key}, block_shape={tma_block_shape}, "
                f"expected_elements={block_n * scale_block_k}."
            )
        tma_offsets = (
            tuple("0" for _ in n_plan["block_shape"])
            + _tma_shared_axis_coordinates(
                f"k_tile * {scale_block_k}", k_plan
            )
        )
        scale_plans.append(
            (pointer_key, n_plan, k_plan, tma_block_shape, tma_offsets)
        )

    if not scale_plans:
        raise ValueError("PyNTT NVFP4 projection requires a weight-scale pointer.")
    (
        _,
        scale_n_plan,
        scale_k_plan,
        scale_tma_block_shape,
        scale_tma_offsets,
    ) = scale_plans[0]
    for pointer_key, n_plan, k_plan, tma_block_shape, tma_offsets in scale_plans[1:]:
        if (
            tuple(n_plan["block_shape"]) != tuple(scale_n_plan["block_shape"])
            or tuple(k_plan["block_shape"]) != tuple(scale_k_plan["block_shape"])
            or tma_block_shape != scale_tma_block_shape
            or tma_offsets != scale_tma_offsets
        ):
            raise ValueError(
                "PyNTT fused NVFP4 projections require one shared scale TMA tile ABI, "
                f"but {pointer_key} uses {tma_block_shape} and the first "
                f"projection uses {scale_tma_block_shape}."
            )
    ...
    packed_k_offsets = packed_tma_offsets[len(packed_n_plan["block_shape"]) :]
    scale_k_offsets = scale_tma_offsets[len(scale_n_plan["block_shape"]) :]
    packed_tma_offsets_by_n_tile = tuple(
        _tma_shared_axis_coordinates(str(n_tile * block_n), packed_n_plan)
        + packed_k_offsets
        for n_tile in range(num_n_tiles)
    )
    scale_tma_offsets_by_n_tile = tuple(
        _tma_shared_axis_coordinates(str(n_tile * block_n), scale_n_plan)
        + scale_k_offsets
        for n_tile in range(num_n_tiles)
    )
    ...
    return {
        ...
        "packed_tma_offsets_by_n_tile": packed_tma_offsets_by_n_tile,
        "scale_block_k": scale_block_k,
        "scale_n_plan": scale_n_plan,
        "scale_k_plan": scale_k_plan,
        "scale_tma_block_shape": scale_tma_block_shape,
        "scale_tma_offsets": scale_tma_offsets,
        "scale_tma_offsets_by_n_tile": scale_tma_offsets_by_n_tile,
        "group_size": group_size,
        "input_access": _tensor_access(
...
def _nvfp4_matmul_template_context(model: dict[str, Any]) -> dict[str, Any]:
    context = _nvfp4_projection_template_context(
        ...
        n_tiles_per_activation_batch=5,
        ...
        weight_scale_pointer_keys=("RhsScale",),
    )
    context.update(
        ...
        scale_descriptor_name=_require_string(
            model.get("RhsScaleDescriptorName"),
            "RhsScaleDescriptorName",
            nonempty=True,
        ),
    ...
    if not isinstance(model["HasAddend"], bool):
        raise ValueError("PyNTT NVFP4MatMul HasAddend must be a boolean.")
    if not isinstance(model["HasNormStats"], bool):
        raise ValueError("PyNTT NVFP4MatMul HasNormStats must be a boolean.")
    output_lanes = tuple(int(value) for value in model["OutputVectorLanes"])
    if model["HasAddend"]:
        if len(model["AddendShape"]) != len(model["OutputShape"]):
            raise ValueError(
                "PyNTT NVFP4MatMul addend/output ranks must match: "
                f"addend={len(model['AddendShape'])}, output={len(model['OutputShape'])}."
            )
        context["addend_access"] = _tensor_access(
            ("0", f"offs_n // {output_lanes[0]}"),
            model["AddendStrides"],
            (f"offs_n % {output_lanes[0]}",),
            output_lanes,
        ...
        context["addend_access"] = None
    if model["HasNormStats"]:
        if int(model["NormAxis"]) != len(model["OutputShape"]) - 1:
            raise ValueError(
                "PyNTT NVFP4MatMul normalization statistics must reduce the final output axis."
            )
        if len(model["StatsShape"]) != len(model["OutputShape"]) + 1:
            raise ValueError(
                "PyNTT NVFP4MatMul statistics rank must be output rank plus one."
            )
        if not isinstance(model["UseMean"], bool):
            raise ValueError("PyNTT NVFP4MatMul UseMean must be a boolean.")
        context["stats_accesses"] = tuple(
...
def _nvfp4_matmul_glu_template_context(model: dict[str, Any]) -> dict[str, Any]:
    context = _nvfp4_projection_template_context(
        ...
        weight_scale_pointer_keys=("GateWeightScale", "UpWeightScale"),
    )
        ...
        projections=tuple(
            {
                ...
                "scale_descriptor_name": _require_string(
                    model.get(f"{prefix}WeightScaleDescriptorName"),
                    f"{prefix}WeightScaleDescriptorName",
                    nonempty=True,
                ),
                "packed_weight_access": _tensor_access(
                    (
    ...
    context["shared_input_quantization"] = (
        model["GateInputGlobalScale"] == model["UpInputGlobalScale"]
    )
    input_normalization = model.get("InputNormalization")
    if input_normalization is not None:
        context["input_normalization"] = _nvfp4_input_normalization_context(
            model, context
        )
    else:
        context["input_normalization"] = None
    if (
        context["num_n_tiles"]
        * context["num_k_tiles"]
...
def _nvfp4_input_normalization_context(
    model: dict[str, Any], projection_context: dict[str, Any]
) -> dict[str, Any]:
    """Prepare pooled distributed activation reads and partial RMS statistics."""

    normalization = model.get("InputNormalization")
    if not isinstance(normalization, dict):
        raise ValueError(
            "PyNTT normalized NVFP4MatMulGlu requires InputNormalization metadata."
        )
    if model["Input"].get("DistributedStorageKind") not in (
        "CompactLocal",
        "CompactPerOwner",
    ):
        raise ValueError(
            "PyNTT normalized NVFP4MatMulGlu input must use compact distributed storage."
        )
    input_shape = model["InputShape"]
    input_strides = model["InputStrides"]
    input_pointer = model["Input"]
    input_lanes = _validate_coordinate_lane_shape(
        model["InputVectorLanes"], "PyNTT normalized NVFP4MatMulGlu input"
    )
    if len(input_shape) != 2 or len(input_strides) != 2 or len(input_lanes) != 1:
        raise ValueError(
            "PyNTT normalized NVFP4MatMulGlu requires rank-2 input with one "
            "contiguous vector-lane axis."
        )
    if int(normalization["Axis"]) != 1:
        raise ValueError(
            "PyNTT normalized NVFP4MatMulGlu requires the final input axis."
        )
    lane_count = input_lanes[0]
    hierarchy = tuple(int(extent) for extent in normalization["Hierarchy"])
    if tuple(int(extent) for extent in input_pointer["Hierarchy"]) != hierarchy:
        raise ValueError(
            "PyNTT normalized NVFP4MatMulGlu input and statistics hierarchies differ."
        )
    if any(_fixed(offset) != 0 for offset in input_pointer["GlobalOffsets"]):
        raise ValueError(
            "PyNTT normalized NVFP4MatMulGlu requires a full logical input view."
        )
    input_shard_axes = input_pointer["ShardAxes"]
    if len(input_shard_axes) != 2:
        raise ValueError(
            "PyNTT normalized NVFP4MatMulGlu input shard mapping must have rank 2."
        )
    if _shard_axis_stages(input_shard_axes[0]):
        raise ValueError(
            "PyNTT normalized NVFP4MatMulGlu does not support a split M axis."
        )

    global_packed_k = f"offs_k // {lane_count}"
    input_plan = _global_to_local_coordinate(
        global_packed_k,
        input_pointer["GlobalShape"][1],
        input_shard_axes[1],
        list(hierarchy),
    )
    owner_coordinates = tuple(
        input_plan["owners"].get(axis, f"shard_coord{axis}")
        for axis in range(len(hierarchy))
    )
    source_shard_index = _flatten_coordinates(owner_coordinates, hierarchy)
    input_address = normalization["InputAddress"]
    source_pool_index = _pool_index_expression(
        source_shard_index, input_address["PoolScopeSize"]
    )
    input_scalar_offset = (
        f"(({input_plan['local_coordinate']}) * "
        f"({_dim(input_strides[1])}) * {lane_count} + (offs_k % {lane_count}))"
    )

    scale_lanes = _validate_coordinate_lane_shape(
        normalization["NormScaleVectorLanes"],
        "PyNTT normalized NVFP4MatMulGlu scale",
    )
    bias_lanes = _validate_coordinate_lane_shape(
        normalization["NormBiasVectorLanes"],
        "PyNTT normalized NVFP4MatMulGlu bias",
    )
    if scale_lanes != input_lanes or bias_lanes != input_lanes:
        raise ValueError(
            "PyNTT normalized NVFP4MatMulGlu input/scale/bias lane layouts differ."
        )
    if len(normalization["NormScaleShape"]) != 1 or len(
        normalization["NormBiasShape"]
    ) != 1:
        raise ValueError(
            "PyNTT normalized NVFP4MatMulGlu scale and bias must have rank 1."
        )

    partial = _partial_reduction_context(
        normalization["StatsDType"],
        normalization["StatsTritonDType"],
        normalization["PartialStatsAddress"],
        hierarchy,
        tuple(int(axis) for axis in normalization["PartialAxes"]),
    )
    component_count = 2 if normalization["UseMean"] else 1
    stats_strides = normalization["StatsStrides"]
    if len(stats_strides) < 1:
        raise ValueError(
            "PyNTT normalized NVFP4MatMulGlu statistics must retain a component axis."
        )
    stats_accesses = tuple(
        _tensor_access(
            (str(component),) + ("0",) * (len(stats_strides) - 1),
            stats_strides,
        )
        for component in range(component_count)
    )
    return {
        "has_bias": bool(normalization["HasBias"]),
        "epsilon": normalization["Epsilon"],
        "use_mean": bool(normalization["UseMean"]),
        "input_address": input_address,
        "input_pointer_type": _pointer_type(
            model["InputTritonDType"], input_address["AddressSpace"]
        ),
        "input_scalar_offset": input_scalar_offset,
        "source_pool_index": source_pool_index,
        "partial": partial,
        "stats_accesses": stats_accesses,
        "normalization_size": _dim(projection_context["k"]),
    }
...
def _qkv_parallel_linear_template_context(
    ...
    k = _qkv_input_active_shape(model)[-1]
...
def _matmul_glu_weight_lane_shape(
    ...
    rhs_layout = model.get("RhsLayout", "n_major")
    if not packed or rhs_layout == "n_major":
        return n_lane_shape
...
def _matmul_glu_weight_access(
    ...
    *,
    ...
    rhs_layout = model.get("RhsLayout", "n_major")
    n_lane_shape = _matmul_glu_lane_shape(model, packed=packed)
    if rhs_layout != "n_major_k_packed" and tuple(n_lane_shape) != tuple(n_axis["lane_shape"]):
        raise ValueError(
    ...
    if not packed:
        ...
        lane_coordinates = ()
    elif rhs_layout == "n_major":
        matrix_coordinates = (n_axis["physical_coordinate"], k_expr)
        lane_coordinates = n_axis["lane_coordinates"]
    elif rhs_layout == "n_major_k_packed":
        k_pack = int(model["KPackLaneCount"])
        k_lane = int(model["KVectorLaneCount"])
        k_atom = k_pack * k_lane
        matrix_coordinates = (
...
def _matmul_glu_output_access(
    ...
    *,
    ...
    coordinates = _aligned_batch_coordinates(
        model[f"{prefix}Shape"], 2, output_batch_rank
    ) + (m_expr, n_axis["physical_coordinate"])
    return _tensor_access(
        coordinates,
        model[f"{prefix}Strides"],
...
def _matmul_glu_template_context(
    ...
    up_output_access = (
        ...
        if bool(model.get("EmitPartialResults"))
        else None
    )
    projections = []
    ...
    context.update(
        ...
        output_access=output_access,
        up_output_access=up_output_access,
        output_structured_shape=output_structured_shape,
...
def _matmul_glu_weight_k_index(
    ...
    if model.get("RhsLayout", "n_major") in ("n_major", "n_major_k_packed"):
...
def _matmul_glu_weight_n_index(
    ...
    *,
    ...
    weight_axis = -2 if model.get("RhsLayout", "n_major") in (
        "n_major",
        "n_major_k_packed",
    ) else -1
...
def _validate_packed_gemv_pipeline_resource_contract(
    *,
    ...
    if (
        ...
        raise ValueError(
            f"PyNTT {algorithm} resource contract requires "
            f"a power-of-two block_n in [8, {maximum_block_n}], "
            "block_k in [128, 1024], "
...
def _gated_delta_net_convolution_template_context(
    ...
    physical_local_n = _require_int(
        model.get("LocalConvDim"), "LocalConvDim", minimum=1
    )
    local_weight_n = _require_int(
        model.get("LocalWeightConvDim"), "LocalWeightConvDim", minimum=1
    )
    global_n = _require_int(model.get("GlobalConvDim"), "GlobalConvDim", minimum=1)
    lane_count = _require_int(
        model.get("ActivationLaneCount"), "ActivationLaneCount", minimum=1
    )
    qkv = model["QKV"]
    conv_weight = model["ConvWeight"]
    qkv_output = model["QKVOutput"]
    for pointer_name, pointer in (
        ("QKV", qkv),
        ("ConvWeight", conv_weight),
        ("QKVOutput", qkv_output),
    ):
        if pointer.get("DistributedStorageKind") not in (
            "CanonicalGlobal",
            "CompactLocal",
            "CompactPerOwner",
        ):
            raise ValueError(
                "PyNTT GatedDeltaNet convolution does not support "
                f"{pointer_name} storage kind "
                f"{pointer.get('DistributedStorageKind')!r}."
            )
    if any(
        qkv.get(field) != qkv_output.get(field)
        for field in ("GlobalShape", "GlobalOffsets", "ShardAxes", "Hierarchy")
    ):
        raise ValueError(
            "PyNTT GatedDeltaNet convolution requires QKV input and output to "
            "share one distributed packed-channel mapping."
        )
    qkv_global_shape = qkv["GlobalShape"]
    weight_global_shape = conv_weight["GlobalShape"]
    output_global_shape = qkv_output["GlobalShape"]
    weight_is_canonical_global = (
        conv_weight.get("DistributedStorageKind") == "CanonicalGlobal"
    )
    if not weight_is_canonical_global:
        qkv_stages = _shard_axis_stages(qkv_output["ShardAxes"][1])
        weight_stages = _shard_axis_stages(conv_weight["ShardAxes"][0])
        mappings_match = len(qkv_stages) == len(weight_stages) <= 1
        if mappings_match and qkv_stages:
            qkv_stage = qkv_stages[0]
            weight_stage = weight_stages[0]
            mappings_match = (
                qkv_stage.get("HierarchyAxes")
                == weight_stage.get("HierarchyAxes")
                and qkv_stage.get("Distribution")
                == weight_stage.get("Distribution")
            )
            if mappings_match:
                shard_count = _stage_shard_count(
                    qkv_stage, qkv_output["Hierarchy"]
                )
                distribution = qkv_stage.get("Distribution")
                if distribution == "Contiguous":
                    qkv_granularity = qkv_stage.get("Granularity")
                    weight_granularity = weight_stage.get("Granularity")
                    qkv_capacity = (
                        _fixed(qkv_granularity)
                        if qkv_granularity is not None
                        else (_fixed(qkv_global_shape[1]) + shard_count - 1)
                        // shard_count
                    )
                    weight_capacity = (
                        _fixed(weight_granularity)
                        if weight_granularity is not None
                        else (_fixed(weight_global_shape[0]) + shard_count - 1)
                        // shard_count
                    )
                    mappings_match = (
                        qkv_capacity is not None
                        and weight_capacity is not None
                        and qkv_capacity == weight_capacity
                    )
                elif distribution == "BlockCyclic":
                    qkv_block = qkv_stage.get("BlockSize")
                    weight_block = weight_stage.get("BlockSize")
                    mappings_match = (
                        isinstance(qkv_block, int)
                        and isinstance(weight_block, int)
                        and qkv_block == weight_block
                    )
                else:
                    mappings_match = False
        qkv_offset = _fixed(qkv_output["GlobalOffsets"][1])
        weight_offset = _fixed(conv_weight["GlobalOffsets"][0])
        mappings_match = (
            mappings_match
            and qkv_offset is not None
            and weight_offset is not None
            and qkv_offset == weight_offset
            and qkv_output["Hierarchy"] == conv_weight["Hierarchy"]
            and local_weight_n == physical_local_n
        )
        if not mappings_match:
            raise ValueError(
                "PyNTT GatedDeltaNet convolution requires a compact-local "
                "weight channel mapping equivalent to the packed QKV mapping; "
                "use canonical-global weight storage when their physical shard "
                "capacities differ."
            )
    if (
        ...
        or block_n > 256
        or block_n > physical_local_n
        or physical_local_n % lane_count != 0
        or global_n % lane_count != 0
        or len(qkv_global_shape) != 2
        or _fixed(qkv_global_shape[1]) != global_n // lane_count
        or len(weight_global_shape) != 2
        or _fixed(weight_global_shape[0]) != global_n // lane_count
        or len(output_global_shape) != 2
        or _fixed(output_global_shape[1]) != global_n // lane_count
    ):
        raise ValueError(
            "PyNTT GatedDeltaNet convolution requires block_m=block_k=num_stages=1, "
            "a power-of-two block_n no larger than the local scalar capacity, "
            "packed operands over one common global channel domain, "
            f"and sufficient local capacity; got parameters={parameters}, "
            f"global_n={global_n}, "
            f"physical_local_n={physical_local_n}, lane_count={lane_count}."
        )
    context = {
        "microkernel": microkernel,
        "block_n": block_n,
        "local_n": physical_local_n,
        "global_n": global_n,
        "weight_is_canonical_global": weight_is_canonical_global,
    }
    if model.get("QKVPartial") is not None:
        context["qkv_partial"] = _gated_delta_net_partial_input_context(
            model["QKVPartial"],
            model["QKVOutput"],
            "channel",
            lane_count,
            1,
            "GatedDeltaNetConvolution QKV",
            "qkv_partial",
        )
    else:
        context["qkv_partial"] = None
    return context
...
def _gated_delta_net_recurrent_core_template_context(
    ...
    if local_value_capacity % state_value_tile != 0:
        raise ValueError(
            "PyNTT GatedDeltaNet recurrent core requires local value capacity "
            f"{local_value_capacity} to be aligned to state tile {state_value_tile}."
        )
    value_tiles_per_head = value_dim // state_value_tile
    value_tasks_per_key_head = value_heads_per_key_head * value_tiles_per_head
    total_value_tasks = num_value_heads * value_tiles_per_head
    tasks_per_cta = local_value_capacity // state_value_tile
    if block_count * tasks_per_cta < total_value_tasks:
        raise ValueError(
            "PyNTT GatedDeltaNet recurrent core local task slices do not cover "
            f"the value domain: blocks={block_count}, tasks_per_cta={tasks_per_cta}, "
            f"total_tasks={total_value_tasks}."
        )
    state_waves = (tasks_per_cta + 7) // 8
    projection_head_counts = []
    for shard_index in range(block_count):
        task_begin = shard_index * tasks_per_cta
        task_end = min(task_begin + tasks_per_cta, total_value_tasks)
    ...
    if (
        ...
        or not _is_positive_power_of_two(projection_head_capacity)
        ...
        or projection_capacity != projection_head_capacity
    ...
    consumer_warps = _require_int(
        model.get("NumWarps"), "NumWarps", minimum=1
    )
    state_task_tile = 8
    if (
        not _is_positive_power_of_two(consumer_warps)
        or consumer_warps > state_task_tile
        or state_task_tile % consumer_warps != 0
    ):
        raise ValueError(
            "PyNTT GatedDeltaNet recurrent core requires a power-of-two "
            f"consumer warp count dividing {state_task_tile}, got "
            f"{consumer_warps}."
        )
    projection_head_limit = min(
        maximum_projection_heads_per_cta, 4, consumer_warps
    )
    projection_heads_per_group = 1 << (
        projection_head_limit.bit_length() - 1
    )
    projection_k_warps = consumer_warps // projection_heads_per_group
    projection_k_threads = 32 * projection_k_warps
    if block_k % projection_k_threads != 0:
        raise ValueError(
            "PyNTT GatedDeltaNet recurrent projection requires block_k "
            "divisible by its K-distributed consumer threads; got "
            f"block_k={block_k}, consumer_warps={consumer_warps}, "
            f"projection_heads={projection_heads_per_group}."
        )
    context = {
        "microkernel": microkernel,
        "block_n": block_n,
        ...
        "value_tasks_per_key_head": value_tasks_per_key_head,
        "total_value_tasks": total_value_tasks,
        "aligned_key_head_task_slices": (
            tasks_per_cta <= value_tasks_per_key_head
            and value_tasks_per_key_head % tasks_per_cta == 0
            and tasks_per_cta * block_count == total_value_tasks
        ),
        "has_state_tail": state_waves * 8 != tasks_per_cta
        or tasks_per_cta * block_count != total_value_tasks,
        ...
        "projection_k_warps": projection_k_warps,
        "projection_groups": (
    ...
    if model.get("ZPartial") is not None:
        context["z_partial"] = _gated_delta_net_partial_input_context(
            model["ZPartial"],
            model["GatedOutput"],
            "local_value_index",
            1,
            2,
            "GatedDeltaNetRecurrentCore Z",
            "z_partial",
        )
    else:
        context["z_partial"] = None
    return context
...
def _packed_gemv_pipeline_template_context(
    ...
    *,
    ...
    expected_contract = ("bfloat16", expected_rhs_dtype, "bfloat16")
    unscaled_fp8_contract = (
        "float8e4m3fn",
        "float8e4m3fn",
        "float32",
    )
    if dtype_contract != expected_contract and not (
        allow_unscaled_fp8 and dtype_contract == unscaled_fp8_contract
    ):
        raise ValueError(
            "PyNTT packed GEMV pipeline has an incompatible dtype contract: "
            f"expected={expected_contract}"
            + (
                f" or {unscaled_fp8_contract}"
                if allow_unscaled_fp8
                else ""
            )
            + f", actual={dtype_contract}."
        )
    ...
    rhs_lane_shape = _matmul_n_lane_shape(model, "Rhs")
    output_lane_shape = _matmul_n_lane_shape(model, "Output")
    n_vector_lanes = 16 if dtype_contract == unscaled_fp8_contract else 8
    k_vector_lanes = (
        16 if dtype_contract == unscaled_fp8_contract else expected_k_vector_lanes
    )
    ...
    if (
        model["TransposeB"]
        or rhs_lane_shape != (n_vector_lanes,)
        or output_lane_shape != (n_vector_lanes,)
        or int(model["RhsKPackLaneCount"]) != 2
        or int(model["RhsKVectorLaneCount"]) != k_vector_lanes
    ):
        raise ValueError(
            "PyNTT K-major GEMV pipeline requires "
            f"rhs=[K/{2 * k_vector_lanes},N/{n_vector_lanes}]"
            f"<{n_vector_lanes},2,{k_vector_lanes}> and "
            f"output=[M,N/{n_vector_lanes}]<{n_vector_lanes}>."
        )
    ...
    _validate_packed_gemv_pipeline_resource_contract(
        ...
        block_k=block_k,
        num_stages=num_stages,
        maximum_block_n=maximum_block_n,
    )
    ...
    tma_payload_outer_extent, tma_contiguous_extent = (
        _packed_gemv_tma_payload_shape(
            n_lane,
            k_pack,
            k_lane,
            model["RhsDType"],
...
def _block_scaled_fp8_pipeline_reduction_group(
    model: dict[str, Any],
    *,
    family: str,
    variant: str,
    ...
    """Choose one nested compute group without changing the scale ABI."""

    microkernel = _microkernel_context(model, family, variant)
    stage_k = microkernel["parameters"]["block_k"]
    reduction_group = min(stage_k, scale_group_k)
    if max(stage_k, scale_group_k) % reduction_group != 0:
        raise ValueError(
            "PyNTT block-scaled FP8 pipeline requires nested Shared-stage and "
            "scale-group K extents, got "
            f"stage_k={stage_k}, scale_group_k={scale_group_k}."
        )
    return reduction_group


def _packed_gemv_register_bounded_reduction_group(
    model: dict[str, Any],
    *,
    block_n: int,
    maximum_reduction_group: int,
    accumulator_count: int,
) -> int:
    """Choose the largest reduction group whose live vectors fit the worker."""

    if accumulator_count <= 0:
        raise ValueError(
            "PyNTT packed GEMV accumulator count must be positive, got "
            f"{accumulator_count}."
        )
    kernel_config = model.get("KernelConfig")
    if not isinstance(kernel_config, dict):
        raise ValueError(
            "PyNTT packed GEMV register selection requires KernelConfig."
        )
    consumer_registers = _require_int(
        kernel_config.get("consumer_registers"),
        "KernelConfig.consumer_registers",
        minimum=1,
    )
    uniform_registers = _require_int(
        kernel_config.get("uniform_registers"),
        "KernelConfig.uniform_registers",
        minimum=1,
    )
    register_granularity = _require_int(
        kernel_config.get("register_granularity"),
        "KernelConfig.register_granularity",
        minimum=1,
    )
    consumer_warps = _require_int(model.get("NumWarps"), "NumWarps", minimum=1)
    worker_width = _require_int(
        model.get("TargetWorkerWidth"), "TargetWorkerWidth", minimum=1
    )
    if not _is_positive_power_of_two(maximum_reduction_group):
        raise ValueError(
            "PyNTT packed GEMV maximum reduction group must be a positive "
            f"power of two, got {maximum_reduction_group}."
        )

    # At an FMA update, the activation, quantized activation, Shared address,
    # current weight, input-scale broadcast, weight-scale broadcast, and product
    # temporary can be live together with every accumulator. Keep scalar loop and
    # base-address state separate because it does not scale with the tile vector.
    register_budget = (
        min(consumer_registers, uniform_registers)
        if bool(model.get("NoInline"))
        else consumer_registers
    )
    live_vector_count = accumulator_count + PACKED_GEMV_TRANSIENT_VECTOR_COUNT
    scalar_register_reserve = (
        PACKED_GEMV_TRANSIENT_SCALAR_REGISTER_GROUPS * register_granularity
    )
    reduction_group = maximum_reduction_group
    while reduction_group >= 1:
        layout = _packed_gemv_consumer_layout(
            block_n=block_n,
            reduction_group=reduction_group,
            consumer_warps=consumer_warps,
            target_worker_width=worker_width,
        )
        vector_registers = layout["size_per_thread"][1] * live_vector_count
        if scalar_register_reserve + vector_registers <= register_budget:
            return reduction_group
        reduction_group //= 2

    raise ValueError(
        "PyNTT packed GEMV cannot fit one reduction group in the configured "
        f"consumer register partition: block_n={block_n}, "
        f"maximum_reduction_group={maximum_reduction_group}, "
        f"accumulator_count={accumulator_count}, "
        f"register_budget={register_budget}."
    )


def _block_scaled_fp8_pipeline_grouping_context(
    context: dict[str, Any],
    *,
    scale_group_k: int,
) -> dict[str, Any]:
    """Describe how one scale group is covered by Shared transfer stages."""

    stage_k = int(context["block_k"])
    reduction_group = int(context["reduction_group"])
    k = _fixed(context["k"])
    if k is None or k <= 0 or k % scale_group_k != 0:
        raise ValueError(
            "PyNTT block-scaled FP8 pipeline requires fixed K divisible by its "
            f"scale group, got K={k}, scale_group_k={scale_group_k}."
        )
    if stage_k % reduction_group != 0:
        raise ValueError(
            "PyNTT block-scaled FP8 reduction group must divide its Shared "
            f"stage, got stage_k={stage_k}, reduction_group={reduction_group}."
        )

    spans_stages = scale_group_k > stage_k
    if spans_stages and scale_group_k % stage_k != 0:
        raise ValueError(
            "PyNTT block-scaled FP8 scale group must contain an integral number "
            f"of Shared stages, got scale_group_k={scale_group_k}, stage_k={stage_k}."
        )
    if not spans_stages and stage_k % scale_group_k != 0:
        raise ValueError(
            "PyNTT block-scaled FP8 Shared stage must contain an integral number "
            f"of scale groups, got stage_k={stage_k}, scale_group_k={scale_group_k}."
        )

    return {
        "scale_group_k": scale_group_k,
        "scale_group_spans_stages": spans_stages,
        "num_scale_groups": k // scale_group_k,
        "stages_per_scale_group": scale_group_k // stage_k if spans_stages else 1,
    }
...
def _packed_block_fp8_gemv_pipeline_template_context(
    ...
    reduction_group = _block_scaled_fp8_pipeline_reduction_group(
        model,
        family="triton.matmul",
        variant="simt_block_fp8_fma_smem_pipeline",
        scale_group_k=block_k,
    )
    context = _packed_gemv_pipeline_template_context(
        model,
    ...
    grouping = _block_scaled_fp8_pipeline_grouping_context(
        context,
        scale_group_k=block_k,
    )
    context.update(
        **grouping,
        ...
        scale_group_lhs_access=_matmul_lhs_access(
            model,
            output_batch_rank=0,
            m_expr="0",
            k_expr="scale_group_offs_k",
            coordinate_shape=_coordinate_shape((block_k,)),
        ),
...
def _packed_block_fp8_mma_gemv_pipeline_template_context(
    ...
    if variant != "mma_block_fp8_smem_pipeline":
        raise ValueError(
            "PyNTT block-FP8 MMA GEMV requires an mma.sync "
            f"pipeline variant, got {variant!r}."
        )
    ...
    fragment_accumulate_mma = (
        microkernel["parameters"].get("fragment_accumulate_mma", 0) == 1
    )
    auxiliary_only = microkernel["auxiliary_consumer"] is not None
    auxiliary_consumer_warps = _require_int(
        model.get(
            "AuxiliaryConsumerWarps",
            WARP_SPECIALIZATION_AUXILIARY_CONSUMER_WARPS,
        ),
        "auxiliary_consumer_warps",
        minimum=1,
    )
    large_block_n_mma = (
        microkernel["parameters"].get("large_block_n_mma", 0) == 1
    )
    linear_weight_stage = microkernel["parameters"].get(
        "linear_weight_stage", 0
    )
    if linear_weight_stage not in (0, 1):
        raise ValueError(
            "PyNTT block-FP8 MMA GEMV linear_weight_stage must be 0 or 1, "
            f"got {linear_weight_stage}."
        )
    block_n = microkernel["parameters"]["block_n"]
    block_k = microkernel["parameters"]["block_k"]
    num_stages = microkernel["parameters"]["num_stages"]
    ...
    if (
        ...
        raise ValueError(
            "PyNTT block-FP8 MMA GEMV requires block_n divisible by its "
            "native matrix-M tile and "
            "transfer-K containing complete scale groups, got "
            f"block_n={block_n}, block_k={block_k}, "
    ...
    _validate_packed_gemv_pipeline_resource_contract(
        ...
        block_k=block_k,
        num_stages=num_stages,
        maximum_block_n=(256 if large_block_n_mma else 128),
    )
    if large_block_n_mma and block_n != 256:
        raise ValueError(
            "PyNTT experimental large-N block-FP8 MMA GEMV requires block_n=256."
        )
    reduction_groups_per_stage = block_k // reduction_group
    merge_scale_group_transfers = (
        microkernel["parameters"].get("disable_scale_group_transfer_merge", 0)
        != 1
    )
    transfer_chunks_per_group = 1
    reduction_groups_per_transfer = (
        transfer_block_k // reduction_group
        if merge_scale_group_transfers
        else 1
    )
    transfers_per_stage = (
        block_k // transfer_block_k
        if merge_scale_group_transfers
        else reduction_groups_per_stage
    )
    num_warps = _require_int(model.get("NumWarps"), "NumWarps", minimum=1)
    mma_instruction_n = 8
    if (
        block_n % num_warps != 0
        or mma_instruction_n % num_warps != 0
    ):
        raise ValueError(
            "PyNTT block-FP8 MMA GEMV register-quantized activation requires "
            "one complete scale group per MMA operand, block_n divisible by the "
            "consumer warp count, and the MMA instruction N dimension divisible "
            f"by the consumer warp count; got block_n={block_n}, "
            f"mma_n={mma_instruction_n}, num_warps={num_warps}."
        )
    k = _fixed(context["k"])
    rhs_k = _fixed(context["rhs_k"])
    lhs_stage_extent = microkernel["parameters"].get("lhs_stage_extent")
    use_complete_consumer_lhs_stage = lhs_stage_extent is not None
    prequantize_lhs = microkernel["parameters"].get("prequantize_lhs", 0)
    if prequantize_lhs not in (0, 1):
        raise ValueError(
            "PyNTT block-FP8 MMA GEMV prequantize_lhs must be 0 or 1, "
            f"got {prequantize_lhs}."
        )
    prequantize_complete_lhs = prequantize_lhs == 1
    reuse_prequantized_lhs = microkernel["parameters"].get(
        "reuse_prequantized_lhs", 0
    )
    if reuse_prequantized_lhs not in (0, 1):
        raise ValueError(
            "PyNTT block-FP8 MMA GEMV reuse_prequantized_lhs must be 0 or 1, "
            f"got {reuse_prequantized_lhs}."
        )
    reuse_prequantized_lhs = reuse_prequantized_lhs == 1
    if sum(
        (
            prequantize_complete_lhs,
            reuse_prequantized_lhs,
            use_complete_consumer_lhs_stage,
        )
    ) > 1:
        raise ValueError(
            "PyNTT block-FP8 MMA GEMV complete-LHS materialization and reuse "
            "modes are mutually exclusive."
        )
    materialize_complete_lhs_quantization = (
        prequantize_complete_lhs or use_complete_consumer_lhs_stage
    )
    reuse_complete_lhs_quantization = (
        materialize_complete_lhs_quantization or reuse_prequantized_lhs
    )
    if auxiliary_only and (
        block_n != 64
        or block_n % auxiliary_consumer_warps != 0
        or mma_instruction_n % auxiliary_consumer_warps != 0
        or not fragment_accumulate_mma
        or not reuse_complete_lhs_quantization
    ):
        raise ValueError(
            "PyNTT block-FP8 MMA GEMV auxiliary-only execution requires "
            "block_n=64, primary and auxiliary warp counts dividing block_n "
            "and mma_n=8, fragment accumulation, and complete reusable LHS "
            "quantization."
        )
    if (
        k is None
        or k <= 0
        or rhs_k != k
        or k % reduction_group != 0
        or (k % block_k != 0 and not reuse_complete_lhs_quantization)
    ):
        raise ValueError(
            "PyNTT block-FP8 MMA GEMV requires equal fixed lhs/rhs K divisible "
            "by the scale group; a partial block_k tail additionally requires "
            "complete consumer LHS staging; got "
            f"lhs={k}, rhs={rhs_k}, block_k={block_k}, "
            f"scale_group_k={reduction_group}, "
            f"complete_lhs_quantization={reuse_complete_lhs_quantization}."
        )
    lhs_copy_segments: tuple[dict[str, Any], ...] = ()
    if use_complete_consumer_lhs_stage:
        if (
            ...
            raise ValueError(
                "PyNTT block-FP8 MMA GEMV complete consumer LHS staging "
                "requires contiguous K and a power-of-two Shared extent "
                f"covering K; got K={k}, lhs_stage_extent={lhs_stage_extent}, "
                f"lhs_stride={_fixed(model['LhsStrides'][-1])}."
            )
        raw_segments = _power_of_two_segments(k)
        if any(extent < reduction_group for _, extent in raw_segments):
            raise ValueError(
                "PyNTT block-FP8 MMA GEMV complete LHS copy segments must "
                f"cover complete scale groups, got {raw_segments}."
            )
        lhs_copy_segments = tuple(
            {
                "offset": offset,
    ...
    logical_activation_group_count = k // reduction_group
    num_k_tiles = (k + block_k - 1) // block_k
    materialized_activation_group_count = (
        num_k_tiles * reduction_groups_per_stage
    )
    activation_group_count = (
        1 << (materialized_activation_group_count - 1).bit_length()
        if reuse_complete_lhs_quantization
        else reduction_groups_per_stage
    )
    ...
    expected_lhs_quantized_shape = (activation_group_count, reduction_group)
    if (
    ...
    expected_lhs_scale_shape = (activation_group_count, 1)
    if (
    ...
    if use_complete_consumer_lhs_stage:
        expected_lhs_stage_shape = (1, lhs_stage_extent)
        if (
            microkernel["shared_workspace_shapes"]["lhs_stage"]
            != expected_lhs_stage_shape
        ):
            raise ValueError(
                "PyNTT block-FP8 MMA GEMV complete LHS workspace must be "
                f"{expected_lhs_stage_shape}, got "
                f"{microkernel['shared_workspace_shapes']['lhs_stage']}."
            )
    max_n = _max_value(context["n"])
    if max_n is None:
        raise ValueError(
    ...
    stage_transfers = tuple(
        _tma_local_axis_transfer(
            ...
            local_offset=0,
            tile_index=(
                f"k_tile * {transfers_per_stage} + {transfer}"
                if merge_scale_group_transfers
                else f"k_tile * {reduction_groups_per_stage} + {transfer}"
            ),
            tile_stride=(
                transfer_outer
                if merge_scale_group_transfers
                else reduction_group // k_atom
            ),
            tile_extent=transfer_outer,
            context=(
                "N-major K-packed GEMV RHS "
                f"K transfer {transfer}"
            ),
        )
        for transfer in range(transfers_per_stage)
    )
    if any(
        transfer["is_block_cyclic"]
        or tuple(transfer["block_shape"]) != (transfer_outer,)
        or len(transfer["coordinates"]) != 1
        for transfer in stage_transfers
    ):
        raise ValueError(
            "PyNTT block-FP8 MMA GEMV requires a contiguous packed K axis."
        )
    n_descriptor_plan = _n_major_k_packed_gemv_descriptor_transfer_plan(
        model["Rhs"], block_n
    )
    rhs_descriptor_chunks_per_tile = int(
        n_descriptor_plan["descriptors_per_logical_tile"]
    )
    tma_block_shape = (
        (reduction_groups_per_transfer,)
        + tuple(n_descriptor_plan["block_shape"])
        + (reduction_group,)
        if reduction_groups_per_transfer > 1
        else tuple(n_descriptor_plan["block_shape"]) + (transfer_block_k,)
    )
    ...
    transfer_descriptor_offsets = tuple(
        (
            (
                f"(({transfer['coordinates'][0]}) * {k_atom} + "
                f"({descriptor_origin_elements})) // {reduction_group}",
                *tuple("0" for _ in n_descriptor_plan["block_shape"]),
                f"(({transfer['coordinates'][0]}) * {k_atom} + "
                f"({descriptor_origin_elements})) % {reduction_group}",
            )
            if reduction_groups_per_transfer > 1
            else tuple("0" for _ in n_descriptor_plan["block_shape"])
            + (
                f"(({transfer['coordinates'][0]}) * {k_atom}) + "
                f"({descriptor_origin_elements})",
            )
    ...
    global_k_start = _pointer_local_to_global_coordinate(
        model["Lhs"],
        1,
        f"k_tile * {block_k} + group * {reduction_group}",
    )
    context.update(
        block_n=block_n,
        block_k=block_k,
        ...
        n_tiles_per_activation_batch=n_tiles_per_activation_batch,
        runtime_num_n_tiles=f"tl.cdiv(active_n, {block_n})",
        activation_group_count=activation_group_count,
        logical_activation_group_count=logical_activation_group_count,
        use_complete_consumer_lhs_stage=use_complete_consumer_lhs_stage,
        prequantize_complete_lhs=prequantize_complete_lhs,
        reuse_prequantized_lhs=reuse_prequantized_lhs,
        materialize_complete_lhs_quantization=(
            materialize_complete_lhs_quantization
        ),
        reuse_complete_lhs_quantization=reuse_complete_lhs_quantization,
        lhs_stage_extent=lhs_stage_extent,
        lhs_copy_segments=lhs_copy_segments,
        consumer_input_copy_layout_name=(
            f"{model['FunctionName']}__input_copy_layout"
        ...
        reduction_groups_per_transfer=reduction_groups_per_transfer,
        transfers_per_stage=transfers_per_stage,
        transfer_block_k=transfer_block_k,
        transfer_chunks_per_group=transfer_chunks_per_group,
        use_nv_mma_weight_stage=(linear_weight_stage == 0),
        mma_instruction_n=mma_instruction_n,
        execution_consumer_warps=(
            auxiliary_consumer_warps if auxiliary_only else num_warps
        ),
        activation_rows_per_warp=(
            mma_instruction_n
            // (auxiliary_consumer_warps if auxiliary_only else num_warps)
        ),
        ...
        transfer_descriptor_offsets=transfer_descriptor_offsets,
        ...
        pipeline_lhs_access=_matmul_lhs_access(
            ...
            k_expr="pipeline_offs_k",
            coordinate_shape=_coordinate_shape(
                (mma_instruction_n, reduction_group)
            ),
        ),
        pipeline_output_access=_contiguous_vector_axis_access(
            ("0", "0"),
        ...
        fragment_accumulate_mma=fragment_accumulate_mma,
        auxiliary_only=auxiliary_only,
        auxiliary_consumer_warps=auxiliary_consumer_warps,
        auxiliary_mma_layout_name=(
            f"{model['FunctionName']}__auxiliary_mma_layout"
        ),
        auxiliary_weight_dot_layout_name=(
            f"{model['FunctionName']}__auxiliary_weight_dot_layout"
        ),
        auxiliary_activation_dot_layout_name=(
            f"{model['FunctionName']}__auxiliary_activation_dot_layout"
        ),
        auxiliary_output_layout_name=(
            f"{model['FunctionName']}__auxiliary_output_layout"
        ),
        auxiliary_activation_quantization_layout_name=(
            f"{model['FunctionName']}__auxiliary_activation_quantization_layout"
        ),
    )
    return context
...
def _packed_qkv_gemv_pipeline_template_context(
    ...
    *,
    ...
    expected_k_vector_lanes: int = 8,
    expected_quantization_modes: tuple[str, ...] = ("none",),
) -> dict[str, Any]:
    ...
    quantization_mode = str(model.get("QuantizationMode", "none"))
    if quantization_mode not in expected_quantization_modes:
        raise ValueError(
            "PyNTT packed QKV quantization contract does not match its selected "
            f"variant {expected_variant!r}: mode={quantization_mode!r}, "
            f"expected={expected_quantization_modes}."
        )
    if quantization_mode == "none" and (
        bool(model.get("HasOperandScales")) or bool(model.get("HasWeightScales"))
    ):
        raise ValueError("Unquantized packed QKV cannot carry operand scales.")
    if quantization_mode == "static_tensor" and not bool(
        model.get("HasOperandScales")
    ):
        raise ValueError("Static-tensor packed QKV requires all operand scales.")
    if quantization_mode == "dynamic_tensor" and (
        bool(model.get("HasOperandScales"))
        or not bool(model.get("HasWeightScales"))
    ):
        raise ValueError(
            "Dynamic-tensor packed QKV requires only Q/K/V weight scales."
        )
    ...
    input_active_shape = _qkv_input_active_shape(model)
    if (
        len(model["InputShape"]) != 2
        or len(input_active_shape) != 2
    ...
    return {
        ...
        "projections": tuple(projections),
        "k": model["InputShape"][-1],
        "active_k": _dim(input_active_shape[-1]),
        "k_atom": k_atom,
        "packed_k_outer": packed_k_outer,
        "reduction_group": reduction_group,
        "reduction_groups_per_stage": block_k // reduction_group,
        ...
        "dynamic_scale_input_access": _qkv_input_access(
            model,
            output_batch_rank=0,
            m_expr="0",
            k_expr="scale_offs_k",
...
def _packed_fp8_qkv_gemv_pipeline_template_context(
    ...
    """Prepare the E4M3-weight fused QKV algorithm."""
    ...
    return _packed_qkv_gemv_pipeline_template_context(
        ...
        expected_k_vector_lanes=16,
        expected_quantization_modes=("static_tensor", "dynamic_tensor"),
    )


def _packed_fp8_qkv_mma_gemv_pipeline_template_context(
    model: dict[str, Any],
) -> dict[str, Any]:
    """Prepare direct N-major TMA-to-MMA dynamic-tensor FP8 QKV GEMV."""
    ...
    microkernel = _microkernel_context(
        model,
        "triton.qkv_parallel_linear",
        "mma_fp8_smem_pipeline",
        required_workspace_names=("rhs_stage",),
    )
    if (
        model["InputDType"] != "bfloat16"
        or model["WeightDType"] != "float8e4m3fn"
        or model["OutputDType"] != "bfloat16"
    ):
        raise ValueError(
            "PyNTT FP8 QKV MMA GEMV requires BF16/E4M3/BF16 operands."
        )
    if (
        model.get("QuantizationMode") != "dynamic_tensor"
        or bool(model.get("HasOperandScales"))
        or not bool(model.get("HasWeightScales"))
    ):
        raise ValueError(
            "PyNTT FP8 QKV MMA GEMV requires dynamic-tensor activation "
            "quantization and Q/K/V weight scales only."
        )
    if (
        model.get("RhsLayout") != "n_major_k_packed"
        or int(model["NPackedLaneCount"]) != 1
        or int(model["NVectorLaneCount"]) != 8
        or int(model["KPackLaneCount"]) != 2
        or int(model["KVectorLaneCount"]) != 16
    ):
        raise ValueError(
            "PyNTT FP8 QKV MMA GEMV requires weight=[N,K/32]<2,16> "
            "and output=[M,N/8]<8>."
        )
    input_active_shape = _qkv_input_active_shape(model)
    if (
        ...
        or len(model["WeightShape"]) != 2
        or any(
            len(model[f"{prefix}OutputShape"]) != 2
            for prefix in ("Q", "K", "V")
        )
    ):
        raise ValueError("PyNTT FP8 QKV MMA GEMV requires rank-2 operands.")
    ...
    if (
        block_n % 16 != 0
        or block_k % transfer_block_k != 0
        or transfer_block_k % 32 != 0
    ):
        raise ValueError(
            "PyNTT FP8 QKV MMA GEMV requires block_n divisible by 16 and "
            "a block_k/transfer_k hierarchy aligned to the 32-value packed K atom, "
            f"got block_n={block_n}, block_k={block_k}, "
            f"transfer_k={transfer_block_k}."
        )
    _validate_packed_gemv_pipeline_resource_contract(
        algorithm="FP8 QKV MMA GEMV pipeline",
        block_n=block_n,
        block_k=block_k,
        num_stages=num_stages,
        maximum_block_n=128,
    )
    transfer_chunks_per_stage = block_k // transfer_block_k
    expected_workspace_shape = (
        num_stages,
        transfer_chunks_per_stage,
        block_n,
        transfer_block_k,
    )
    if microkernel["shared_workspace_shapes"]["rhs_stage"] != expected_workspace_shape:
        raise ValueError(
            "PyNTT FP8 QKV MMA GEMV Shared workspace must be "
            f"{expected_workspace_shape}, got "
            f"{microkernel['shared_workspace_shapes']['rhs_stage']}."
        )
    ...
    if k is None or k <= 0:
        raise ValueError(
            "PyNTT FP8 QKV MMA GEMV requires a fixed positive storage K, "
            f"got K={k}."
        )
    n_lane = int(model["NVectorLaneCount"])
    k_atom = int(model["KPackLaneCount"]) * int(model["KVectorLaneCount"])
    ...
    if any(value % n_lane != 0 for value in projection_ns.values()):
        raise ValueError(
            "PyNTT FP8 QKV MMA projection capacities must be N-vector aligned."
        )
    projection_starts: dict[str, int] = {}
    total_n = 0
    for prefix in ("Q", "K", "V"):
    ...
    expected_weight_shape = (total_n, (k + k_atom - 1) // k_atom)
    if tuple(_fixed(value) for value in model["WeightShape"]) != expected_weight_shape:
        raise ValueError(
            "PyNTT FP8 QKV MMA fused weight shape does not match its capacities: "
            f"expected={expected_weight_shape}, actual={model['WeightShape']}."
        )
    if (
        _fixed(model["WeightStrides"][-1]) != 1
        ...
        raise ValueError(
            "PyNTT FP8 QKV MMA requires positive-stride "
            "[N,K/32]<2,16> storage with contiguous packed K."
        )
    ...
    global_offsets = model.get("WeightGlobalOffsets")
    if not isinstance(descriptor_name, str) or not descriptor_name:
        raise ValueError("PyNTT FP8 QKV MMA requires a host weight descriptor.")
    if not isinstance(descriptor_origin_elements, str) or not descriptor_origin_elements:
        raise ValueError("PyNTT FP8 QKV MMA requires a descriptor origin.")
    if not isinstance(global_offsets, list) or len(global_offsets) != 2:
        raise ValueError("PyNTT FP8 QKV MMA requires two weight global offsets.")

    weight_pointer = model["Weight"]
    n_descriptor_plan = _n_major_k_packed_gemv_descriptor_transfer_plan(
        weight_pointer, block_n
    )
    descriptor_chunks_per_tile = int(
        n_descriptor_plan["descriptors_per_logical_tile"]
    )
    transfer_outer = transfer_block_k // k_atom
    transfers = tuple(
        _tma_local_axis_transfer(
            ...
            local_offset=chunk * transfer_outer,
            tile_index="k_tile",
            tile_stride=block_k // k_atom,
            tile_extent=transfer_outer,
            context=f"FP8 QKV MMA K transfer chunk {chunk}",
        )
        for chunk in range(transfer_chunks_per_stage)
    )
    if any(
        transfer["is_block_cyclic"]
        or tuple(transfer["block_shape"]) != (transfer_outer,)
        ...
        raise ValueError(
            "PyNTT FP8 QKV MMA requires a contiguous packed local K axis."
        )
    tma_block_shape = tuple(n_descriptor_plan["block_shape"]) + (
        transfer_block_k,
    )
    descriptor_offsets = tuple(
        tuple("0" for _ in n_descriptor_plan["block_shape"])
        + (
            f"(({transfer['coordinates'][0]}) * {k_atom}) + "
            f"({descriptor_origin_elements})",
    ...
    num_k_tiles = (k + block_k - 1) // block_k
    if num_n_tiles * num_k_tiles > 2**31 - 1:
        raise ValueError("PyNTT FP8 QKV MMA pipe sequence exceeds signed int32.")
    rhs_scalar_bytes = TMA_DTYPE_ITEM_SIZES.get(model["WeightDType"])
    if rhs_scalar_bytes is None:
        raise ValueError(
            "PyNTT FP8 QKV MMA cannot size Shared aliases for "
            f"dtype {model['WeightDType']!r}."
        )
    ...
    return {
        ...
        "num_k_tiles": num_k_tiles,
        "num_n_tiles": num_n_tiles,
        "dynamic_scale_num_tiles": (
            k + transfer_block_k - 1
        ) // transfer_block_k,
        "transfer_block_k": transfer_block_k,
        "transfer_chunks_per_stage": transfer_chunks_per_stage,
        "transfer_chunk_stage_bytes": (
            num_stages * block_n * transfer_block_k * rhs_scalar_bytes
        ),
        "descriptor_name": descriptor_name,
        "descriptor_entries_per_owner": (
            num_n_tiles * descriptor_chunks_per_tile
        ),
        "descriptor_chunks_per_tile": descriptor_chunks_per_tile,
        "descriptor_offsets": descriptor_offsets,
        "tma_block_shape": tma_block_shape,
        "tma_n_elements": prod(tma_block_shape[:-1]),
        "rhs_scalar_bytes": rhs_scalar_bytes,
        ...
        "pipeline_input_access": _qkv_input_access(
            ...
            m_expr="0",
            k_expr="pipeline_offs_k",
            coordinate_shape=_coordinate_shape((transfer_block_k,)),
        ),
        "dynamic_scale_input_access": _qkv_input_access(
            model,
            output_batch_rank=0,
...
def _packed_matmul_glu_gemv_pipeline_template_context(
    ...
    *,
    ...
    _validate_packed_gemv_pipeline_resource_contract(
        ...
        maximum_block_n=(
            128
            if expected_variant == "mma_block_fp8_smem_pipeline"
            else 64
        ),
    )
...
def _packed_block_fp8_matmul_glu_gemv_pipeline_template_context(
    model: dict[str, Any],
    *,
    expected_variant: str = "simt_block_fp8_fma_smem_pipeline",
) -> dict[str, Any]:
    ...
    maximum_reduction_group = _block_scaled_fp8_pipeline_reduction_group(
        model,
        family="triton.matmul_glu",
        variant="simt_block_fp8_fma_smem_pipeline",
        scale_group_k=block_k,
    ...
    reduction_group = _packed_gemv_register_bounded_reduction_group(
        model,
        block_n=microkernel["parameters"]["block_n"],
        maximum_reduction_group=maximum_reduction_group,
        accumulator_count=2,
    )
    ...
    context.update(
        **grouping,
        ...
        global_output_n=global_output_n,
        global_k_start=global_k_start,
        scale_group_input_access=_matmul_glu_input_access(
            model,
            output_batch_rank=0,
            m_expr="0",
            k_expr="scale_group_pipeline_offs_k",
            coordinate_shape=_coordinate_shape((reduction_group,)),
        ),
        projections=tuple(projections),
    )
...
def _packed_block_fp8_mma_matmul_glu_gemv_pipeline_template_context(
    model: dict[str, Any],
) -> dict[str, Any]:
    """Prepare K-major block-scaled paired GEMV for tensor-core execution."""

    context = _packed_block_fp8_matmul_glu_gemv_pipeline_template_context(
        model,
        expected_variant="mma_block_fp8_smem_pipeline",
    )
    microkernel = context["microkernel"]
    block_n = context["block_n"]
    block_k = context["block_k"]
    num_stages = context["num_stages"]
    reduction_group = context["reduction_group"]
    transfer_block_k = microkernel["parameters"].get("transfer_block_k")
    if transfer_block_k != reduction_group:
        raise ValueError(
            "PyNTT block-FP8 MatMulGlu MMA requires one complete scale group "
            f"per MMA transfer, got transfer_k={transfer_block_k}, "
            f"scale_group_k={reduction_group}."
        )
    if block_n % 16 != 0 or block_k % reduction_group != 0:
        raise ValueError(
            "PyNTT block-FP8 MatMulGlu MMA requires block_n divisible by 16 "
            "and block_k divisible by the scale group, got "
            f"block_n={block_n}, block_k={block_k}, group={reduction_group}."
        )

    k_atom = context["k_atom"]
    n_lane = int(model["NVectorLaneCount"])
    activation_group_count = (
        _fixed(context["k"]) // reduction_group
        if context["use_complete_consumer_lhs_stage"]
        else block_k // reduction_group
    )
    expected_workspaces = {
        "rhs_stage": (
            num_stages,
            (block_k // k_atom) * (block_n // n_lane),
            n_lane * k_atom,
        ),
        "lhs_quantized": (activation_group_count, reduction_group),
        "lhs_scale": (activation_group_count, 1),
    }
    if context["use_complete_consumer_lhs_stage"]:
        expected_workspaces["lhs_stage"] = (
            1,
            context["lhs_stage_extent"],
        )
    if microkernel["shared_workspace_shapes"] != expected_workspaces:
        raise ValueError(
            "PyNTT block-FP8 MatMulGlu MMA workspace contract mismatch: "
            f"expected={expected_workspaces}, "
            f"actual={microkernel['shared_workspace_shapes']}."
        )
    ...
    context.update(
        mma_layout_name=f"{function_name}__mma_layout",
        weight_dot_layout_name=f"{function_name}__weight_dot_layout",
        activation_dot_layout_name=f"{function_name}__activation_dot_layout",
        mma_output_layout_name=f"{function_name}__mma_output_layout",
        transfer_block_k=transfer_block_k,
        load_lhs_directly=False,
    )
    return context


def _packed_block_fp8_mma_n_major_matmul_glu_gemv_pipeline_template_context(
    model: dict[str, Any],
) -> dict[str, Any]:
    """Prepare N-major K-packed paired GEMV for mma.sync execution."""
    ...
    split_projection_channels = (
        isinstance(raw_parameters, dict)
        and raw_parameters.get("split_projection_channels") == 1
    )
    dual_consumer = (
        isinstance(raw_parameters, dict)
        and raw_parameters.get("dual_consumer") == 1
    )
    auxiliary_only = isinstance(
        raw_microkernel.get("AuxiliaryConsumer")
        if isinstance(raw_microkernel, dict)
        else None,
        dict,
    )
    parallel_projection_consumers = (
        isinstance(raw_parameters, dict)
        and raw_parameters.get("parallel_projection_consumers") == 1
    )
    paired_projection_mma = (
        isinstance(raw_parameters, dict)
        and raw_parameters.get("paired_projection_mma") == 1
    )
    direct_mma_column_extract = (
        isinstance(raw_parameters, dict)
        and raw_parameters.get("direct_mma_column_extract") == 1
    )
    fragment_accumulate_mma = (
        isinstance(raw_parameters, dict)
        and raw_parameters.get("fragment_accumulate_mma") == 1
    )
    tail_block_n = (
        raw_parameters.get("tail_block_n")
        if isinstance(raw_parameters, dict)
        else None
    )
    if tail_block_n is not None:
        tail_block_n = _require_int(
            tail_block_n, "tail_block_n", minimum=16
        )
        if split_projection_channels:
            raise ValueError(
                "PyNTT N-major block-FP8 MatMulGlu MMA tail tiling does not "
                "support split projection channels."
            )
    auxiliary_modes = sum(
        int(mode)
        for mode in (
    ...
    if auxiliary_modes > 1:
        raise ValueError(
            "PyNTT N-major block-FP8 MatMulGlu auxiliary execution modes "
            "are mutually exclusive."
        )
    if (dual_consumer or auxiliary_only) and split_projection_channels:
        raise ValueError(
            "PyNTT N-major block-FP8 MatMulGlu auxiliary-consumer execution "
            "does not support split projection channels."
        )
    if parallel_projection_consumers and tail_block_n is not None:
        raise ValueError(
            "PyNTT N-major block-FP8 MatMulGlu auxiliary-consumer execution "
            "supports tail tiling only in auxiliary-only execution."
        )
    if parallel_projection_consumers and not split_projection_channels:
        raise ValueError(
            "PyNTT parallel-projection MatMulGlu requires independent "
            "Gate and Up transfer channels."
        )
    required_workspace_names = (
        ("gate_rhs_stage", "up_rhs_stage", "lhs_quantized", "lhs_scale")
        if split_projection_channels
        else ("rhs_stage", "lhs_quantized", "lhs_scale")
    )
    if isinstance(raw_parameters, dict) and "lhs_stage_extent" in raw_parameters:
        required_workspace_names += ("lhs_stage",)
    microkernel = _microkernel_context(
        model,
        "triton.matmul_glu",
        "mma_n_major_block_fp8_smem_pipeline",
    ...
    if (
        ...
        or model["WeightDType"] != "float8e4m3fn"
        or model["OutputDType"] != "bfloat16"
        or model.get("QuantizationMode") != "dynamic_block"
        or bool(model.get("HasOperandScales"))
        or not bool(model.get("HasWeightScales"))
    ):
        raise ValueError(
            "PyNTT N-major block-FP8 MatMulGlu MMA requires dynamic "
            "BF16/E4M3/BF16 quantization with two weight scales."
        )
    if (
        model.get("RhsLayout") != "n_major_k_packed"
        or int(model["NPackedLaneCount"]) != 1
        ...
        raise ValueError(
            "PyNTT N-major block-FP8 MatMulGlu MMA requires "
            "weight=[N,K/32]<2,16> and output=[M,N/8]<8>."
        )
    if any(
        len(model[name]) != 2
        for name in (
        ...
        raise ValueError(
            "PyNTT N-major block-FP8 MatMulGlu MMA currently requires rank-2 operands."
        )

    parameters = microkernel["parameters"]
    block_n = parameters["block_n"]
    block_k = parameters["block_k"]
    physical_num_stages = parameters["num_stages"]
    transfer_block_k = parameters.get("transfer_block_k")
    reduction_group = _require_int(
        model.get("WeightBlockK"), "WeightBlockK", minimum=1
    )
    weight_block_n = _require_int(
    ...
    if weight_block_n != 128 or reduction_group != 128:
        raise ValueError(
            "PyNTT N-major block-FP8 MatMulGlu MMA implements the official "
            f"128x128 scale ABI, got {weight_block_n}x{reduction_group}."
        )
    if (
        physical_num_stages % 2 != 0
        or physical_num_stages < 4
        or block_n % 16 != 0
        or block_k % reduction_group != 0
        or not isinstance(transfer_block_k, int)
        or transfer_block_k <= 0
        or transfer_block_k % reduction_group != 0
        or block_k % transfer_block_k != 0
    ):
        raise ValueError(
            "PyNTT N-major block-FP8 MatMulGlu MMA has an invalid "
            "projection/stage/tile hierarchy."
        )
    logical_num_stages = physical_num_stages // 2
    if dual_consumer and (
        logical_num_stages < 2 or logical_num_stages % 2 != 0
    ):
        raise ValueError(
            "PyNTT N-major block-FP8 MatMulGlu dual-consumer execution "
            "requires an even logical stage count of at least two, got "
            f"{logical_num_stages}."
        )
    reduction_groups_per_stage = block_k // reduction_group
    reduction_groups_per_transfer = transfer_block_k // reduction_group
    transfers_per_stage = block_k // transfer_block_k
    k = _fixed(model["InputShape"][-1])
    if k is None or k <= 0 or k % block_k != 0:
        raise ValueError(
            "PyNTT N-major block-FP8 MatMulGlu MMA requires fixed K divisible "
            f"by block_k={block_k}, got {k}."
        )
    num_k_tiles = k // block_k
    lhs_stage_extent = parameters.get("lhs_stage_extent")
    use_complete_consumer_lhs_stage = lhs_stage_extent is not None
    prequantize_lhs = parameters.get("prequantize_lhs", 0)
    if prequantize_lhs not in (0, 1):
        raise ValueError(
            "PyNTT N-major block-FP8 MatMulGlu MMA prequantize_lhs must be "
            f"0 or 1, got {prequantize_lhs}."
        )
    prequantize_complete_lhs = prequantize_lhs == 1
    if prequantize_complete_lhs and use_complete_consumer_lhs_stage:
        raise ValueError(
            "PyNTT N-major block-FP8 MatMulGlu MMA direct prequantization "
            "and complete BF16 LHS staging are mutually exclusive."
        )
    materialize_complete_lhs_quantization = (
        prequantize_complete_lhs or use_complete_consumer_lhs_stage
    )
    if paired_projection_mma and (
        block_n != 64
        or split_projection_channels
        or dual_consumer
        or auxiliary_only
        or parallel_projection_consumers
        or tail_block_n is not None
    ):
        raise ValueError(
            "PyNTT paired-projection MatMulGlu MMA requires block_n=64, "
            "one projection pipe, no tail, and no auxiliary consumer."
        )
    if direct_mma_column_extract and paired_projection_mma:
        raise ValueError(
            "PyNTT direct MMA column extraction does not support paired "
            "projection MMA."
        )
    if fragment_accumulate_mma and (
        (split_projection_channels and not parallel_projection_consumers)
        or paired_projection_mma
        or direct_mma_column_extract
    ):
        raise ValueError(
            "PyNTT MMA-fragment MatMulGlu accumulation requires one projection "
            "channel, no tail, and the mma.sync path."
        )
    if auxiliary_modes and not materialize_complete_lhs_quantization:
        raise ValueError(
            "PyNTT N-major block-FP8 MatMulGlu auxiliary-consumer execution "
            "requires one complete shared LHS quantization."
        )
    if parallel_projection_consumers and (
        block_n != 64
        or not fragment_accumulate_mma
        or not use_complete_consumer_lhs_stage
        or bool(model["EmitPartialResults"])
    ):
        raise ValueError(
            "PyNTT parallel-projection MatMulGlu requires block_n=64, "
            "MMA-fragment accumulation, a reusable complete LHS stage, "
            "and a fused GLU output."
        )
    logical_activation_group_count = k // reduction_group
    activation_group_count = (
        1 << (logical_activation_group_count - 1).bit_length()
        if materialize_complete_lhs_quantization
        else reduction_groups_per_stage
    )
    projection_workspace_shape = (
        logical_num_stages,
        transfers_per_stage,
        1,
        block_n,
    ...
    if split_projection_channels:
        expected_workspaces = {
            "gate_rhs_stage": projection_workspace_shape,
            "up_rhs_stage": projection_workspace_shape,
            **expected_workspaces,
        }
    else:
        expected_workspaces = {
    ...
    if use_complete_consumer_lhs_stage:
        expected_workspaces["lhs_stage"] = (1, lhs_stage_extent)
    if microkernel["shared_workspace_shapes"] != expected_workspaces:
        raise ValueError(
            "PyNTT N-major block-FP8 MatMulGlu MMA workspace contract mismatch: "
            f"expected={expected_workspaces}, "
            f"actual={microkernel['shared_workspace_shapes']}."
        )
    ...
    fake_parameters = {
        **parameters,
        "num_stages": logical_num_stages,
        "disable_scale_group_transfer_merge": 1,
    }
    fake_parameters.pop("lhs_stage_extent", None)
    fake_parameters.pop("prequantize_lhs", None)
    # Projection contexts validate the shared matmul geometry only. Role
    # specialization belongs to the enclosing MatMulGlu helper.
    fake_workspace_shapes = {
        "rhs_stage": (
            logical_num_stages,
        ...
        "lhs_quantized": (reduction_groups_per_stage, reduction_group),
        "lhs_scale": (reduction_groups_per_stage, 1),
    }
    fake_microkernel = {
        "Family": "triton.matmul",
        ...
        "SharedWorkspaceOffsets": {
            name: "0" for name in fake_workspace_shapes
        },
        "SharedWorkspaceShapes": {
            name: list(shape) for name, shape in fake_workspace_shapes.items()
        },
    ...
    for prefix in ("Gate", "Up"):
        projection_model = {
            **model,
            "MicroKernel": fake_microkernel,
            "Lhs": model["Input"],
            "Rhs": model[f"{prefix}Weight"],
            "Output": model["Output"],
            "LhsDType": model["InputDType"],
            "RhsDType": model["WeightDType"],
            "OutputDType": model["OutputDType"],
            "LhsShape": model["InputShape"],
            "RhsShape": model[f"{prefix}WeightShape"],
            "OutputShape": model["OutputShape"],
            "LhsStrides": model["InputStrides"],
            "RhsStrides": model[f"{prefix}WeightStrides"],
            "OutputStrides": model["OutputStrides"],
            "TransposeA": False,
            "TransposeB": True,
            "RhsNPackedLaneCount": 1,
            "RhsNVectorLaneCount": 1,
            "RhsKPackLaneCount": model["KPackLaneCount"],
            "RhsKVectorLaneCount": model["KVectorLaneCount"],
            "OutputNPackedLaneCount": model["NPackedLaneCount"],
            "OutputNVectorLaneCount": model["NVectorLaneCount"],
            "HasRhsBlockScale": True,
            "HasOperandScales": False,
            "HasAddend": False,
            "HasNormStats": False,
            "RhsDescriptorName": model[f"{prefix}WeightDescriptorName"],
            "RhsDescriptorOriginElements": model[
                f"{prefix}WeightDescriptorOriginElements"
            ],
            "RhsGlobalOffsets": model[f"{prefix}WeightGlobalOffsets"],
            "RhsScaleShape": model[f"{prefix}WeightScaleShape"],
            "RhsScaleStrides": model[f"{prefix}WeightScaleStrides"],
        }
    ...
    common_keys = (
        ...
        "tma_block_shape",
        "rhs_descriptor_entries_per_owner",
        "rhs_descriptor_chunks_per_tile",
        "rhs_scalar_bytes",
        "transfer_stage_bytes",
        "tma_n_elements",
        "global_output_n",
    ...
    if any(
        ...
        for context in projection_contexts[1:]
        for key in common_keys
    ):
        raise ValueError(
            "PyNTT N-major block-FP8 MatMulGlu MMA requires gate/up "
            "projections to have one common distributed tile layout."
        )

    cooperative_n_tile_plan = _matmul_glu_cooperative_n_tile_plan(
        model, block_n
    )

    max_n = _max_value(base["n"])
    full_n_tiles = base["num_n_tiles"]
    if tail_block_n is not None:
        local_n_remainder = None if max_n is None else max_n % block_n
        if (
            max_n is None
            or tail_block_n >= block_n
            or tail_block_n % 16 != 0
            or local_n_remainder == 0
            or local_n_remainder > tail_block_n
        ):
            raise ValueError(
                "PyNTT N-major block-FP8 MatMulGlu MMA tail tiling requires "
                "a 16-aligned tail smaller than block_n and covering the "
                f"non-zero bounded local-N remainder; max_n={max_n}, "
                f"block_n={block_n}, tail_block_n={tail_block_n}."
            )
        full_n_tiles = max_n // block_n

    if cooperative_n_tile_plan is not None:
        tail_block_n = None
        full_n_tiles = int(cooperative_n_tile_plan["tiles_per_owner"])

    n_lane = int(model["NVectorLaneCount"])
    if cooperative_n_tile_plan is not None:
        merged_tma_block_shape = (
            (reduction_groups_per_transfer, block_n, reduction_group)
            if reduction_groups_per_transfer > 1
            else (block_n, transfer_block_k)
        )
    else:
        merged_tma_block_shape = (
            (reduction_groups_per_transfer,)
            + tuple(base["tma_block_shape"][:-1])
            + (reduction_group,)
            if reduction_groups_per_transfer > 1
            else tuple(base["tma_block_shape"])
        )
    projections = []
    for prefix, projection_context in zip(("Gate", "Up"), projection_contexts):
        projections.append(
            {
                "prefix": prefix,
                "lower": prefix.lower(),
                "descriptor_name": projection_context["rhs_descriptor_name"],
                "descriptor_offsets": projection_context[
                    "rhs_descriptor_offsets"
                ],
                "transfer_descriptor_offsets": tuple(
                    (
                        (
                            f"(({projection_context['rhs_descriptor_offsets'][transfer * reduction_groups_per_transfer][0][-1]}) // {reduction_group})",
                            "0",
                            f"(({projection_context['rhs_descriptor_offsets'][transfer * reduction_groups_per_transfer][0][-1]}) % {reduction_group})",
                        )
                        if cooperative_n_tile_plan is not None
                        and reduction_groups_per_transfer > 1
                        else (
                            "0",
                            projection_context["rhs_descriptor_offsets"][
                                transfer
                            ][0][-1],
                        )
                        if cooperative_n_tile_plan is not None
                            ...
                            f"(({projection_context['rhs_descriptor_offsets'][transfer * reduction_groups_per_transfer][0][-1]}) // {reduction_group})",
                            *projection_context["rhs_descriptor_offsets"][
                                transfer * reduction_groups_per_transfer
                            ][0][:-1],
                            f"(({projection_context['rhs_descriptor_offsets'][transfer * reduction_groups_per_transfer][0][-1]}) % {reduction_group})",
                        )
                        if reduction_groups_per_transfer > 1
                        else projection_context["rhs_descriptor_offsets"][
                            transfer
                        ][0]
                    )
                    for transfer in range(transfers_per_stage)
                ),
                "weight_scale_access": projection_context["rhs_scale_access"],
                "has_bias": bool(model[f"Has{prefix}Bias"]),
                "bias_access": (
                    _contiguous_vector_axis_access(
                        ("0",),
                        model[f"{prefix}BiasStrides"],
                        tensor_shape=model[f"{prefix}BiasShape"],
                        packed_axis=0,
                        logical_index="pipeline_output_n",
                        lane_count=n_lane,
                        coordinate_shape=_coordinate_shape((block_n,)),
    ...
    tail_projections = tuple(
        {
            **projection,
            "weight_scale_access": _tensor_access(
                ("weight_scale_n", "weight_scale_k"),
                model[f"{projection['prefix']}WeightScaleStrides"],
                coordinate_shape=_coordinate_shape((tail_block_n,)),
                global_coordinate_axes=(0, 1),
            ),
            "bias_access": (
                _contiguous_vector_axis_access(
                    ("0",),
                    model[f"{projection['prefix']}BiasStrides"],
                    tensor_shape=model[f"{projection['prefix']}BiasShape"],
                    packed_axis=0,
                    logical_index="pipeline_output_n",
                    lane_count=n_lane,
                    coordinate_shape=_coordinate_shape((tail_block_n,)),
    ...
    ) if tail_block_n is not None else ()
    ...
    if use_complete_consumer_lhs_stage:
        if (
            ...
            raise ValueError(
                "PyNTT N-major block-FP8 MatMulGlu complete LHS staging "
                "requires contiguous K and a power-of-two covering extent."
            )
        lhs_copy_segments = tuple(
            {
                "offset": offset,
            ...
            for offset, extent in _power_of_two_segments(k)
        )

    function_name = model["FunctionName"]
    output_n_stages = _shard_axis_stages(model["Output"]["ShardAxes"][1])
    runtime_active_n = _dim(base["n"])
    if any(
        stage.get("Distribution") == "BlockCyclic"
        for stage in output_n_stages
    ):
        runtime_active_n = (
            f"({_local_axis_active_extent(model['Output']['GlobalShape'][1], model['Output']['ShardAxes'][1], model['Output']['Hierarchy'])})"
            f" * {n_lane}"
        )
    return {
        **base,
        "microkernel": microkernel,
        "num_stages": physical_num_stages,
        "logical_num_stages": logical_num_stages,
        "dual_pipeline_num_stages": (
            logical_num_stages // 2 if dual_consumer else logical_num_stages
        ),
        "split_projection_channels": split_projection_channels,
        "dual_consumer": dual_consumer,
        "auxiliary_only": auxiliary_only,
        "parallel_projection_consumers": parallel_projection_consumers,
        "paired_projection_mma": paired_projection_mma,
        "direct_mma_column_extract": direct_mma_column_extract,
        "fragment_accumulate_mma": fragment_accumulate_mma,
        "auxiliary_consumer_warps": base["auxiliary_consumer_warps"],
        "tail_block_n": tail_block_n,
        "full_n_tiles": full_n_tiles,
        "cooperative_n_tiles": cooperative_n_tile_plan is not None,
        "num_n_tiles": (
            int(cooperative_n_tile_plan["tiles_per_owner"])
            if cooperative_n_tile_plan is not None
            else base["num_n_tiles"]
        ),
        "rhs_descriptor_entries_per_owner": (
            int(cooperative_n_tile_plan["tiles_per_owner"])
            if cooperative_n_tile_plan is not None
            else base["rhs_descriptor_entries_per_owner"]
        ),
        "global_output_n": (
            str(cooperative_n_tile_plan["global_output_n"])
            if cooperative_n_tile_plan is not None
            else base["global_output_n"]
        ),
        "reduction_groups_per_transfer": reduction_groups_per_transfer,
        "transfers_per_stage": transfers_per_stage,
        "tma_block_shape": merged_tma_block_shape,
        "projection_shared_bytes": (
            logical_num_stages * block_n * block_k
        ),
        "activation_group_count": activation_group_count,
        "logical_activation_group_count": logical_activation_group_count,
        "runtime_active_n": (
            str(cooperative_n_tile_plan["runtime_active_n"])
            if cooperative_n_tile_plan is not None
            else runtime_active_n
        ),
        "projections": tuple(projections),
        "tail_projections": tail_projections,
        "use_complete_consumer_lhs_stage": use_complete_consumer_lhs_stage,
        "prequantize_complete_lhs": prequantize_complete_lhs,
        "parallelize_complete_lhs_quantization": (
            prequantize_complete_lhs
            or (use_complete_consumer_lhs_stage and auxiliary_only)
        ),
        "materialize_complete_lhs_quantization": (
            materialize_complete_lhs_quantization
        ),
        ...
        "complete_pipeline_input_access": _matmul_glu_input_access(
            model,
            output_batch_rank=0,
            m_expr="0",
            k_expr="pipeline_offs_k",
            coordinate_shape=_coordinate_shape(
                (base["mma_instruction_n"], reduction_group)
            ),
        ),
        "pipeline_output_access": _contiguous_vector_axis_access(
            ("0", "0"),
        ...
        "up_output_access": (
            _contiguous_vector_axis_access(
                ("0", "0"),
                model["UpOutputStrides"],
                tensor_shape=model["UpOutputShape"],
                packed_axis=1,
                logical_index="pipeline_output_n",
                lane_count=n_lane,
                coordinate_shape=_coordinate_shape((block_n,)),
        ...
        "tail_up_output_access": (
            ...
            if tail_block_n is not None and bool(model.get("EmitPartialResults"))
            else None
        ),
        "pipeline_output_mask": "pipeline_output_n < active_n",
        "mma_layout_name": f"{function_name}__mma_layout",
        ...
        "mma_output_layout_name": f"{function_name}__mma_output_layout",
        "auxiliary_mma_layout_name": (
            f"{function_name}__auxiliary_mma_layout"
        ),
        "auxiliary_weight_dot_layout_name": (
            f"{function_name}__auxiliary_weight_dot_layout"
        ),
        "auxiliary_activation_dot_layout_name": (
            f"{function_name}__auxiliary_activation_dot_layout"
        ),
        "auxiliary_mma_output_layout_name": (
            f"{function_name}__auxiliary_mma_output_layout"
        ),
        "consumer_input_copy_layout_name": (
            f"{function_name}__input_copy_layout"
        ),
        "activation_quantization_layout_name": (
            f"{function_name}__activation_quantization_layout"
        ),
        "input_copy_size_per_thread": 4,
        "weight_block_n": weight_block_n,
        "weight_block_k": reduction_group,
...
def _reduce_template_context(model: dict[str, Any]) -> dict[str, Any]:
    ...
    axes = tuple(int(axis) for axis in model["Axes"])
    output_numel: int | None = 1
    for extent in output_shape:
        fixed_extent = _fixed(extent)
        if fixed_extent is None:
            output_numel = None
            break
        output_numel *= fixed_extent
    reduction_extent = _fixed(model["InputShape"][axes[0]]) if len(axes) == 1 else None

    # A small result cannot expose enough output parallelism to use the normal
    # output-lane schedule. Assign the lanes to the reduction axis instead.
    reduction_parallel = (
        output_numel is not None
        and output_numel <= 8
        and reduction_extent is not None
        and reduction_extent >= 32
        and len(axes) == 1
    )
    if reduction_parallel:
        reduction_axis = axes[0]
        output_index = 0
        input_coordinates: list[str] = []
        for input_index in range(len(model["InputShape"])):
            if input_index == reduction_axis:
                input_coordinates.append("reduce_lane")
                if model["KeepDims"]:
                    output_index += 1
                continue
            input_coordinates.append(f"out_idx{output_index}")
            output_index += 1

        reduce_expression = {
            "sum": "tl.sum(acc, axis=0)",
            "mean": "tl.sum(acc, axis=0)",
            "max": "tl.max(acc, axis=0)",
            "min": "tl.min(acc, axis=0)",
        }.get(model["ReduceOp"])
        if reduce_expression is None:
            raise ValueError(
                f"PyNTT reduction-parallel schedule does not support "
                f"{model['ReduceOp']!r}."
            )
        finalize_expression = reduce_expression
        if model["ReduceOp"] == "mean":
            finalize_expression = (
                f"({reduce_expression}) / "
                "tl.full((), "
                f"{_dim(model['InputShape'][reduction_axis])}, tl.float32)"
            )
        ...
        return {
            ...
            "loop_axes": tuple(range(rank)),
            "output_access": _tensor_access(
                tuple(f"out_idx{axis}" for axis in range(rank)),
                model["OutputStrides"],
            ),
            "finalize_expression": finalize_expression,
        }
...
def _packed_matmul_sampling_partial_template_context(
    ...
    has_operand_scales = bool(matmul.get("HasOperandScales"))
    context = _packed_gemv_pipeline_template_context(
        prepared_matmul,
        expected_family="triton.matmul_sampling_partial",
        require_operand_scales=has_operand_scales,
        required_workspace_names=_packed_matmul_sampling_partial_workspace_names(),
        allow_unscaled_fp8=True,
        maximum_block_n=128,
    )
    sampling_context = _sampling_template_context(sampling)
    rhs_scale_access = None
    if has_operand_scales:
        rhs_scale_shape = _require_list(
            matmul.get("RhsScaleShape"),
            "PackedMatMulSamplingPartial RhsScaleShape",
        )
        rhs_scale_strides = _require_list(
            matmul.get("RhsScaleStrides"),
            "PackedMatMulSamplingPartial RhsScaleStrides",
        )
        if len(rhs_scale_shape) != 2 or len(rhs_scale_strides) != 2:
            raise ValueError(
                "PyNTT packed matmul sampling rhs scale must be rank 2."
            )
        rhs_scale_access = _contiguous_vector_axis_access(
            ("0", "0"),
            rhs_scale_strides,
            tensor_shape=rhs_scale_shape,
            packed_axis=1,
            logical_index="pipeline_output_n",
            lane_count=int(matmul["OutputNVectorLaneCount"]),
            coordinate_shape=_coordinate_shape((context["block_n"],)),
        )
    context.update(
        matmul=prepared_matmul,
        sampling=sampling_context,
        rhs_scale_access=rhs_scale_access,
    )
    return context
...
def _norm_apply_template_context(model: dict[str, Any]) -> dict[str, Any]:
    ...
    input_coordinates = tensor_coordinates
    input_global_coordinate_axes: tuple[int, ...] = ()
    if model["InputUsesOutputGlobalCoordinates"]:
        input_coordinates = tuple(
            _pointer_local_to_global_coordinate(
                model["Output"], tensor_axis, coordinate
            )
            for tensor_axis, coordinate in enumerate(tensor_coordinates)
        )
        if model["Input"].get("DistributedStorageKind") == "CanonicalGlobal":
            input_global_coordinate_axes = tuple(range(rank))
    ...
    context["input_access"] = _tensor_access(
        ...
        context["lane_coordinates"],
        context["lane_shape"],
        global_coordinate_axes=input_global_coordinate_axes,
    )
...
def _gather_reduce_norm_apply_template_context(
    ...
    context["partial"] = _partial_reduction_context(
        norm_apply["StatsDType"],
        norm_apply["StatsTritonDType"],
        model["PartialStatsAddress"],
        hierarchy,
        partial_axes,
    )
    return context
...
def _partial_reduction_context(
    dtype: str,
    triton_dtype: str,
    ...
    """Build the common pooled Sum-partial reduction contract."""

    if not partial_axes or len(set(partial_axes)) != len(partial_axes):
        raise ValueError("PyNTT partial reduction requires non-empty unique axes")
    if any(axis < 0 or axis >= len(hierarchy) for axis in partial_axes):
        raise ValueError("PyNTT partial reduction axes are outside the hierarchy")
    ...
    return {
        ...
        "pointer_type": _pointer_type(
            triton_dtype, address["AddressSpace"]
        ),
...
def _gather_reduce_add_norm_template_context(
    ...
    norm = None
    cooperative = None
    hierarchy = model["Reshard"]["Hierarchy"]
    partial_axes = tuple(reshard["input_partial_mesh_axes"])
    lane_shape = tuple(model["Reshard"]["VectorLaneShape"])
    rank = len(model["Reshard"]["GlobalShape"])
    stats_address = model.get("StatsAddress")
    if (
        norm_model is not None
        and model["Reshard"]["Output"].get("DistributedStorageKind")
        == "CanonicalGlobal"
        and model["NormStats"]["Input"].get("DistributedStorageKind")
        == "CanonicalGlobal"
        and axis == rank - 1
        and len(lane_shape) == 1
        and input_split_mesh_axes | set(partial_axes) == set(range(len(hierarchy)))
        and isinstance(stats_address, dict)
        and str(stats_address.get("PoolStrideBytes", "0")) != "0"
    ):
        local_major_extent = _fixed(model["Reshard"]["InputActiveShape"][-1])
        global_major_extent = _fixed(model["Reshard"]["GlobalShape"][-1])
        lane_count = int(model["Reshard"]["VectorLaneCount"])
        if (
            local_major_extent is not None
            and global_major_extent is not None
            and local_major_extent > 0
            and global_major_extent > 0
            and lane_count > 0
            and lane_shape == (lane_count,)
        ):
            partial_extent = _product_int([hierarchy[a] for a in partial_axes])
            target_major_tile = max(1, 512 // lane_count)
            worker_count = min(
                partial_extent,
                (local_major_extent + target_major_tile - 1)
                // target_major_tile,
            )
            worker_major_extent = (
                local_major_extent + worker_count - 1
            ) // worker_count
            tile_major_width = 1 << (worker_major_extent - 1).bit_length()
            split_axes = tuple(sorted(input_split_mesh_axes))
            split_extent = _product_int([hierarchy[a] for a in split_axes])
            stats_count = split_extent * worker_count
            stats_width_cap = 1 << (stats_count - 1).bit_length()
            partial_worker_index = _split_linear_expression(
                list(partial_axes), hierarchy
            )
            global_major = _local_to_global_coordinate(
                "cooperative_local_major",
                model["Reshard"]["GlobalShape"][-1],
                model["Reshard"]["InputShardAxes"][-1],
                hierarchy,
                local_extent=local_major_extent,
            )
            coordinate_shape = f"({tile_major_width}, {lane_count})"
            prefix_coordinates = tuple("0" for _ in range(rank - 1))
            local_access = _tensor_access(
                prefix_coordinates + ("cooperative_local_major",),
                model["Reshard"]["InputStrides"],
                ("cooperative_lane",),
                lane_shape,
                coordinate_shape,
            )
            global_access = _tensor_access(
                prefix_coordinates + (global_major,),
                model["Reshard"]["OutputStrides"],
                ("cooperative_lane",),
                lane_shape,
                coordinate_shape,
            )

            split_strides: dict[int, int] = {}
            stride = 1
            for placement_axis in reversed(split_axes):
                split_strides[placement_axis] = stride
                stride *= hierarchy[placement_axis]
            partial_strides: dict[int, int] = {}
            stride = 1
            for placement_axis in reversed(partial_axes):
                partial_strides[placement_axis] = stride
                stride *= hierarchy[placement_axis]
            stats_source_coordinates = {}
            for placement_axis in range(len(hierarchy)):
                if placement_axis in split_strides:
                    stats_source_coordinates[placement_axis] = (
                        f"((stats_split_index // {split_strides[placement_axis]})"
                        f" % {hierarchy[placement_axis]})"
                    )
                else:
                    stats_source_coordinates[placement_axis] = (
                        f"((stats_worker_index // {partial_strides[placement_axis]})"
                        f" % {hierarchy[placement_axis]})"
                    )
            stats_source_shard_index = _split_linear_expression(
                list(range(len(hierarchy))),
                hierarchy,
                "stats_source_shard_coord",
            )
            stats_source_pool_index = _pool_index_expression(
                "stats_source_shard_index", stats_address["PoolScopeSize"]
            )
            cooperative = {
                "global_access": global_access,
                "global_major": global_major,
                "local_access": local_access,
                "local_major_extent": local_major_extent,
                "partial_worker_index": partial_worker_index,
                "stats_address": stats_address,
                "stats_count": stats_count,
                "stats_pointer_type": _pointer_type(
                    model["NormStats"]["OutputTritonDType"],
                    stats_address["AddressSpace"],
                ),
                "stats_source_coordinates": stats_source_coordinates,
                "stats_source_pool_index": stats_source_pool_index,
                "stats_source_shard_index": stats_source_shard_index,
                "stats_width_cap": stats_width_cap,
                "tile_major_width": tile_major_width,
                "worker_count": worker_count,
                "worker_major_extent": worker_major_extent,
            }

    cooperative_norm_apply = cooperative is not None and norm_model is not None
    if norm_model is not None:
        if cooperative_norm_apply:
            expected_scalar_extent = _product_int(
                [
                    _require_int(
                        dim.get("FixedValue"),
                        "PyNTT GatherReduceAddNorm global shape",
                        minimum=1,
                    )
                    for dim in model["Reshard"]["GlobalShape"]
                ]
            ) * int(model["Reshard"]["VectorLaneCount"])

            def require_canonical_scalar_extent(
                pointer: dict[str, Any], lane_count: int, field: str
            ) -> None:
                storage = pointer.get("DistributedStorageKind")
                if storage not in (None, "CanonicalGlobal"):
                    raise ValueError(
                        f"PyNTT GatherReduceAddNorm owner-direct {field} must use "
                        f"canonical global storage, got {storage}."
                    )
                shape = pointer.get("GlobalShape")
                strides = pointer.get("Strides")
                if not isinstance(shape, list) or not isinstance(strides, list) or len(shape) != len(strides):
                    raise ValueError(
                        f"PyNTT GatherReduceAddNorm owner-direct {field} has incomplete layout metadata."
                    )
                physical_extent = 1
                for dimension, stride in zip(reversed(shape), reversed(strides)):
                    extent = _require_int(
                        dimension.get("FixedValue"),
                        f"PyNTT GatherReduceAddNorm owner-direct {field} extent",
                        minimum=1,
                    )
                    stride_value = _require_int(
                        stride.get("FixedValue"),
                        f"PyNTT GatherReduceAddNorm owner-direct {field} stride",
                        minimum=0,
                    )
                    if extent != 1 and stride_value != physical_extent:
                        raise ValueError(
                            f"PyNTT GatherReduceAddNorm owner-direct {field} must be scalar-contiguous."
                        )
                    physical_extent *= extent
                if physical_extent * lane_count != expected_scalar_extent:
                    raise ValueError(
                        f"PyNTT GatherReduceAddNorm owner-direct {field} scalar extent "
                        f"{physical_extent * lane_count} does not match {expected_scalar_extent}."
                    )

            require_canonical_scalar_extent(
                norm_model["Scale"], int(norm_model["ScaleVectorLaneCount"]), "scale"
            )
            require_canonical_scalar_extent(
                norm_model["Bias"], int(norm_model["BiasVectorLaneCount"]), "bias"
            )
            require_canonical_scalar_extent(
                norm_model["Output"], int(norm_model["OutputVectorLaneCount"]), "output"
            )
            logical_input_global_shape = _logical_shape(
                norm_model["InputGlobalShape"], norm_model["InputVectorLaneCount"]
            )
            norm = {
                "normalization_size": _product(logical_input_global_shape[axis:]),
            }
        else:
            if bool(norm_model.get("RequiresCooperativeOwnerDirect")):
                raise ValueError(
                    "PyNTT GatherReduceAddNorm terminal output cast requires the "
                    "cooperative owner-direct lowering."
                )
            norm = _norm_apply_template_context(norm_model)
    return {
        "cooperative": cooperative,
        "cooperative_norm_apply": cooperative_norm_apply,
        "reshard": reshard,
        "stats": stats,
        "norm": norm,
    }
...
def _rope_template_context(model: dict[str, Any]) -> dict[str, Any]:
    ...
    rotary_physical_extent = (
        None
        if sincos_physical_extent is None
        else sincos_physical_extent * sincos_pack_factor
    )
    if (
        output_physical_extent is None
        or sincos_physical_extent is None
        or rotary_physical_extent is None
        or rotary_physical_extent <= 0
        or rotary_physical_extent > output_physical_extent
        or rotary_physical_extent % 2 != 0
    ):
        raise ValueError(
            "PyNTT RoPE requires a static positive even rotary prefix no larger "
            "than the output rotary extent: "
    ...
    is_rotary = f"{output_physical_rotary} < {rotary_physical_extent}"
    half_rotary_physical_extent = rotary_physical_extent // 2
    first_half = f"{output_physical_rotary} < {half_rotary_physical_extent}"
    paired_physical_rotary = (
        f"tl.where({first_half}, {output_physical_rotary} + "
        f"{half_rotary_physical_extent}, tl.where({is_rotary}, "
        f"{output_physical_rotary} - {half_rotary_physical_extent}, "
        f"{output_physical_rotary}))"
    )
    lane_coordinates = context["lane_coordinates"]
    lane_flat = _flatten_coordinates(lane_coordinates, output_lane_shape)
    logical_rotary = (
    ...
    if sincos_pack_factor == 1:
        sincos_physical_rotary = output_physical_rotary
        sincos_lane_coordinates = lane_coordinates
    else:
        sincos_physical_rotary = (
            f"({output_physical_rotary}) // {sincos_pack_factor}"
        )
        pack_coordinate = f"({output_physical_rotary}) % {sincos_pack_factor}"
        sincos_lane_coordinates = (pack_coordinate,) + tuple(lane_coordinates)
    ...
    context.update(
        ...
        rotary_physical_extent=rotary_physical_extent,
...
def _gated_delta_net_partial_input_context(
    partial: dict[str, Any],
    target: dict[str, Any],
    target_scalar_index: str,
    target_lane_count: int,
    target_tile_rank: int,
    field: str,
    variable_prefix: str,
) -> dict[str, Any]:
    """Map one local GDN output element to compact Sum-partial storage."""

    if len(partial["GlobalShape"]) != 2 or len(partial["Strides"]) != 2:
        raise ValueError(f"PyNTT {field} partial input must be a packed rank-2 tensor")
    if target_lane_count <= 0:
        raise ValueError(f"PyNTT {field} target lane count must be positive")
    if not variable_prefix.isidentifier():
        raise ValueError(
            f"PyNTT {field} partial variable prefix must be an identifier"
        )
    hierarchy = [int(extent) for extent in partial["Hierarchy"]]
    if target.get("Hierarchy") != partial["Hierarchy"]:
        raise ValueError(f"PyNTT {field} partial and output views must use one hierarchy")
    partial_axes = tuple(sorted(int(axis) for axis in partial["PartialAxes"]))
    if not partial_axes or any(axis < 0 or axis >= len(hierarchy) for axis in partial_axes):
        raise ValueError(f"PyNTT {field} partial axes are invalid: {partial_axes}")
    split_axes = set(_shard_axes_hierarchy_axes(partial["ShardAxes"]))
    if split_axes & set(partial_axes):
        raise ValueError(f"PyNTT {field} mesh axes cannot be both split and partial")

    source_lane_shape = _validate_coordinate_lane_shape(
        partial["VectorLaneShape"], f"PyNTT {field} partial"
    )
    source_lane_count = _product_int(list(source_lane_shape)) if source_lane_shape else 1
    if source_lane_count != int(partial["VectorLaneCount"]):
        raise ValueError(f"PyNTT {field} partial lane metadata is inconsistent")

    target_token_name = f"{variable_prefix}_target_token"
    target_packed_name = f"{variable_prefix}_target_packed"
    target_scalar_name = f"{variable_prefix}_target_scalar"
    source_packed_name = f"{variable_prefix}_source_packed"
    source_lane_flat_name = f"{variable_prefix}_source_lane_flat"
    target_token = _pointer_local_to_global_coordinate(target, 0, "0")
    target_packed = _pointer_local_to_global_coordinate(
        target, 1, f"({target_scalar_index}) // {target_lane_count}"
    )
    coordinate_bindings = [
        {"name": target_token_name, "expression": target_token},
        {"name": target_packed_name, "expression": target_packed},
        {
            "name": target_scalar_name,
            "expression": (
                f"({target_packed_name}) * {target_lane_count} + "
                f"({target_scalar_index}) % {target_lane_count}"
            ),
        },
        {
            "name": source_packed_name,
            "expression": f"({target_scalar_name}) // {source_lane_count}",
        },
        {
            "name": source_lane_flat_name,
            "expression": f"({target_scalar_name}) % {source_lane_count}",
        },
    ]
    source_global = (target_token_name, source_packed_name)
    source_lane_coordinates: list[str] = []
    lane_stride = source_lane_count
    for index, extent in enumerate(source_lane_shape):
        lane_stride //= extent
        if len(source_lane_shape) == 1 and extent == source_lane_count:
            source_lane_coordinates.append(source_lane_flat_name)
            continue
        coordinate_name = f"{variable_prefix}_source_lane{index}"
        coordinate_bindings.append(
            {
                "name": coordinate_name,
                "expression": (
                    f"(({source_lane_flat_name}) // {lane_stride}) % {extent}"
                    if lane_stride != 1
                    else f"({source_lane_flat_name}) % {extent}"
                ),
            }
        )
        source_lane_coordinates.append(coordinate_name)
    ...
    for plan in axis_plans:
        for axis, owner in plan["owners"].items():
            ...
            if previous != owner:
                raise ValueError(
                    f"PyNTT {field} source mesh axis {axis} has conflicting owners"
                )
    ...
    if dtype in ("float16", "bfloat16", "float32"):
        ...
        raise ValueError(f"PyNTT {field} partial Sum does not support dtype {dtype}")
...
def _reshard_template_context(model: dict[str, Any]) -> dict[str, Any]:
    ...
    context["output_local_access"] = _tensor_access(
        output_global_origins,
        model["OutputStrides"],
        tuple("0" for _ in context["lane_shape"]),
        context["lane_shape"],
    )
...
def _paged_attention_template_context(model: dict[str, Any]) -> dict[str, Any]:
    ...
    has_output_gate = bool(model.get("HasOutputGate"))
    if has_output_gate != isinstance(model.get("OutputGate"), dict):
        raise ValueError(
            "PyNTT PagedAttention output-gate metadata and pointer disagree"
        )
    if has_output_gate and len(model.get("OutputGateStrides", ())) != len(
        model["OutputShape"]
    ):
        raise ValueError(
            "PyNTT PagedAttention output gate must have the output rank"
        )
    ...
    context = {
        ...
        "has_output_gate": has_output_gate,
...
def _paged_attention_merge_template_context(
    ...
    if has_output_gate != isinstance(model.get("OutputGate"), dict):
        raise ValueError(
            "PyNTT PagedAttentionMerge output-gate metadata and pointer disagree"
        )
    if has_output_gate and len(model.get("OutputGateStrides", ())) != output_rank:
        raise ValueError(
            "PyNTT PagedAttentionMerge output gate must have the output rank"
        )
    ...
    output_gate_is_canonical = has_output_gate and (
        model["OutputGate"].get("DistributedStorageKind") == "CanonicalGlobal"
    )
    output_gate_indices = ["0"] * output_rank
    output_gate_indices[model["SeqAxis"]] = (
        global_query_id if output_gate_is_canonical else "local_query_id"
    )
    output_gate_indices[model["HeadAxis"]] = (
        global_q_head if output_gate_is_canonical else "q_head"
    )
    output_gate_indices[dim_axis] = (
        global_output_physical_dim
        if output_gate_is_canonical
        else output_dim_axis["physical_coordinate"]
    )
