...
internal sealed class PyNTTKernelSourceConvertVisitor : ExprFunctor<Unit, Unit>
{
    ...
    protected override Unit VisitPrimFunction(PrimFunction expr)
    {
        ...
        if (expr.Role is FunctionRole.Dispatch or FunctionRole.ModuleDispatch)
        {
        ...
        HashSet<PrimFunction> activeFunctions)
    {
        switch (expr)
        {
            ...
            case Call { Target: PrimFunction callee } call
                when callee.ModuleKind != PyNTTTarget.Kind:
                if (callee.Parameters.Length != call.Arguments.Length)
                {
                    throw new NotSupportedException(
                        $"Cross-module call to {callee.ModuleKind} function {callee.Name} has " +
                        $"{call.Arguments.Length} arguments for {callee.Parameters.Length} parameters.");
                }
    ...
    private static bool ContainsExecutableKernelWork(BaseExpr expr, bool includeCallees, HashSet<PrimFunction> activeFunctions)
    {
        if (expr is Call call)
        {
            switch (call.Target)
            {
                case PrimFunction callee when includeCallees && callee.ModuleKind == PyNTTTarget.Kind:
...
#pragma warning disable SA1201
    private sealed class PyNTTPrimFunctionSourceVisitor : ExprFunctor<Unit, Unit>
    {
        ...
        private readonly SortedDictionary<string, PyNTTGridBarrierAxisGroup> _gridBarrierAxisGroups = new(StringComparer.Ordinal);
        ...
        protected override Unit VisitProducerConsumerRegion(ProducerConsumerRegion expr)
        {
            ...
            try
            {
                ...
                {
                    AuxiliaryConsumerFunctionName = state.HasAuxiliaryConsumer
                        ? $"{helperName}__auxiliary_consumer"
                        : null,
                    AuxiliaryConsumerBodySource = state.AuxiliaryConsumerBodySource,
                    AuxiliaryConsumerEndpointNames = state.AuxiliaryConsumerEndpointNames.ToArray(),
                };
                WriteHelperTemplate(
        ...
        protected override Unit VisitPipelineDrain(PipelineDrain expr)
        {
            ...
            var endpoints = _pipelineRole switch
            {
                PipelineExecutionRole.Consumer => stage.ConsumerDrainEndpointNames
                    .Where(name => !stage.AuxiliaryConsumerDrainEndpointNames.Contains(name))
                    .ToArray(),
        ...
        protected override Unit VisitCall(Call expr)
        {
            ...
            switch (expr.Target)
            {
                ...
                case Nncase.TIR.Memcopy:
                    VisitMemcopy(args);
                    break;
                case Nncase.TIR.ChannelProduce produce:
                    VisitPipelineChannelEndpoint(produce, args, isProducer: true);
                    break;
                case Nncase.TIR.ChannelConsume consume:
                    VisitPipelineChannelEndpoint(consume, args, isProducer: false);
                    break;
                ...
                case Nncase.TIR.NTT.PackedMatMul packedMatmul:
                    ...
                    break;
                case Nncase.TIR.NTT.PackedScaledMatMul packedScaledMatmul:
                    VisitPackedScaledMatmul(
                        packedScaledMatmul,
                        args,
                        RequireMicroKernel(microKernel, packedScaledMatmul));
                    break;
                case Nncase.TIR.NTT.PackedBlockScaledMatMul packedBlockScaledMatmul:
                    VisitPackedBlockScaledMatmul(
                        packedBlockScaledMatmul,
                        args,
                        RequireMicroKernel(microKernel, packedBlockScaledMatmul));
                    break;
                case Nncase.TIR.NTT.PackedBlockScaledMatMulNormStats packedBlockScaledMatmulNormStats:
                    VisitPackedBlockScaledMatmulNormStats(
                        packedBlockScaledMatmulNormStats,
                        args,
                ...
                case Nncase.TIR.NTT.PagedAttentionMergePackedMatMul fused:
                    ...
                    break;
                case Nncase.TIR.NTT.NVFP4MatMulNormStats nvfp4MatmulNormStats:
                    VisitNVFP4MatmulNormStats(
                        nvfp4MatmulNormStats,
                        args,
                        RequireMicroKernel(microKernel, nvfp4MatmulNormStats));
                    break;
                ...
                case Nncase.TIR.NTT.NVFP4MatMulGlu nvfp4MatMulGlu:
                    ...
                    break;
                case Nncase.TIR.NTT.GatherReduceNormApplyNVFP4MatMulGlu fusedNormGlu:
                    VisitGatherReduceNormApplyNVFP4MatMulGlu(
                        fusedNormGlu,
                        args,
                        RequireMicroKernel(microKernel, fusedNormGlu));
                    break;
                ...
                case Nncase.TIR.NTT.QKVRoPEWithCache qkvRoPEWithCache:
                    ...
                    break;
                case Nncase.TIR.NTT.GatedDeltaNetConvolution gatedDeltaNetConvolution:
                    VisitGatedDeltaNetConvolution(
                        gatedDeltaNetConvolution,
                        args,
                        RequireMicroKernel(microKernel, gatedDeltaNetConvolution));
                    break;
            ...
            IReadOnlyList<BaseExpr> sharedWorkspace)
        {
            ...
            var auxiliaryConsumer = selection.TransferPipeline?.AuxiliaryConsumer is { } auxiliary
                ? new PyNTTAuxiliaryConsumerTemplateModel(
                    auxiliary.ChannelIndices
                        .Select(index => transferChannels[index].Name)
                        .ToArray(),
                    auxiliary.ConsumerSharedWorkspaceIndices
                        .Select(index => selection.SharedWorkspaces[index].Name)
                        .ToArray())
                : null;
            return new(
            ...
            IReadOnlyDictionary<IVar, PyNTTShardAxisTemplateModel[]> tensorSourceShardAxes)
        {
            ...
            var relocationBindings = BuildChipLocalRDataRelocationBindings(
                callee,
                deviceFunctionName);
            ...
            for (var i = 0; i < calleeParameters.Length; i++)
            {
                ...
                if (IsObjectDataType(parameter.CheckedDataType))
                {
                    ...
                    if (IsPipelineChannelExpression((BaseExpr)parameter))
                    {
                        extraParameters.Add(baseName);
                        extraPointerParameterTritonTypes.Add(baseName, "tl.uint8");
                    }
                ...
                if (parameter is BufferVar bufferVar)
                {
                    ...
                    var globalOffsetNames = isRuntimeStrided
                        ? Enumerable.Range(0, tensorType.Shape.Rank)
                            ...
                            .ToArray()
                        : Array.Empty<string>();
                    string? relocationAnchorName = null;
                    long relocationOffsetBytes = 0;
                    if (relocationBindings.TryGetValue(i, out var relocation))
                    {
                        relocationAnchorName = relocation.AnchorName;
                        relocationOffsetBytes = relocation.RelativeOffsetBytes;
                        var scalarTritonDType = GetScalarTritonDType(tensorType.DType);
                        var relativeOffset = relocationOffsetBytes.ToString(CultureInfo.InvariantCulture);
                        baseName = relocationOffsetBytes == 0
                            ? $"({relocationAnchorName}).to({GetPointerTypeExpression(scalarTritonDType, 1)})"
                            : $"({relocationAnchorName} + {relativeOffset}).to({GetPointerTypeExpression(scalarTritonDType, 1)})";
                        extraParameters.Add(relocationAnchorName);
                        extraPointerParameterTritonTypes.TryAdd(relocationAnchorName, "tl.uint8");
                    }
                    ...
                    if (relocationAnchorName is null)
                    {
                        extraParameters.Add(baseName);
                    ...
                    parameters.Add(new DeviceFunctionFormalParameter(i, parameter, DeviceFunctionFormalParameterKind.Tensor, baseName, poolStrideName, poolScopeSizeName, strideNames, dimensionNames, globalOffsetNames, null, null, relocationAnchorName, relocationOffsetBytes));
                    continue;
                }
        ...
        private Dictionary<int, DeviceFunctionRelocationBinding> BuildChipLocalRDataRelocationBindings(
            PrimFunction callee,
            string deviceFunctionName)
        {
            var callSites = ExprCollector.Collect(_bodyExpr)
                .OfType<Call>()
                .Where(call =>
                    call.Target is PrimFunction target &&
                    string.Equals(target.Name, callee.Name, StringComparison.Ordinal))
                .ToArray();
            if (callSites.Length == 0)
            {
                throw new InvalidOperationException(
                    $"PyNTT cannot find a call site for device function {deviceFunctionName}.");
            }

            var candidates = new List<DeviceFunctionRelocationCandidate>();
            var parameters = callee.Parameters.ToArray();
            for (var index = 0; index < parameters.Length; index++)
            {
                if (parameters[index] is not BufferVar { Role: not BufferVarRole.Workspace } parameter)
                {
                    continue;
                }

                var locations = new List<DeviceFunctionRelocationLocation>(callSites.Length);
                foreach (var callSite in callSites)
                {
                    if (callSite.Arguments.Length != parameters.Length ||
                        !TryGetDirectBufferOperand(callSite.Arguments[index], out var buffer) ||
                        buffer.MemSpan.Buffer.Location != MemoryLocation.ChipLocalRdata)
                    {
                        locations.Clear();
                        break;
                    }
                    ...
                    if (!bufferRef.IsByteAddressed ||
                        ...
                        !string.IsNullOrWhiteSpace(bufferRef.IndexExpression) ||
                        !long.TryParse(
                    ...
                    locations.Add(new(bufferRef.BaseName, offsetBytes));
                }

                if (locations.Count != callSites.Length)
                {
                    continue;
                }

                var firstOffset = locations[0].OffsetBytes;
                var translationSignature = string.Join(
                    ";",
                    locations.Select(location =>
                        $"{location.BaseName}:{checked(location.OffsetBytes - firstOffset).ToString(CultureInfo.InvariantCulture)}"));
                candidates.Add(new(index, firstOffset, translationSignature));
            }

            var bindings = new Dictionary<int, DeviceFunctionRelocationBinding>();
            var groupIndex = 0;
            foreach (var group in candidates
                         .GroupBy(candidate => candidate.TranslationSignature, StringComparer.Ordinal)
                         .Where(group => group.Count() > 1)
                         .OrderBy(group => group.Min(candidate => candidate.Index)))
            {
                var anchorOffsetBytes = group.Min(candidate => candidate.FirstOffsetBytes);
                var anchorName = SanitizePythonIdentifier(
                    $"{deviceFunctionName}_chip_local_rdata_anchor_{groupIndex.ToString(CultureInfo.InvariantCulture)}");
                foreach (var candidate in group)
                {
                    bindings.Add(
                        candidate.Index,
                        new DeviceFunctionRelocationBinding(
                            anchorName,
                            checked(candidate.FirstOffsetBytes - anchorOffsetBytes)));
                }
        ...
        private static bool TryGetDirectBufferOperand(BaseExpr expr, out TIR.Buffer buffer)
        {
            expr = UnwrapInputBoxing(expr);
            if (expr is TIR.Buffer directBuffer)
            {
                buffer = directBuffer;
                return true;
            }
            ...
            foreach (var parameter in definitionParameters)
            {
                ...
                switch (parameter.Kind)
                {
                    ...
                    case DeviceFunctionFormalParameterKind.Tensor:
                        ...
                        if (parameter.RelocationAnchorName is { } relocationAnchorName)
                        {
                            var bufferRef = ResolveBufferRef(buffer);
                            if (!bufferRef.IsByteAddressed ||
                                bufferRef.PoolStrideBytes != "0" ||
                            ...
                            {
                                throw new NotSupportedException(
                                    $"PyNTT call to {callee.Name} cannot bind relocation anchor " +
                                    $"{relocationAnchorName} from parameter {parameter.Parameter.Name}: " +
                                    $"base={bufferRef.BaseName}, offset={bufferRef.OffsetBytes}, " +
                                    $"stride={bufferRef.PoolStrideBytes}, index={bufferRef.IndexExpression ?? "<none>"}.");
                            }

                            var anchorOffsetBytes = checked(offsetBytes - parameter.RelocationOffsetBytes);
                            if (anchorOffsetBytes < 0)
                            {
                                throw new InvalidOperationException(
                                    $"PyNTT call to {callee.Name} relocation anchor {relocationAnchorName} " +
                                    $"underflows for parameter {parameter.Parameter.Name}: " +
                                    $"offset={offsetBytes}, relative={parameter.RelocationOffsetBytes}.");
                            }

                            var anchorRef = bufferRef with
                            {
                                OffsetBytes = anchorOffsetBytes.ToString(CultureInfo.InvariantCulture),
                                IndexExpression = null,
                            };
                            AddConsistentDeviceFunctionBinding(
                                values,
                                relocationAnchorName,
                                BuildPointerExpression(anchorRef, "tl.uint8"),
                                callee.Name);
                        }
                        else
                        {
                    ...
                    case DeviceFunctionFormalParameterKind.Object:
                        ...
                        if (IsPipelineChannelExpression((BaseExpr)parameter.Parameter))
                        {
                            values[objectBaseName] = GetPipelineChannelPointerArgument(argument);
                        }
        ...
        private static void AddConsistentDeviceFunctionBinding(
            IDictionary<string, string> values,
            string name,
            string value,
            string calleeName)
        {
            if (values.TryGetValue(name, out var existing) && existing != value)
            {
                throw new NotSupportedException(
                    $"PyNTT call to {calleeName} has incompatible relocation anchor {name}: " +
                    $"{existing} versus {value}.");
            }
        ...
        private void VisitPipelineChannelEndpoint(
            Op endpoint,
            IReadOnlyList<BaseExpr> args,
            bool isProducer)
        {
            if (args.Count != 2 || args[0] is not IVar channel || args[1] is not TIR.Buffer payload)
            {
                throw new NotSupportedException(
                    $"PyNTT {endpoint.GetType().Name} expects an opaque channel parameter and one TIR buffer payload.");
            }

            if (channel.CheckedDataType is not ReferenceType
                {
                    ElemType: Nncase.IR.Heterogeneous.PipelineChannelType,
                })
            {
                throw new NotSupportedException(
                    $"PyNTT {endpoint.GetType().Name} channel {channel.Name} has invalid type {channel.CheckedType}.");
            }

            if (payload.DistributedType?.Partial is not null)
            {
                throw new NotSupportedException(
                    $"PyNTT heterogeneous channel {GetPipelineChannelId(endpoint)} cannot transfer partial tensor {payload.DistributedType}. " +
                    "Materialize the reduction before crossing a module boundary.");
            }

            var channelInput = GetPipelineChannelPointerArgument((BaseExpr)channel);
            var localShape = GetBufferActiveShape(payload);
            var zeroOffsets = CreateZeroDimExpressions(payload.Rank);
            var globalShape = GetBufferGlobalShape(payload);
            var globalStrides = CreateContiguousStrides(globalShape);
            var shardAxes = GetBufferSourceShardAxes(payload, payload.Rank);
            var hierarchy = GetHierarchy(payload);
            var canonicalPointer = new PyNTTBufferPointerTemplateModel(
                $"({channelInput} + {Nncase.IR.Heterogeneous.PipelineChannelContract.HeaderBytes.ToString(CultureInfo.InvariantCulture)}).to(tl.pointer_type({GetScalarTritonDType(payload.ElemType)}))")
            {
                DistributedStorageKind = DistributedBufferStorageKind.CanonicalGlobal.ToString(),
                GlobalShape = globalShape,
                GlobalOffsets = zeroOffsets,
                Strides = globalStrides,
                ShardAxes = shardAxes,
                Hierarchy = hierarchy,
            };
            var localPointer = GetRegionCopyBufferPointer(payload);
            var source = isProducer ? localPointer : canonicalPointer;
            var destination = isProducer ? canonicalPointer : localPointer;
            var copyPlan = BuildRegionCopyPlan(
                localShape,
                localShape,
                zeroOffsets,
                zeroOffsets,
                isProducer ? GetBufferStrides(payload) : globalStrides,
                isProducer ? globalStrides : GetBufferStrides(payload),
                GetVectorLanes(payload.ElemType));
            var endpointName = isProducer ? "produce" : "consume";
            var channelId = GetPipelineChannelId(endpoint);
            var phase = GetPipelineChannelPhase(endpoint) + 1;
            var helperName = GetNextHelperName($"pipeline_channel_{endpointName}");
            var copyModel = new PyNTTRegionCopyTemplateModel(
                $"{helperName}_copy",
                source,
                destination,
                GetPyNTTDTypeName(payload.ElemType),
                GetScalarTritonDType(payload.ElemType),
                localShape,
                localShape,
                zeroOffsets,
                zeroOffsets,
                ...
                isProducer ? globalStrides : GetBufferStrides(payload),
                GetVectorLanes(payload.ElemType),
                $"PipelineChannel{endpointName}",
                copyPlan,
                false,
                $"{channelId}: {payload.Name}");
            var producerOwnerAxes = shardAxes
            ...
            _attrs["requires_grid_barrier"] = true;
            WriteHelperTemplate(
                "triton/kernels/PipelineChannel.py.jinja",
                new PyNTTPipelineChannelEndpointTemplateModel(
                    helperName,
                    channelInput,
                    phase,
                    isProducer,
                    producerOwnerAxes,
                    copyModel,
                    $"{endpointName} {channelId} phase {phase.ToString(CultureInfo.InvariantCulture)}"));
            WriteLine(BuildHelperCall(helperName));
        }

        private static string GetPipelineChannelId(Op endpoint)
            => endpoint switch
            {
                Nncase.TIR.ChannelProduce produce => produce.ChannelId,
                Nncase.TIR.ChannelConsume consume => consume.ChannelId,
                _ => throw new ArgumentOutOfRangeException(nameof(endpoint)),
            };

        private static int GetPipelineChannelPhase(Op endpoint)
            => endpoint switch
            {
                Nncase.TIR.ChannelProduce produce => produce.Phase,
                Nncase.TIR.ChannelConsume consume => consume.Phase,
                _ => throw new ArgumentOutOfRangeException(nameof(endpoint)),
            };

        private static PyNTTDimExpression[] CreateContiguousStrides(
            IReadOnlyList<PyNTTDimExpression> shape)
        {
            ...
            {
                strides[axis] = stride;
                stride = MultiplyDimExpressions(stride, shape[axis]);
            }
        ...
        private void VisitGatherReduceScatter(Nncase.TIR.NTT.GatherReduceScatter gatherReduceScatter, IReadOnlyList<BaseExpr> args)
        {
            ...
            {
                throw new NotSupportedException(
                    $"PyNTT GatherReduceScatter in {_currentFunction.Name} cannot materialize a partial output; " +
                    $"partial reduction must be carried by the input type. Input={gatherReduceScatter.InType}, " +
                    $"output={gatherReduceScatter.OutType}.");
            }
        ...
        private void VisitSlice(Nncase.TIR.NTT.Slice slice, IReadOnlyList<BaseExpr> args)
        {
            ...
            ValidateMatchingVectorLanes("PyNTT Slice input/output", input.ElemType, output.ElemType);
            _attrs["op"] = "slice";
            _attrs["dtype"] = GetPyNTTScalarDTypeName(output.ElemType);
            _attrs["shape"] = outputShape;
            ...
            Action<string, PyNTTMatmulTemplateModel>? emit = null)
        {
            ...
            var hasTensorScales = lhsScale is not null && rhsScale is not null;
            var hasBlockScale = lhsScale is null && rhsScale is not null;
            if (lhsScale is not null && rhsScale is null)
            {
                throw new NotSupportedException(
                    "PyNTT PackedMatMul cannot have an lhs scale without an rhs scale.");
            }

            if (hasTensorScales)
            {
                ValidateOperandScaleBuffer("PyNTT PackedMatMul lhs", lhsScale!);
                ValidateOperandScaleBuffer("PyNTT PackedMatMul rhs", rhsScale!);
                if (weightBlockN != 0 || weightBlockK != 0)
                {
                    throw new NotSupportedException(
                        "PyNTT tensor-scaled PackedMatMul cannot declare weight block dimensions.");
                }
            }
            else if (hasBlockScale)
            {
                if (weightBlockN <= 0 || weightBlockK <= 0 ||
                    GetBufferActiveShape(rhsScale!).Length != 2)
                {
                    throw new NotSupportedException(
                        "PyNTT block-scaled PackedMatMul requires a rank-2 rhs scale and positive block dimensions.");
                }
            }
            else if (weightBlockN != 0 || weightBlockK != 0)
            {
                throw new NotSupportedException(
                    "PyNTT PackedMatMul weight block dimensions require an rhs block scale.");
            }
            ...
            var lhsScalarType = GetScalarDataType(lhs.ElemType);
            var rhsScalarType = GetScalarDataType(rhs.ElemType);
            var outputScalarType = GetScalarDataType(output.ElemType);
            if (!hasTensorScales && !hasBlockScale)
            {
                if (lhsScalarType != rhsScalarType)
                {
                    throw new NotSupportedException(
                        $"PyNTT PackedMatMul expects unscaled lhs/rhs scalar dtypes to match, got {lhsScalarType}/{rhsScalarType}.");
                }
            }
            else if (lhsScalarType != DataTypes.BFloat16 ||
                     rhsScalarType != DataTypes.Float8E4M3 ||
                     outputScalarType != DataTypes.BFloat16)
            {
                throw new NotSupportedException(
                    "PyNTT scaled PackedMatMul currently requires BF16 input/output and E4M3 weights, " +
                    $"got {lhsScalarType}/{rhsScalarType}/{outputScalarType}.");
            }
            ...
            switch (matmul.RhsLayout)
            {
                case IR.NTT.PackedMatMulRhsLayout.NMajor when rhsVectorLanes.Length == 2:
                    ...
                    rhsNVectorLaneCount = nVectorLaneCount;
                    kPackedLaneCount = 1;
                    kVectorLaneCount = 1;
                    transposeB = true;
                    rhsLayout = "n_major";
                ...
                case IR.NTT.PackedMatMulRhsLayout.KMajor when rhsVectorLanes.Length == 3:
                    ...
                    break;
                case IR.NTT.PackedMatMulRhsLayout.NMajorKPacked when rhsVectorLanes.Length == 2:
                    if (outputNVectorLaneCount is null or <= 0)
                    {
                        throw new NotSupportedException(
                            "PyNTT PackedMatMul N-major K-packed layout requires an explicit positive output N vector lane count.");
                    }

                    nPackedLaneCount = 1;
                    nVectorLaneCount = outputNVectorLaneCount.Value;
                    rhsNPackedLaneCount = 1;
                    rhsNVectorLaneCount = 1;
                    kPackedLaneCount = rhsVectorLanes[0];
                    kVectorLaneCount = rhsVectorLanes[1];
                    transposeB = true;
                    rhsLayout = "n_major_k_packed";
                    if (outputVectorLanes.Length != 1 ||
                        outputVectorLanes[0] != nVectorLaneCount)
                    {
                        throw new NotSupportedException(
                            $"PyNTT PackedMatMul N-major K-packed expects output lanes [{nVectorLaneCount}], got [{string.Join(",", outputVectorLanes)}].");
                    }
            ...
            var rhsPhysicalNScalarLaneCount = checked(rhsNPackedLaneCount * rhsNVectorLaneCount);
            var lhsK = lhsShape[^1];
            var rhsK = transposeB
                ? MultiplyDim(rhsShape[^1], checked(kPackedLaneCount * kVectorLaneCount))
                : MultiplyDim(rhsShape[^2], checked(kPackedLaneCount * kVectorLaneCount));
            var lhsM = lhsShape[^2];
            var rhsNOuter = transposeB ? rhsShape[^2] : rhsShape[^1];
            var rhsN = MultiplyDim(rhsNOuter, rhsPhysicalNScalarLaneCount);
            var outputN = MultiplyDim(outputShape[^1], rhsNScalarLaneCount);
            if (!SameDim(lhsK, rhsK) ||
                !SameDim(outputShape[^2], lhsM) ||
                !SameDim(outputN, rhsN))
            {
                var expectedRhs = transposeB
                    ? matmul.RhsLayout == IR.NTT.PackedMatMulRhsLayout.NMajorKPacked
                        ? "rhs=[...,N,K/(KPack*KVector)]<KPack,KVector>"
            ...
            _attrs["op"] = hasBlockScale
                ? stats is null
                    ? "packed_block_scaled_matmul"
                    : "packed_block_scaled_matmul_norm_stats"
                : stats is null ? "packed_matmul" : "packed_matmul_norm_stats";
            _attrs["dtype"] = GetPyNTTScalarDTypeName(output.ElemType);
            _attrs["shape"] = outputShape;
            ...
            var helperName = GetNextHelperName(
                hasBlockScale
                    ? stats is null
                        ? "block_fp8_gemv_compute"
                        : "block_fp8_gemv_norm_stats_compute"
                    : stats is null
            ...
            var templateModel = new PyNTTMatmulTemplateModel(
                ...
                $"{lhs.Name}, {rhs.Name} -> {output.Name}")
            {
                LhsGlobalOffsets = GetBufferGlobalOffsets(lhs),
                OutputGlobalOffsets = GetBufferGlobalOffsets(output),
                ...
                RhsNPackedLaneCount = rhsNPackedLaneCount,
                OutputNPackedLaneCount = nPackedLaneCount,
                RhsLayout = rhsLayout,
                RhsKPackLaneCount = kPackedLaneCount,
                RhsKVectorLaneCount = kVectorLaneCount,
                LhsScale = lhsScale is null ? null : GetBufferScalarPointer(lhsScale),
                RhsScale = rhsScale is null ? null : GetBufferScalarPointer(rhsScale),
                RhsScaleShape = rhsScale is null
                    ? Array.Empty<PyNTTDimExpression>()
                    : GetBufferActiveShape(rhsScale),
                RhsScaleStrides = rhsScale is null
                    ? Array.Empty<PyNTTDimExpression>()
                    : GetBufferStrides(rhsScale),
                WeightBlockN = weightBlockN,
                WeightBlockK = weightBlockK,
        ...
        private void VisitPackedScaledMatmul(
            Nncase.TIR.NTT.PackedScaledMatMul matmul,
            IReadOnlyList<BaseExpr> args,
            PyNTTMicroKernelTemplateModel microKernel)
        {
            if (args.Count != 5 ||
                args[0] is not TIR.Buffer lhs ||
                args[1] is not TIR.Buffer rhs ||
                args[2] is not TIR.Buffer lhsScale ||
                args[3] is not TIR.Buffer rhsScale ||
                args[4] is not TIR.Buffer output)
            {
                throw new NotSupportedException(
                    "PyNTT PackedScaledMatMul codegen expects lhs, rhs, lhs/rhs scale, and output TIR buffers.");
            }
        ...
        private void VisitPackedBlockScaledMatmul(
            Nncase.TIR.NTT.PackedBlockScaledMatMul matmul,
            IReadOnlyList<BaseExpr> args,
            PyNTTMicroKernelTemplateModel microKernel)
        {
            if (args.Count != 5 ||
                args[0] is not TIR.Buffer lhs ||
                args[1] is not TIR.Buffer rhs ||
                args[2] is not TIR.Buffer rhsScale ||
                args[3] is not TIR.Buffer output)
            {
                throw new NotSupportedException(
                    "PyNTT PackedBlockScaledMatMul codegen expects lhs, rhs, rhs scale, output, and addend operands.");
            }
            ...
            VisitPackedMatmul(
                ...
                microKernel,
                rhsScale: rhsScale,
                weightBlockN: matmul.WeightBlockN,
                weightBlockK: matmul.WeightBlockK,
                outputNVectorLaneCount: matmul.OutputNVectorLaneCount);
        }

        private void VisitPackedBlockScaledMatmulNormStats(
            Nncase.TIR.NTT.PackedBlockScaledMatMulNormStats matmul,
            IReadOnlyList<BaseExpr> args,
            PyNTTMicroKernelTemplateModel microKernel)
        {
            if (args.Count != 6 ||
                ...
                args[2] is not TIR.Buffer rhsScale ||
                args[3] is not TIR.Buffer output ||
                args[4] is not TIR.Buffer stats)
            {
                throw new NotSupportedException(
                    "PyNTT PackedBlockScaledMatMulNormStats codegen expects lhs, rhs, rhs scale, value output, statistics output, and addend operands.");
            }
            ...
            PyNTTMicroKernelTemplateModel microKernel)
        {
            ...
            {
                throw new NotSupportedException(
                    "PyNTT PackedMatMulSamplingPartial codegen expects lhs, rhs, state, raw logits, processed logits, " +
                    "partial argmax state, matmul scale/addend, and optional lhs/rhs output scales.");
            }
            ...
            var lhsScale = args[8] switch
            {
                TIR.Buffer buffer => buffer,
                None => null,
                _ => throw new NotSupportedException(
                    "PyNTT PackedMatMulSamplingPartial lhs scale must be a TIR buffer or None."),
            };
            var rhsScale = args[9] switch
            {
                TIR.Buffer buffer => buffer,
                None => null,
                _ => throw new NotSupportedException(
                    "PyNTT PackedMatMulSamplingPartial rhs scale must be a TIR buffer or None."),
            };
            if ((lhsScale is null) != (rhsScale is null))
            {
                throw new NotSupportedException(
                    "PyNTT PackedMatMulSamplingPartial requires both lhs/rhs output scales or neither.");
            }
            ...
            var accumulatorType = GetScalarDataType(sampling.PackedOutputType.TensorType.DType);
            if (GetVectorLanes(lhs.ElemType).Length != 0 ||
                rhsLanes.Length != 3 ||
                outputLanes.Length != 1 ||
                outputLanes[0] != rhsLanes[0] ||
                accumulatorType != sampling.AccumulatorDataType ||
                logits.ElemType != sampling.OutputDataType ||
                processedLogits.ElemType != DataTypes.Float32 ||
                argMaxState.ElemType != DataTypes.UInt64)
            {
                throw new NotSupportedException(
                    "PyNTT PackedMatMulSamplingPartial requires scalar lhs, K-major RHS lanes " +
                    "[NVector,KPack,KVector], a matching packed accumulator NVector lane, scalar logits, " +
                    "FP32 processed logits, and UInt64 argmax state.");
            }

            PyNTTDimExpression[] lhsScaleShape = Array.Empty<PyNTTDimExpression>();
            PyNTTDimExpression[] rhsScaleShape = Array.Empty<PyNTTDimExpression>();
            if (lhsScale is not null && rhsScale is not null)
            {
                lhsScaleShape = GetBufferActiveShape(lhsScale);
                rhsScaleShape = GetBufferActiveShape(rhsScale);
                if (GetVectorLanes(lhsScale.ElemType).Length != 0 ||
                    GetScalarDataType(lhsScale.ElemType) != accumulatorType ||
                    GetScalarDataType(rhsScale.ElemType) != accumulatorType ||
                    !GetVectorLanes(rhsScale.ElemType).SequenceEqual(outputLanes))
                {
                    throw new NotSupportedException(
                        "PyNTT PackedMatMulSamplingPartial output scales must use a scalar accumulator-typed lhs scale " +
                        "and an accumulator-typed rhs scale with the packed output lanes.");
                }

                ValidateBroadcastable(
                    "PyNTT PackedMatMulSamplingPartial lhs scale",
                    lhsScaleShape,
                    packedOutputShape);
                ValidateBroadcastable(
                    "PyNTT PackedMatMulSamplingPartial rhs scale",
                    rhsScaleShape,
                    packedOutputShape);
            }
                ...
                $"{lhs.Name}, {rhs.Name} -> {logits.Name}")
            {
                ...
                RhsLayout = "k_major",
                RhsKPackLaneCount = rhsLanes[1],
                RhsKVectorLaneCount = rhsLanes[2],
            ...
            PyNTTMicroKernelTemplateModel microKernel)
        {
            ...
            var qInputScale = GetOptionalBuffer(5, "q input scale");
            var kInputScale = GetOptionalBuffer(6, "k input scale");
            var vInputScale = GetOptionalBuffer(7, "v input scale");
            var qWeightScale = GetOptionalBuffer(8, "q weight scale");
            var kWeightScale = GetOptionalBuffer(9, "k weight scale");
            var vWeightScale = GetOptionalBuffer(10, "v weight scale");
            var operandScales = new[]
            {
                qInputScale,
                kInputScale,
                vInputScale,
            ...
            var hasOperandScales = operandScales.All(scale => scale is not null);
            var hasWeightScales = weightScales.All(scale => scale is not null);
            var scaleContractValid = qkv.QuantizationMode switch
            {
                Nncase.IR.Math.MatMulQuantizationMode.None =>
                    operandScales.All(scale => scale is null),
                Nncase.IR.Math.MatMulQuantizationMode.StaticTensor => hasOperandScales,
                Nncase.IR.Math.MatMulQuantizationMode.DynamicTensor =>
                    inputScales.All(scale => scale is null) && hasWeightScales,
                _ => false,
            };
            if (!scaleContractValid)
            {
                throw new NotSupportedException(
                    $"PyNTT PackedQKVParallelLinear scale operands do not match quantization mode " +
                    $"{qkv.QuantizationMode}.");
            }

            if (qkv.QuantizationMode == Nncase.IR.Math.MatMulQuantizationMode.StaticTensor)
            {
                foreach (var scale in operandScales.OfType<TIR.Buffer>())
                {
                    ValidateOperandScaleBuffer("PyNTT PackedQKVParallelLinear", scale);
                }
            }
            ...
            var inputShape = GetBufferShape(input);
            var inputActiveShape = GetBufferActiveShape(input);
            var weightShape = GetBufferShape(weight);
            var qBiasShape = qBias is null ? Array.Empty<PyNTTDimExpression>() : GetBufferActiveShape(qBias);
            var kBiasShape = kBias is null ? Array.Empty<PyNTTDimExpression>() : GetBufferActiveShape(kBias);
            var vBiasShape = vBias is null ? Array.Empty<PyNTTDimExpression>() : GetBufferActiveShape(vBias);
            var qOutputShape = GetBufferActiveShape(qOutput);
            ...
            ValidateRank("PyNTT PackedQKVParallelLinear input active shape", inputActiveShape, inputShape.Length);
            ...
            var (nVectorLaneCount, kPackedLaneCount, kVectorLaneCount) =
                (qkv.RhsLayout, weightLanes.Length) switch
                {
                    (Nncase.IR.NTT.PackedMatMulRhsLayout.KMajor, 3) =>
                        (weightLanes[0], weightLanes[1], weightLanes[2]),
                    (Nncase.IR.NTT.PackedMatMulRhsLayout.NMajorKPacked, 2) =>
                        (qkv.OutputNVectorLaneCount, weightLanes[0], weightLanes[1]),
                    _ => throw new NotSupportedException(
                        "PyNTT fused packed QKV requires KMajor <N,KPack,K> or " +
                        $"NMajorKPacked <KPack,K> weights, got layout={qkv.RhsLayout}, " +
                        $"lanes=[{string.Join(",", weightLanes)}]."),
                };
            if (nVectorLaneCount <= 0)
            {
                throw new NotSupportedException(
                    $"PyNTT fused packed QKV requires a positive output N lane count, got {nVectorLaneCount}.");
            }
            ...
            if (qkv.QuantizationMode == Nncase.IR.Math.MatMulQuantizationMode.DynamicTensor)
            {
                foreach (var (name, scale, output) in new[]
                {
                    ("q", qWeightScale!, qOutput),
                    ("k", kWeightScale!, kOutput),
                    ("v", vWeightScale!, vOutput),
                })
                {
                    var scaleShape = GetBufferActiveShape(scale);
                    var scaleStrides = GetBufferStrides(scale);
                    if (scaleShape.Length != 1 ||
                        !GetVectorLanes(scale.ElemType).SequenceEqual(new[] { nVectorLaneCount }) ||
                        scaleShape[0] != GetBufferActiveShape(output)[^1] ||
                        scaleStrides.Length != 1 ||
                        scaleStrides[0].MinValue != 1 || scaleStrides[0].MaxValue != 1)
                    {
                        throw new NotSupportedException(
                            $"PyNTT dynamic-tensor packed QKV {name} weight scale must be contiguous " +
                            $"[N/{nVectorLaneCount}]<{nVectorLaneCount}> matching its output, got " +
                            $"shape=[{ShapeText(scaleShape)}], dtype={scale.ElemType}, " +
                            $"strides=[{ShapeText(scaleStrides)}].");
                    }
                }
            }
            ...
            ValidateBroadcastable("PyNTT PackedQKVParallelLinear input/q output batch", inputActiveShape[..^2], qOutputShape[..^2]);
            ValidateBroadcastable("PyNTT PackedQKVParallelLinear input/k output batch", inputActiveShape[..^2], kOutputShape[..^2]);
            ValidateBroadcastable("PyNTT PackedQKVParallelLinear input/v output batch", inputActiveShape[..^2], vOutputShape[..^2]);

            var inputScalarType = GetScalarDataType(input.ElemType);
            var weightScalarType = GetScalarDataType(weight.ElemType);
            var outputScalarType = GetScalarDataType(qOutput.ElemType);
            if (qkv.QuantizationMode == Nncase.IR.Math.MatMulQuantizationMode.None &&
                inputScalarType != weightScalarType)
            {
                throw new NotSupportedException($"PyNTT fused packed QKV input/weight dtypes differ: {input.ElemType}/{weight.ElemType}.");
            }

            if (qkv.QuantizationMode != Nncase.IR.Math.MatMulQuantizationMode.None &&
                (inputScalarType != DataTypes.BFloat16 ||
                 weightScalarType != DataTypes.Float8E4M3 ||
                 outputScalarType != DataTypes.BFloat16))
            {
                throw new NotSupportedException(
                    "PyNTT scaled packed QKV currently requires BF16 input/output and E4M3 weights, " +
                    $"got {inputScalarType}/{weightScalarType}/{outputScalarType}.");
            }
            ...
            var totalScalarN = checked(projectionNCapacities.Sum());
            var expectedWeightN = qkv.RhsLayout == Nncase.IR.NTT.PackedMatMulRhsLayout.KMajor
                ? checked(totalScalarN / nVectorLaneCount)
                : totalScalarN;
            var weightN = qkv.RhsLayout == Nncase.IR.NTT.PackedMatMulRhsLayout.KMajor
                ? weightShape[1]
                : weightShape[0];
            if (weightN.MinValue != expectedWeightN || weightN.MaxValue != expectedWeightN)
            {
                throw new NotSupportedException(
                    $"PyNTT fused packed QKV weight N extent {weightN} does not match " +
                    $"the {qkv.RhsLayout} projection capacity {expectedWeightN}.");
            }

            var kAtom = checked(kPackedLaneCount * kVectorLaneCount);
            var weightK = qkv.RhsLayout == Nncase.IR.NTT.PackedMatMulRhsLayout.KMajor
                ? weightShape[0]
                : weightShape[1];
            if (inputShape[^1].MinValue != inputShape[^1].MaxValue ||
                inputActiveShape[^1].MinValue < 0 ||
                inputActiveShape[^1].MaxValue > inputShape[^1].MaxValue ||
                weightK.MinValue != weightK.MaxValue ||
                weightK.MinValue != checked((inputShape[^1].MaxValue + kAtom - 1) / kAtom))
            {
                throw new NotSupportedException(
                    $"PyNTT fused packed QKV weight K extent {weightK} does not match input " +
                    $"storage K {inputShape[^1]} with active K {inputActiveShape[^1]} packed by {kAtom}.");
            }
            ...
            _attrs["rhs_layout"] = qkv.RhsLayout switch
            {
                Nncase.IR.NTT.PackedMatMulRhsLayout.KMajor => "k_major",
                Nncase.IR.NTT.PackedMatMulRhsLayout.NMajorKPacked => "n_major_k_packed",
                _ => throw new NotSupportedException(
                    $"PyNTT fused packed QKV does not support {qkv.RhsLayout}."),
            };
            _attrs["k_pack_lane"] = kPackedLaneCount;
            _attrs["k_lane"] = kVectorLaneCount;
            _attrs["num_heads"] = qkv.NumHeads;
            _attrs["num_kv_heads"] = qkv.NumKvHeads;
            _attrs["has_operand_scales"] =
                qkv.QuantizationMode == Nncase.IR.Math.MatMulQuantizationMode.StaticTensor;
            _attrs["quantization_mode"] = qkv.QuantizationMode.ToString();
            ...
            var usesHostWeightDescriptors =
                ...
                microKernel.Variant is
                    "simt_fma_smem_pipeline" or
                    "simt_fp8_fma_smem_pipeline" or
                    "mma_smem_pipeline" or
                ...
                $"{input.Name}, fused packed {weight.Name} -> packed({qOutput.Name}, {kOutput.Name}, {vOutput.Name})")
            {
                ...
                RhsLayout = qkv.RhsLayout switch
                {
                    Nncase.IR.NTT.PackedMatMulRhsLayout.KMajor => "k_major",
                    Nncase.IR.NTT.PackedMatMulRhsLayout.NMajorKPacked => "n_major_k_packed",
                    _ => throw new NotSupportedException(
                ...
                QInputScale = qInputScale is null ? null : GetBufferScalarPointer(qInputScale),
                KInputScale = kInputScale is null ? null : GetBufferScalarPointer(kInputScale),
                VInputScale = vInputScale is null ? null : GetBufferScalarPointer(vInputScale),
                QWeightScale = qWeightScale is null ? null : GetBufferScalarPointer(qWeightScale),
                KWeightScale = kWeightScale is null ? null : GetBufferScalarPointer(kWeightScale),
                VWeightScale = vWeightScale is null ? null : GetBufferScalarPointer(vWeightScale),
                QuantizationMode = qkv.QuantizationMode switch
                {
                    Nncase.IR.Math.MatMulQuantizationMode.None => "none",
                    Nncase.IR.Math.MatMulQuantizationMode.StaticTensor => "static_tensor",
                    Nncase.IR.Math.MatMulQuantizationMode.DynamicTensor => "dynamic_tensor",
                    _ => throw new NotSupportedException(
                        $"Unsupported PackedQKVParallelLinear quantization mode {qkv.QuantizationMode}."),
                },
        ...
        private void VisitNVFP4MatmulNormStats(
            Nncase.TIR.NTT.NVFP4MatMulNormStats matmul,
            IReadOnlyList<BaseExpr> args,
            PyNTTMicroKernelTemplateModel microKernel)
        {
            if (args.Count != 8 || args[6] is not TIR.Buffer stats)
            {
                throw new NotSupportedException(
                    "PyNTT NVFP4MatMulNormStats codegen expects lhs, packed weight, block scale, " +
                    "input global scale, weight global scale, value output, statistics output, and addend operands.");
            }
        ...
        private void VisitNVFP4Matmul(
            ...
            string context)
        {
            var expectedCount = stats is null ? 7 : 8;
            var addendIndex = expectedCount - 1;
            if (args.Count != expectedCount || args.Take(6).Any(static argument => argument is not TIR.Buffer))
            {
                throw new NotSupportedException(
                    $"{context} codegen expects lhs, packed weight, block scale, input global scale, " +
                    "weight global scale, value output, optional statistics output, and addend operands.");
            }

            var addend = args[addendIndex] switch
            {
                TIR.Buffer buffer => buffer,
                None => null,
                _ => throw new NotSupportedException(
                    $"{context} addend must be a TIR buffer or None."),
            };
            ...
            SetComputeOp(stats is null ? "nvfp4_matmul" : "nvfp4_matmul_norm_stats");
            var lhsShape = GetBufferActiveShape(lhs);
            ...
            if (addend is not null)
            {
                ...
                {
                    throw new NotSupportedException(
                        $"{context} addend must have the same element type and active shape as output, " +
                        $"got addend={addend.ElemType}[{ShapeText(addendShape)}], " +
                        $"output={output.ElemType}[{ShapeText(outputShape)}].");
                }
            }
            ...
            if (stats is not null)
            {
                ...
                {
                    throw new NotSupportedException(
                        $"{context} requires scalar float32 statistics, got {stats.ElemType}.");
                }

                normalizedNormAxis = NormalizeAxis(normAxis, outputShape.Length, context);
                if (normalizedNormAxis != outputShape.Length - 1)
                {
                    throw new NotSupportedException(
                        $"{context} requires normalization over the final logical output axis.");
                }
            ...
            _attrs["dtype"] = GetPyNTTScalarDTypeName(output.ElemType);
            _attrs["shape"] = outputShape;
            _attrs["group_size"] = groupSize;
            _attrs["has_addend"] = addend is not null;
            var helperName = GetNextHelperName(
                stats is null ? "nvfp4_gemv_compute" : "nvfp4_gemv_norm_stats_compute");
            var rhsPackedDescriptor = RegisterHostTensorDescriptor(
                rhsPacked,
                $"{helperName}__rhs_packed_descriptor");
            var rhsScaleDescriptor = RegisterHostTensorDescriptor(
                rhsScale,
                $"{helperName}__rhs_scale_descriptor");
                ...
                $"{lhs.Name}, {rhsPacked.Name}, {rhsScale.Name} -> {output.Name}")
            {
                ...
                RhsScaleDescriptorName = rhsScaleDescriptor.Name,
                Addend = addend is null ? null : GetBufferScalarPointer(addend),
                AddendShape = addendShape,
                AddendStrides = addend is null
                    ? Array.Empty<PyNTTDimExpression>()
        ...
        private void VisitNVFP4MatMulGlu(
            ...
            IReadOnlyList<BaseExpr> args,
            PyNTTMicroKernelTemplateModel microKernel)
            => VisitNVFP4MatMulGluCore(
                matMulGlu.GluType,
                matMulGlu.GroupSize,
                args,
                microKernel,
                inputNormalization: null,
                normalizedInputType: null);

        private void VisitGatherReduceNormApplyNVFP4MatMulGlu(
            Nncase.TIR.NTT.GatherReduceNormApplyNVFP4MatMulGlu fused,
            IReadOnlyList<BaseExpr> args,
            PyNTTMicroKernelTemplateModel microKernel)
        {
            const string context = "PyNTT GatherReduceNormApplyNVFP4MatMulGlu";
            if (args.Count != 13 || args.Any(static argument => argument is not TIR.Buffer))
            {
                throw new NotSupportedException(
                    $"{context} expects partial stats, input, norm scale/bias, gate/up " +
                    "NVFP4 projection operands, and output TIR buffers.");
            }

            var partialStats = (TIR.Buffer)args[0];
            var input = (TIR.Buffer)args[1];
            var normScale = (TIR.Buffer)args[2];
            var normBias = (TIR.Buffer)args[3];
            var (hierarchy, partialAxes, partialStatsAddress) =
                GetGatherReducePartialStatsContext(
                    context,
                    fused.InStatsType,
                    fused.OutStatsType,
                    partialStats);
            if (input.DistributedStorageKind is not (
                    DistributedBufferStorageKind.CompactLocal or
                    DistributedBufferStorageKind.CompactPerOwner))
            {
                throw new NotSupportedException(
                    $"{context} input {input.Name} must use compact distributed storage, got " +
                    $"{input.DistributedStorageKind}.");
            }
            ...
            {
                throw new NotSupportedException(
                    $"{context} input {input.Name} requires distinct per-owner storage.");
            }

            var inputShape = GetBufferGlobalShape(input);
            var normalizedAxis = NormalizeAxis(fused.Axis, inputShape.Length, context);
            if (inputShape.Length != 2 || normalizedAxis != inputShape.Length - 1)
            {
                throw new NotSupportedException(
                    $"{context} currently requires rank-2 activation normalized on its final axis.");
            }

            var inputVectorLanes = GetVectorLanes(input.ElemType);
            var scaleVectorLanes = GetVectorLanes(normScale.ElemType);
            var biasVectorLanes = GetVectorLanes(normBias.ElemType);
            if (!inputVectorLanes.SequenceEqual(scaleVectorLanes) ||
                !inputVectorLanes.SequenceEqual(biasVectorLanes))
            {
                throw new NotSupportedException(
                    $"{context} requires matching input/scale/bias vector lanes, got " +
                    $"input=[{string.Join(',', inputVectorLanes)}], " +
                    $"scale=[{string.Join(',', scaleVectorLanes)}], " +
                    $"bias=[{string.Join(',', biasVectorLanes)}].");
            }

            var inputNormalization = new PyNTTNVFP4InputNormalizationTemplateModel(
                GetPooledByteAddressTemplateModel(inputRef),
                partialStatsAddress,
                GetBufferScalarPointer(normScale),
                GetBufferScalarPointer(normBias),
                GetPyNTTScalarDTypeName(partialStats.ElemType),
                GetScalarTritonDType(partialStats.ElemType),
                GetScalarTritonDType(normScale.ElemType),
                GetScalarTritonDType(normBias.ElemType),
                GetBufferActiveShape(partialStats),
                GetBufferStrides(partialStats),
                GetBufferGlobalShape(normScale),
                GetBufferStrides(normScale),
                GetBufferGlobalShape(normBias),
                GetBufferStrides(normBias),
                scaleVectorLanes,
                biasVectorLanes,
                hierarchy,
                partialAxes,
                ...
                fused.UseMean,
                fused.HasBias);
            var matMulArguments = new BaseExpr[]
            {
                args[1],
                args[4],
                args[5],
            ...
            VisitNVFP4MatMulGluCore(
                ...
                microKernel,
                inputNormalization,
                fused.NormalizedInputType);
        }

        private void VisitNVFP4MatMulGluCore(
            GluType gluType,
            long groupSize,
            IReadOnlyList<BaseExpr> args,
            PyNTTMicroKernelTemplateModel microKernel,
            PyNTTNVFP4InputNormalizationTemplateModel? inputNormalization,
            DistributedType? normalizedInputType)
        {
            ...
            var inputShape = inputNormalization is null
            ...
            _attrs["dtype"] = GetPyNTTScalarDTypeName(output.ElemType);
            _attrs["shape"] = outputShape;
            _attrs["glu_type"] = GetGluTypeName(gluType);
            _attrs["group_size"] = groupSize;
            ...
            var gateWeightScaleDescriptor = RegisterHostTensorDescriptor(
                gateWeightScale,
                $"{helperName}__gate_weight_scale_descriptor");
            var upWeightScaleDescriptor = RegisterHostTensorDescriptor(
                upWeightScale,
                $"{helperName}__up_weight_scale_descriptor");
                ...
                $"{input.Name}, ({gateWeightPacked.Name}, {upWeightPacked.Name}) -> {output.Name}")
            {
                ...
                GateWeightScaleDescriptorName = gateWeightScaleDescriptor.Name,
                UpWeightScaleDescriptorName = upWeightScaleDescriptor.Name,
        ...
        private void ValidateNVFP4ProjectionBuffers(
            ...
            TIR.Buffer output,
            long groupSize,
            DistributedType? normalizedInputType = null)
        {
            if (groupSize != 16)
            {
            ...
            var inputShape = normalizedInputType is not null
                ? GetBufferGlobalShape(input)
                : GetBufferActiveShape(input);
            ...
            var inferredOutput = Nncase.Evaluator.IR.NTT.PackedNVFP4MatMulEvaluator.InferProjectionType(
                ...
                16,
                8,
                normalizedInputType ?? GetNVFP4BufferType(input),
            ...
            PyNTTMicroKernelTemplateModel microKernel)
        {
            if (args.Count != 12 ||
                ...
                args[2] is not TIR.Buffer upWeight)
            {
                throw new NotSupportedException("PyNTT PackedMatMulGlu codegen expects input, packed gate/up weights, optional packed gate/up bias and scale values, and direct or gate/up partial output TIR buffers.");
            }
            ...
            var gateBias = GetOptionalBuffer(3, "gate bias");
            var upBias = GetOptionalBuffer(4, "up bias");
            var gateInputScale = GetOptionalBuffer(5, "gate input scale");
            var upInputScale = GetOptionalBuffer(6, "up input scale");
            var gateWeightScale = GetOptionalBuffer(7, "gate weight scale");
            var upWeightScale = GetOptionalBuffer(8, "up weight scale");
            var gateOutput = GetOptionalBuffer(9, "gate output");
            var upOutput = GetOptionalBuffer(10, "up output");
            var directOutput = GetOptionalBuffer(11, "output");
            var output = matMulGlu.EmitPartialResults ? gateOutput : directOutput;
            if (output is null ||
                (matMulGlu.EmitPartialResults
                    ? upOutput is null || directOutput is not null
                    : gateOutput is not null || upOutput is not null))
            {
                throw new NotSupportedException(
                    "PyNTT PackedMatMulGlu direct mode requires only output, while split-K mode requires only gate/up outputs.");
            }

            if (matMulGlu.EmitPartialResults &&
                (microKernel.Family != "triton.matmul_glu" ||
                 microKernel.Variant != "mma_n_major_block_fp8_smem_pipeline"))
            {
                throw new NotSupportedException(
                    "PyNTT PackedMatMulGlu split-K currently requires the selected N-major block-FP8 MMA pipeline.");
            }
            ...
            var weightScales = new[]
            {
                gateWeightScale,
                upWeightScale,
            };
            var scaleContractValid = matMulGlu.QuantizationMode switch
            {
                Nncase.IR.Math.MatMulQuantizationMode.None =>
                    inputScales.All(scale => scale is null) &&
                    weightScales.All(scale => scale is null) &&
                    matMulGlu.WeightBlockN == 0 && matMulGlu.WeightBlockK == 0,
                Nncase.IR.Math.MatMulQuantizationMode.StaticTensor =>
                    inputScales.All(scale => scale is not null) &&
                    weightScales.All(scale => scale is not null) &&
                    matMulGlu.WeightBlockN == 0 && matMulGlu.WeightBlockK == 0,
                Nncase.IR.Math.MatMulQuantizationMode.DynamicBlock =>
                    inputScales.All(scale => scale is null) &&
                    weightScales.All(scale => scale is not null) &&
                    matMulGlu.WeightBlockN > 0 && matMulGlu.WeightBlockK > 0,
                _ => false,
            ...
            {
                throw new NotSupportedException(
                    $"PyNTT PackedMatMulGlu scale operands do not match quantization mode " +
                    $"{matMulGlu.QuantizationMode} and block " +
                    $"[{matMulGlu.WeightBlockN}, {matMulGlu.WeightBlockK}].");
            }

            if (matMulGlu.QuantizationMode == Nncase.IR.Math.MatMulQuantizationMode.StaticTensor)
            {
                foreach (var scale in inputScales.Concat(weightScales).OfType<TIR.Buffer>())
                {
                    ValidateOperandScaleBuffer("PyNTT PackedMatMulGlu", scale);
                }
            }
            else if (matMulGlu.QuantizationMode == Nncase.IR.Math.MatMulQuantizationMode.DynamicBlock)
            {
                foreach (var (name, scale) in new[]
                {
                    ("gate weight", gateWeightScale!),
                    ("up weight", upWeightScale!),
                })
                {
                    var scaleShape = GetBufferActiveShape(scale);
                    ValidateRank($"PyNTT PackedMatMulGlu {name} block scale", scaleShape, 2);
                    if (!GetScalarDataType(scale.ElemType).IsFloat())
                    {
                        throw new NotSupportedException(
                            $"PyNTT PackedMatMulGlu {name} block scale must be floating, got {scale.ElemType}.");
                    }
                }
            }
            ...
            var upBiasShape = upBias is null ? Array.Empty<PyNTTDimExpression>() : GetBufferActiveShape(upBias);
            var outputShape = GetBufferActiveShape(output);
            var upOutputShape = upOutput is null
                ? Array.Empty<PyNTTDimExpression>()
                : GetBufferActiveShape(upOutput);
            ...
            if (upOutput is not null)
            {
                ValidateSameShape(
                    "PyNTT PackedMatMulGlu split-K outputs",
                    outputShape,
                    upOutputShape);
                if (output.ElemType != upOutput.ElemType)
                {
                    throw new NotSupportedException(
                        $"PyNTT PackedMatMulGlu split-K outputs must have the same dtype, got {output.ElemType}/{upOutput.ElemType}.");
                }
            }
            ...
            if (matMulGlu.EmitPartialResults && (gateBias is not null || upBias is not null))
            {
                throw new NotSupportedException(
                    "PyNTT PackedMatMulGlu split-K cannot apply bias before materialization.");
            }

            ValidatePackedMatMulGluProjectionShape("gate", matMulGlu.RhsLayout, gateWeightLanes, outputLanes, inputShape, gateWeightShape, outputShape);
            ValidatePackedMatMulGluProjectionShape("up", matMulGlu.RhsLayout, upWeightLanes, outputLanes, inputShape, upWeightShape, outputShape);
            ValidateMatMulGluBiasShape("gate", gateBiasShape, outputShape);
            ValidateMatMulGluBiasShape("up", upBiasShape, outputShape);
            ...
            var inputScalarType = GetScalarDataType(input.ElemType);
            var gateWeightScalarType = GetScalarDataType(gateWeight.ElemType);
            var upWeightScalarType = GetScalarDataType(upWeight.ElemType);
            var outputScalarType = GetScalarDataType(output.ElemType);
            if (matMulGlu.QuantizationMode == Nncase.IR.Math.MatMulQuantizationMode.None &&
                (inputScalarType != gateWeightScalarType ||
                 gateWeightScalarType != upWeightScalarType))
            {
            ...
            if (matMulGlu.QuantizationMode != Nncase.IR.Math.MatMulQuantizationMode.None &&
                (inputScalarType != DataTypes.BFloat16 ||
                 gateWeightScalarType != DataTypes.Float8E4M3 ||
                 upWeightScalarType != DataTypes.Float8E4M3 ||
                 outputScalarType != DataTypes.BFloat16))
            {
                throw new NotSupportedException(
                    "PyNTT scaled PackedMatMulGlu currently requires BF16 input/output and E4M3 weights, " +
                    $"got input={inputScalarType}, gate={gateWeightScalarType}, up={upWeightScalarType}, output={outputScalarType}.");
            }
            ...
            var nPackedLaneCount = matMulGlu.RhsLayout == Nncase.IR.NTT.PackedMatMulRhsLayout.NMajor
                ? gateWeightLanes[0]
                : 1;
            var nVectorLaneCount = matMulGlu.RhsLayout switch
            {
                Nncase.IR.NTT.PackedMatMulRhsLayout.NMajor => gateWeightLanes[1],
                Nncase.IR.NTT.PackedMatMulRhsLayout.KMajor => gateWeightLanes[0],
                Nncase.IR.NTT.PackedMatMulRhsLayout.NMajorKPacked => outputLanes[0],
                _ => throw new NotSupportedException(),
            };
            var kPackedLaneCount = matMulGlu.RhsLayout switch
            {
                Nncase.IR.NTT.PackedMatMulRhsLayout.KMajor => gateWeightLanes[1],
                Nncase.IR.NTT.PackedMatMulRhsLayout.NMajorKPacked => gateWeightLanes[0],
                _ => 1,
            };
            var kVectorLaneCount = matMulGlu.RhsLayout switch
            {
                Nncase.IR.NTT.PackedMatMulRhsLayout.KMajor => gateWeightLanes[2],
                Nncase.IR.NTT.PackedMatMulRhsLayout.NMajorKPacked => gateWeightLanes[1],
                _ => 1,
            };
            ...
            _attrs["glu_type"] = GetGluTypeName(matMulGlu.GluType);
            _attrs["has_operand_scales"] =
                matMulGlu.QuantizationMode == Nncase.IR.Math.MatMulQuantizationMode.StaticTensor;
            _attrs["quantization_mode"] = matMulGlu.QuantizationMode.ToString();
            ...
            var usesHostWeightDescriptors =
                ...
                microKernel.Variant is "simt_fma_smem_pipeline" or
                    "simt_fp8_fma_smem_pipeline" or
                    "simt_block_fp8_fma_smem_pipeline" or
                    "mma_block_fp8_smem_pipeline" or
                ...
                $"{input.Name}, packed({gateWeight.Name}, {upWeight.Name}) -> packed({output.Name})")
            {
                ...
                KPackLaneCount = kPackedLaneCount,
                KVectorLaneCount = kVectorLaneCount,
                GateInputScale = gateInputScale is null ? null : GetBufferScalarPointer(gateInputScale),
                UpInputScale = upInputScale is null ? null : GetBufferScalarPointer(upInputScale),
                GateWeightScale = gateWeightScale is null ? null : GetBufferScalarPointer(gateWeightScale),
                UpWeightScale = upWeightScale is null ? null : GetBufferScalarPointer(upWeightScale),
                QuantizationMode = matMulGlu.QuantizationMode switch
                {
                    Nncase.IR.Math.MatMulQuantizationMode.None => "none",
                    Nncase.IR.Math.MatMulQuantizationMode.StaticTensor => "static_tensor",
                    Nncase.IR.Math.MatMulQuantizationMode.DynamicBlock => "dynamic_block",
                    _ => throw new NotSupportedException(
                        $"Unsupported PackedMatMulGlu quantization mode {matMulGlu.QuantizationMode}."),
                },
                WeightBlockN = checked((int)matMulGlu.WeightBlockN),
                WeightBlockK = checked((int)matMulGlu.WeightBlockK),
                GateWeightScaleShape = gateWeightScale is null
                    ? Array.Empty<PyNTTDimExpression>()
                    : GetBufferActiveShape(gateWeightScale),
                UpWeightScaleShape = upWeightScale is null
                    ? Array.Empty<PyNTTDimExpression>()
                    : GetBufferActiveShape(upWeightScale),
                GateWeightScaleStrides = gateWeightScale is null
                    ? Array.Empty<PyNTTDimExpression>()
                    : GetBufferStrides(gateWeightScale),
                UpWeightScaleStrides = upWeightScale is null
                    ? Array.Empty<PyNTTDimExpression>()
                    : GetBufferStrides(upWeightScale),
                InputGlobalOffsets = GetBufferGlobalOffsets(input),
                OutputGlobalOffsets = GetBufferGlobalOffsets(output),
                ...
                EmitPartialResults = matMulGlu.EmitPartialResults,
                UpOutput = upOutput is null ? null : GetBufferScalarPointer(upOutput),
                UpOutputShape = upOutputShape,
                UpOutputStrides = upOutput is null
                    ? Array.Empty<PyNTTDimExpression>()
                    : GetBufferStrides(upOutput),
                UpOutputGlobalOffsets = upOutput is null
                    ? Array.Empty<PyNTTDimExpression>()
                    : GetBufferGlobalOffsets(upOutput),
            };
            ...
            IReadOnlyList<int> outputLanes)
        {
            ...
            if (weightLanes.Count != expectedWeightLaneCount)
            {
                var expected = rhsLayout == Nncase.IR.NTT.PackedMatMulRhsLayout.NMajor
                    ? "[NPack,NVector]"
                    : rhsLayout == Nncase.IR.NTT.PackedMatMulRhsLayout.KMajor
                        ? "[NVector,KPack,KVector]"
                        : "[KPack,KVector]";
            ...
            var outputMatches = rhsLayout switch
            {
                Nncase.IR.NTT.PackedMatMulRhsLayout.NMajor => weightLanes.SequenceEqual(outputLanes),
                Nncase.IR.NTT.PackedMatMulRhsLayout.KMajor => weightLanes.Take(1).SequenceEqual(outputLanes),
                Nncase.IR.NTT.PackedMatMulRhsLayout.NMajorKPacked => outputLanes.Count == 1,
                _ => false,
            };
            if (!outputMatches)
            {
                throw new NotSupportedException($"PyNTT PackedMatMulGlu {name} {rhsLayout} output lanes are incompatible with weight lanes [{string.Join(",", weightLanes)}], got [{string.Join(",", outputLanes)}].");
            }
        }
            ...
            PyNTTDimExpression[] outputShape)
        {
            ...
            var weightK = rhsLayout switch
            {
                ...
                Nncase.IR.NTT.PackedMatMulRhsLayout.NMajorKPacked => MultiplyDim(
                    weightShape[^1],
                    checked(weightLanes[0] * weightLanes[1])),
                _ => throw new NotSupportedException($"PyNTT PackedMatMulGlu does not support RHS layout {rhsLayout}."),
            };
            var weightN = rhsLayout switch
            {
                Nncase.IR.NTT.PackedMatMulRhsLayout.NMajor => weightShape[^2],
                Nncase.IR.NTT.PackedMatMulRhsLayout.KMajor => weightShape[^1],
                Nncase.IR.NTT.PackedMatMulRhsLayout.NMajorKPacked => weightShape[^2],
                _ => throw new NotSupportedException(),
            };
            var outputN = rhsLayout == Nncase.IR.NTT.PackedMatMulRhsLayout.NMajorKPacked
                ? MultiplyDim(outputShape[^1], outputLanes[0])
                : outputShape[^1];
            if (!SameDim(inputK, weightK) ||
                !SameDim(outputShape[^2], inputM) ||
                !SameDim(outputN, weightN))
            {
                var expectedWeight = rhsLayout == Nncase.IR.NTT.PackedMatMulRhsLayout.NMajor
                    ...
                    : rhsLayout == Nncase.IR.NTT.PackedMatMulRhsLayout.KMajor
                        ...
                        : "weight=[...,N,K]<KPack,KVector>, output=[...,M,N]<NVector>";
            ...
            string helperName)
        {
            ...
            var consumerEndpoints = stage.PipeStages
                .Select(pipeStage => pipeStage.ReaderName)
                .Concat(stage.ConsumerSharedWorkspaceNames)
                .ToArray();
            if (microKernel.AuxiliaryConsumer is { } auxiliaryConsumer)
            {
                var binding = _activePipelineRegion.BindAuxiliaryConsumer(
                    stage,
                    auxiliaryConsumer);
                WriteLine(
                    BuildHelperRoleCall(
                        helperName,
                        "consumer",
                        consumerEndpoints.Concat(binding.PrimaryConsumerEndpointNames),
                        recordMetadata: true));
                _activePipelineRegion.AppendAuxiliaryConsumerLine(
                    BuildHelperRoleCall(
                        helperName,
                        "auxiliary_consumer",
                        binding.AuxiliaryConsumerEndpointNames,
                        recordMetadata: false));
                foreach (var endpoint in binding.DrainEndpointNames)
                {
                    _activePipelineRegion.AppendAuxiliaryConsumerLine(
        ...
        private static string GetMicroKernelTemplatePath(PyNTTMicroKernelTemplateModel microKernel)
            => (microKernel.Family, microKernel.Variant) switch
            {
                ...
                ("triton.qkv_parallel_linear", "mma_fp8_smem_pipeline") => "triton/kernels/qkv_parallel_linear/mma_fp8_smem_pipeline.py.jinja",
                ...
                ("triton.matmul_glu", "mma_block_fp8_smem_pipeline") => "triton/kernels/matmul_glu/mma_block_fp8_smem_pipeline.py.jinja",
                ("triton.matmul_glu", "mma_n_major_block_fp8_smem_pipeline") => "triton/kernels/matmul_glu/mma_n_major_block_fp8_smem_pipeline.py.jinja",
            ...
            IReadOnlyList<BaseExpr> args)
        {
            ...
            var reshard = BuildGatherReduceScatterTemplateModel(
                ...
                op.OutType,
                context);
            ValidateGatherReduceAddNormValueBuffers(
                context,
                collective,
                addend,
                valueOutput,
            ...
            string context)
        {
            ...
            WriteHelperTemplate(
                ...
                new PyNTTGatherReduceAddNormTemplateModel(
                    ...
                    normStats,
                    GetPooledByteAddressTemplateModel(
                        ResolveByteAddressedBufferRef(statsBuffer)),
                    normApply,
                    context));
            WriteHelperInvocation(helperName);
        }
        ...
        {
            var collectiveShape = GetBufferGlobalShape(collective);
            var globalOutputShape = GetDistributedGlobalShape(outputType, $"{context} output");
            var localOutputType = DistributedUtility.GetDividedTensorType(
                outputType,
                DistributedUtility.DivideFlags.MaxShape);
            var localOutputShape = GetRankedShapeDimensions(
                    localOutputType.Shape,
                    $"{context} local output")
                .Select(GetDimensionExpression)
                .ToArray();
            ValidateSameShape($"{context} collective/output", collectiveShape, globalOutputShape);
            ValidateSameShape($"{context} addend/output", GetBufferActiveShape(addend), localOutputShape);
            ValidateSameShape($"{context} value/output", GetBufferActiveShape(valueOutput), localOutputShape);
        ...
        private (int[] Hierarchy, int[] PartialAxes, PyNTTPooledByteAddressTemplateModel Address)
            GetGatherReducePartialStatsContext(
                string context,
        ...
        {
            if (!inStatsType.Placement.Equals(outStatsType.Placement) ||
                ...
                partialAxes.Any(axis => axis < 0 || axis >= hierarchy.Length))
            {
                throw new NotSupportedException(
                    $"{context} has invalid partial axes [{string.Join(",", partialAxes)}] " +
                    $"for hierarchy [{string.Join(",", hierarchy)}].");
            }
        ...
        private PyNTTNormApplyTemplateModel BuildNormApplyTemplateModel(
            ...
            TIR.Buffer output,
            string context,
            DistributedType? semanticOutputType = null)
        {
            var inputShape = GetBufferActiveShape(input);
            var inputGlobalShape = GetBufferGlobalShape(input);
            var outputGlobalShape = GetBufferGlobalShape(output);
            ...
            var resultDataType = semanticOutputType?.TensorType.DType ?? output.ElemType;
            var resultVectorLaneCount = GetSingleVectorLaneCount(resultDataType, $"{context} semantic output");
            if (statsVectorLaneCount != 1)
            {
                throw new NotSupportedException($"{context} expects scalar stats buffer dtype.");
            }

            if (new[] { inputVectorLaneCount, scaleVectorLaneCount, biasVectorLaneCount, resultVectorLaneCount }.Distinct().Count() != 1)
            {
                throw new NotSupportedException($"{context} expects matching input/scale/bias/semantic-output vector lanes, got input={inputVectorLaneCount}, scale={scaleVectorLaneCount}, bias={biasVectorLaneCount}, semantic_output={resultVectorLaneCount}.");
            }

            var requiresCooperativeOwnerDirect =
                GetScalarDataType(resultDataType) != GetScalarDataType(output.ElemType) ||
                resultVectorLaneCount != outputVectorLaneCount;
            if (requiresCooperativeOwnerDirect && semanticOutputType is null)
            {
                throw new NotSupportedException(
                    $"{context} cannot repack or cast its output without a semantic distributed output contract.");
            }

            var semanticOutputShape = semanticOutputType is null
                ? outputShape
                : GetRankedShapeDimensions(
                        DistributedUtility.GetDividedTensorType(
                            semanticOutputType,
                            DistributedUtility.DivideFlags.MaxShape).Shape,
                        $"{context} semantic local output")
                    .Select(GetDimensionExpression)
                    .ToArray();
            var semanticOutputGlobalShape = semanticOutputType is null
                ? outputGlobalShape
                : GetDistributedGlobalShape(semanticOutputType, $"{context} semantic global output");
            var normalizedAxis = NormalizeAxis(axis, semanticOutputShape.Length, context);
            if (inputVectorLaneCount != 1 && normalizedAxis > semanticOutputShape.Length - 1)
            {
                throw new NotSupportedException($"{context} vectorized axis must be inside the normalized dimensions.");
            }

            var logicalInputShape = GetLogicalVectorShape(inputShape, inputVectorLaneCount);
            var logicalInputGlobalShape = GetLogicalVectorShape(inputGlobalShape, inputVectorLaneCount);
            var logicalStatsShape = GetLogicalVectorShape(statsShape, statsVectorLaneCount);
            var logicalScaleShape = GetLogicalVectorShape(scaleShape, scaleVectorLaneCount);
            var logicalBiasShape = GetLogicalVectorShape(biasShape, biasVectorLaneCount);
            var logicalOutputGlobalShape = GetLogicalVectorShape(outputGlobalShape, outputVectorLaneCount);
            var logicalSemanticOutputShape = GetLogicalVectorShape(semanticOutputShape, resultVectorLaneCount);
            var logicalSemanticOutputGlobalShape = GetLogicalVectorShape(semanticOutputGlobalShape, resultVectorLaneCount);
            ValidateSameShape($"{context} logical input/semantic output", logicalInputGlobalShape, logicalSemanticOutputGlobalShape);
            ValidateSameShape($"{context} logical physical/semantic output", logicalOutputGlobalShape, logicalSemanticOutputGlobalShape);
            var inputUsesOutputGlobalCoordinates =
                logicalInputShape.Length != logicalSemanticOutputShape.Length ||
                logicalInputShape.Zip(logicalSemanticOutputShape).Any(pair => !SameDim(pair.First, pair.Second));
            if (inputUsesOutputGlobalCoordinates)
            {
                ValidateSameShape($"{context} replicated input", logicalInputShape, logicalInputGlobalShape);
            }

            ValidateNormStatsShape($"{context} stats", logicalSemanticOutputShape, logicalStatsShape, normalizedAxis, useMean);
            ValidateLayerNormShape($"{context} scale", logicalScaleShape, logicalSemanticOutputShape, normalizedAxis);
            ValidateLayerNormShape($"{context} bias", logicalBiasShape, logicalSemanticOutputShape, normalizedAxis);
            return new PyNTTNormApplyTemplateModel(
                ...
                GetScalarTritonDType(bias.ElemType),
                GetScalarTritonDType(output.ElemType),
                GetPyNTTScalarDTypeName(resultDataType),
                GetScalarTritonDType(resultDataType),
                inputShape,
                inputGlobalShape,
                statsShape,
                scaleShape,
                ...
                GetBufferStrides(bias),
                GetBufferStrides(output),
                inputUsesOutputGlobalCoordinates,
                inputVectorLaneCount,
                ...
                epsilon,
                useMean,
                requiresCooperativeOwnerDirect,
            ...
            IReadOnlyList<BaseExpr> args)
        {
            ...
            BuildQKVRoPEWithCache(
                ...
                fused.QKVLayout,
                fused.AttentionLayout,
                BuildSumPartialInputTemplateModel(qPartial, fused.QInType, "GatherReduceQKVRoPEWithCache Q"),
                BuildSumPartialInputTemplateModel(kPartial, fused.KInType, "GatherReduceQKVRoPEWithCache K"),
                BuildSumPartialInputTemplateModel(vPartial, fused.VInType, "GatherReduceQKVRoPEWithCache V"),
        ...
        private PyNTTSumPartialInputTemplateModel BuildSumPartialInputTemplateModel(
            TIR.Buffer input,
            DistributedType inType,
            string context)
        {
            ...
            {
                throw new NotSupportedException(
                    $"PyNTT {context} input must carry a non-empty Sum partial reduction.");
            }
            ...
            {
                throw new NotSupportedException(
                    $"PyNTT {context} input must use compact per-owner storage.");
            }
            ...
            {
                throw new NotSupportedException(
                    $"PyNTT {context} input requires distinct per-owner storage.");
            }

            return new PyNTTSumPartialInputTemplateModel(
        ...
        private void VisitPagedAttention(Nncase.TIR.NTT.PagedAttention pagedAttention, IReadOnlyList<BaseExpr> args)
        {
            ...
            {
                throw new NotSupportedException("PyNTT PagedAttention codegen expects query, kv-cache, extra, scale, layer id, optional output gate, and output buffers.");
            }

            var outputGate = args[5] switch
            {
                TIR.Buffer buffer => buffer,
                None => null,
                _ => throw new NotSupportedException(
                    $"PyNTT PagedAttention output gate must be a TIR buffer or None, got {args[5].GetType().Name}."),
            };
            ValidatePagedAttentionOutputGate(outputGate, output, "PyNTT PagedAttention");
            ...
            var templateModel = new PyNTTPagedAttentionTemplateModel(
                    helperName,
            ...
            {
                OutputGate = outputGate is null ? null : GetBufferScalarPointer(outputGate),
                HasOutputGate = outputGate is not null,
                OutputGateStrides = outputGate is null
                    ? Array.Empty<PyNTTDimExpression>()
                    : GetBufferStrides(outputGate),
            };
            WriteHelperTemplate(
        ...
        private static void ValidatePagedAttentionOutputGate(
            TIR.Buffer? outputGate,
            TIR.Buffer output,
            string context)
        {
            if (outputGate is not null &&
                (outputGate.ElemType != output.ElemType ||
                 !outputGate.Dimensions.SequenceEqual(output.Dimensions) ||
                 outputGate.DistributedType != output.DistributedType))
            {
                throw new NotSupportedException(
                    $"{context} output gate must have exactly the output buffer type, got " +
                    $"gate={outputGate.CheckedType}, output={output.CheckedType}.");
            }
        }
            ...
            PyNTTMicroKernelTemplateModel microKernel)
        {
            ...
            var outputGate = args[8] switch
            {
                TIR.Buffer buffer => buffer,
                None => null,
                _ => throw new NotSupportedException(
                    $"PyNTT PagedAttentionPartial output gate must be a TIR buffer or None, got {args[8].GetType().Name}."),
            };
            var output = args[9] switch
            {
                TIR.Buffer buffer => buffer,
            ...
            if (outputGate is not null && output is null)
            {
                throw new NotSupportedException(
                    "PyNTT state-only PagedAttentionPartial cannot consume an output gate.");
            }

            if (output is not null)
            {
                ValidatePagedAttentionOutputGate(
                    outputGate,
                    output,
            ...
            Action<string, PyNTTPagedAttentionMergeTemplateModel>? emit = null)
        {
            ...
            {
                throw new NotSupportedException(
                    "PyNTT PagedAttentionMerge codegen expects three state buffers, an optional output gate, and one output buffer.");
            }

            var outputGate = args[3] switch
            {
                TIR.Buffer buffer => buffer,
                None => null,
                _ => throw new NotSupportedException(
                    $"PyNTT PagedAttentionMerge output gate must be a TIR buffer or None, got {args[3].GetType().Name}."),
            };
            ValidatePagedAttentionOutputGate(outputGate, output, "PyNTT PagedAttentionMerge");
            ...
            PyNTTMicroKernelTemplateModel microKernel)
        {
            ...
            VisitPagedAttentionMerge(
                new Nncase.TIR.NTT.PagedAttentionMerge(
                    ...
                    fused.SplitCount),
                new BaseExpr[] { maxState, sumState, accState, None.Default, mergeOutputLayout },
        ...
        private void VisitGatedDeltaNetConvolution(
            Nncase.TIR.NTT.GatedDeltaNetConvolution convolution,
            IReadOnlyList<BaseExpr> args,
            PyNTTMicroKernelTemplateModel microKernel)
        {
            if (args.Count != 5 ||
                args[0] is not TIR.Buffer qkv ||
                args[2] is not TIR.Buffer convWeight ||
                args[3] is not TIR.Buffer qkvOutput)
            {
                throw new NotSupportedException(
                    "PyNTT GatedDeltaNetConvolution expects QKV, state, convolution weight, " +
                    "QKV output, and layer-id operands.");
            }

            var stateConfig = GetGatedDeltaNetStateConfig(args[1], "PyNTT GatedDeltaNetConvolution");
            ...
            var convWeightLanes = GetVectorLanes(convWeight.ElemType);
            var qkvOutputLanes = GetVectorLanes(qkvOutput.ElemType);
            var expectedLanes = GetGatedDeltaNetStateAxisLanes(
                stateConfig,
                GatedDeltaNetStateKind.Convolution,
                GatedDeltaNetStateDimKind.ConvChannels);
            if (!qkvLanes.SequenceEqual(expectedLanes) ||
                !convWeightLanes.SequenceEqual(expectedLanes) ||
                !qkvOutputLanes.SequenceEqual(expectedLanes))
            {
                throw new NotSupportedException(
                    $"PyNTT GatedDeltaNetConvolution QKV/weight/output lanes " +
                    $"[{string.Join(',', qkvLanes)}]/[{string.Join(',', convWeightLanes)}]/" +
                    $"[{string.Join(',', qkvOutputLanes)}] must match " +
                    $"the convolution-state channel lanes [{string.Join(',', expectedLanes)}].");
            }

            var laneCount = GetVectorLaneProduct(expectedLanes);
            var qkvPartial = qkv.DistributedType?.Partial is null
                ? null
                : BuildSumPartialInputTemplateModel(
                    qkv,
                    qkv.DistributedType,
                    "GatedDeltaNetConvolution QKV");
            var globalConvDim = checked((int)stateConfig.GetDimension(
                GatedDeltaNetStateDimKind.ConvChannels));
            var localPackedConvDim = RequireFixedDim(
                GetBufferShape(qkvOutput)[1],
                "PyNTT GatedDeltaNetConvolution local packed convolution-channel capacity");
            var localConvDim = checked(localPackedConvDim * laneCount);
            var activeLocalPackedConvDim = GetBufferActiveShape(qkvOutput)[1];
            if (activeLocalPackedConvDim.MinValue is not { } activeConvMin ||
                activeLocalPackedConvDim.MaxValue is not { } activeConvMax ||
                activeConvMin < 0 ||
                activeConvMax > localPackedConvDim)
            {
                throw new NotSupportedException(
                    "PyNTT GatedDeltaNetConvolution active local channel extent " +
                    $"must be bounded by [0, {localPackedConvDim}], got " +
                    $"{activeLocalPackedConvDim.TritonExpression} with range " +
                    $"[{activeLocalPackedConvDim.MinValue}, {activeLocalPackedConvDim.MaxValue}].");
            }

            if (qkvPartial is null)
            {
                ValidateFixedShape(GetBufferShape(qkv), new long[] { 1, localPackedConvDim }, "PyNTT GatedDeltaNetConvolution QKV");
            }
            else
            {
                ValidateFixedShape(
                    GetBufferGlobalShape(qkv),
                    new long[]
                    {
                        1,
                        stateConfig.GetDimension(GatedDeltaNetStateDimKind.ConvChannels) / laneCount,
                    },
                    "PyNTT GatedDeltaNetConvolution partial QKV global shape");
            }

            var localPackedWeightConvDim = RequireFixedDim(
                GetBufferShape(convWeight)[0],
                "PyNTT GatedDeltaNetConvolution local packed convolution-weight channel capacity");
            var localWeightConvDim = checked(localPackedWeightConvDim * laneCount);
            if (localWeightConvDim <= 0 || localWeightConvDim > localConvDim)
            {
                throw new NotSupportedException(
                    $"PyNTT GatedDeltaNetConvolution local convolution-weight channel extent " +
                    $"must be in [1, {localConvDim}], got {localWeightConvDim}.");
            }

            var qkvGlobalShape = GetBufferGlobalShape(qkv);
            var weightGlobalShape = GetBufferGlobalShape(convWeight);
            var outputGlobalShape = GetBufferGlobalShape(qkvOutput);
            if (RequireFixedDim(
                    qkvGlobalShape[1],
                    "PyNTT GatedDeltaNetConvolution global packed QKV channels") * laneCount != globalConvDim ||
                RequireFixedDim(
                    weightGlobalShape[0],
                    "PyNTT GatedDeltaNetConvolution global packed weight channels") * laneCount != globalConvDim ||
                RequireFixedDim(
                    outputGlobalShape[1],
                    "PyNTT GatedDeltaNetConvolution global packed output channels") * laneCount != globalConvDim)
            {
                throw new NotSupportedException(
                    $"PyNTT GatedDeltaNetConvolution operands must describe the same " +
                    $"global scalar channel domain {globalConvDim}.");
            }

            ValidateFixedShape(GetBufferShape(convWeight), new long[] { localPackedWeightConvDim, convolution.ConvKernelSize }, "PyNTT GatedDeltaNetConvolution convolution weight");
            ValidateFixedShape(GetBufferShape(qkvOutput), new long[] { 1, localPackedConvDim }, "PyNTT GatedDeltaNetConvolution QKV output");
            if (stateConfig.ConvKernelSize != convolution.ConvKernelSize)
            {
                throw new NotSupportedException(
                    $"PyNTT GatedDeltaNetConvolution state convolution kernel " +
                    $"{stateConfig.ConvKernelSize} does not match selected kernel " +
                    $"{convolution.ConvKernelSize}.");
            }

            var activationType = GetScalarDataType(qkv.ElemType);
            if (activationType != stateConfig.ActivationPrimType ||
                GetScalarDataType(convWeight.ElemType) != activationType ||
                GetScalarDataType(qkvOutput.ElemType) != activationType)
            {
                throw new NotSupportedException(
                    "PyNTT GatedDeltaNetConvolution QKV, convolution weight, state, and output " +
                    "must use the configured activation element type.");
            }

            SetComputeOp("gated_delta_net_convolution");
            _attrs["op"] = "gated_delta_net_convolution";
            _attrs["dtype"] = GetPyNTTDTypeName(qkvOutput.ElemType);
            _attrs["shape"] = GetBufferActiveShape(qkvOutput);
            var helperName = GetNextHelperName("gated_delta_net_convolution");
            var templateModel = new PyNTTGatedDeltaNetConvolutionTemplateModel(
                    helperName,
                    ...
                    GetDimensionExpression(
                        args[4],
                        "PyNTT GatedDeltaNetConvolution layer id").TritonExpression,
                    checked((int)convolution.ConvKernelSize),
                    globalConvDim,
                    checked((int)localConvDim),
                    checked((int)localWeightConvDim),
                    $"({activeLocalPackedConvDim.TritonExpression}) * {laneCount}",
                    laneCount,
                    microKernel,
                    $"({qkv.Name}, gated_delta_net_state) -> {qkvOutput.Name}");
            WriteSelectedMicroKernelHelper(microKernel, templateModel, helperName);
            MarkStoredOutput(qkvOutput, "PyNTT GatedDeltaNetConvolution");
        }
        ...
        {
            if (args.Count != 12 ||
                args[1] is not TIR.Buffer qkv ||
                args[2] is not TIR.Buffer z ||
                args[3] is not TIR.Buffer projectionInput ||
                ...
                args[9] is not TIR.Buffer coreScratch ||
                args[10] is not TIR.Buffer gatedOutput)
            {
                throw new NotSupportedException(
                    "PyNTT GatedDeltaNetRecurrentCore expects state, projected QKV/Z, " +
                    "projection input, head-major B/A weights, A_log, dt-bias, norm-weight, " +
                    "gated-output, and layer-id operands.");
            }

            var stateConfig = GetGatedDeltaNetStateConfig(args[0], "PyNTT GatedDeltaNetRecurrentCore");
            var stateLayout = AnalyzeGatedDeltaNetStateLayout(
                stateConfig,
            ...
            var zLanes = GetVectorLanes(z.ElemType);
            var expectedZLanes = GetVectorLanes(stateConfig.ActivationType);
            if (!zLanes.SequenceEqual(expectedZLanes))
            {
                throw new NotSupportedException(
                    $"PyNTT GatedDeltaNetRecurrentCore Z lanes [{string.Join(',', zLanes)}] " +
                    $"must match the configured activation lanes [{string.Join(',', expectedZLanes)}].");
            }

            var zLaneCount = GetVectorLaneProduct(zLanes);
            var projectionInputLanes = GetVectorLanes(projectionInput.ElemType);
            var projectionInputLaneCount = GetVectorLaneProduct(projectionInputLanes);
            ...
            var hiddenSize = checked(
                RequireFixedDim(
                    GetBufferShape(projectionInput)[1],
                    "PyNTT GatedDeltaNetRecurrentCore projection input hidden") *
                projectionInputLaneCount);
            var localValueCapacity = RequireFixedDim(
                GetBufferShape(gatedOutput)[1],
                "PyNTT GatedDeltaNetRecurrentCore local value capacity");
            if (!microKernel.Parameters.TryGetValue("state_value_tile", out var stateValueTile) ||
                stateValueTile <= 0 ||
                localValueCapacity % stateValueTile != 0)
            {
                throw new NotSupportedException(
                    "PyNTT GatedDeltaNetRecurrentCore local output must contain complete state value tiles.");
            }

            var activeLocalValueCapacity = GetBufferActiveShape(gatedOutput)[1];
            if (activeLocalValueCapacity.MinValue is not { } activeValueMin ||
                activeLocalValueCapacity.MaxValue is not { } activeValueMax ||
                activeValueMin < 0 ||
                activeValueMax > localValueCapacity)
            {
                throw new NotSupportedException(
                    "PyNTT GatedDeltaNetRecurrentCore active local value extent " +
                    $"must be bounded by [0, {localValueCapacity}], got " +
                    $"{activeLocalValueCapacity.TritonExpression} with range " +
                    $"[{activeLocalValueCapacity.MinValue}, {activeLocalValueCapacity.MaxValue}].");
            }

            ValidateFixedShape(GetBufferShape(qkv), new long[] { 1, convDim / qkvLaneCount }, "PyNTT GatedDeltaNetRecurrentCore QKV");
            var zPartial = z.DistributedType?.Partial is null
                ? null
                : BuildSumPartialInputTemplateModel(
                    z,
                    z.DistributedType,
            ...
            if (zPartial is null)
            {
                ValidateFixedShape(GetBufferShape(z), new long[] { 1, localValueCapacity / zLaneCount }, "PyNTT GatedDeltaNetRecurrentCore Z");
            }
            else
            {
                ValidateFixedShape(
                    GetBufferGlobalShape(z),
                    new long[] { 1, valueDim / zLaneCount },
                    "PyNTT GatedDeltaNetRecurrentCore partial Z global shape");
            }

            ValidateFixedShape(GetBufferShape(projectionInput), new long[] { 1, hiddenSize / projectionInputLaneCount }, "PyNTT GatedDeltaNetRecurrentCore projection input");
            ValidateFixedShape(GetBufferShape(bWeight), new long[] { recurrentCore.NumValueHeads, hiddenSize }, "PyNTT GatedDeltaNetRecurrentCore B weight");
            ValidateFixedShape(GetBufferShape(aWeight), new long[] { recurrentCore.NumValueHeads, hiddenSize }, "PyNTT GatedDeltaNetRecurrentCore A weight");
            ValidateFixedShape(GetBufferShape(aLog), new long[] { recurrentCore.NumValueHeads }, "PyNTT GatedDeltaNetRecurrentCore A_log");
            ValidateFixedShape(GetBufferShape(dtBias), new long[] { recurrentCore.NumValueHeads }, "PyNTT GatedDeltaNetRecurrentCore dt bias");
            ValidateFixedShape(GetBufferShape(normWeight), new long[] { recurrentCore.ValueHeadDim }, "PyNTT GatedDeltaNetRecurrentCore norm weight");
            ValidateFixedShape(GetBufferShape(coreScratch), new long[] { recurrentCore.NumValueHeads, recurrentCore.ValueHeadDim }, "PyNTT GatedDeltaNetRecurrentCore core scratch");
            ValidateFixedShape(GetBufferShape(gatedOutput), new long[] { 1, localValueCapacity }, "PyNTT GatedDeltaNetRecurrentCore gated output");
            if (stateConfig.NumKeyHeads != recurrentCore.NumKeyHeads ||
            ...
            var activationType = GetScalarDataType(qkv.ElemType);
            if (GetScalarDataType(z.ElemType) != activationType ||
                GetScalarDataType(projectionInput.ElemType) != activationType ||
                GetScalarDataType(bWeight.ElemType) != activationType ||
                GetScalarDataType(aWeight.ElemType) != activationType)
            {
                throw new NotSupportedException(
                    "PyNTT GatedDeltaNetRecurrentCore QKV/Z/projection input/A/B weights " +
                    "must share one scalar element type.");
            }

            var hierarchy = GetBlockHierarchy(_targetOptions);
            var hierarchyLevels = GetBlockHierarchyLevels(_targetOptions);
            var blockAxes = Enumerable.Range(0, hierarchy.Length)
                .Where(axis => hierarchyLevels[axis] == 'b')
                .ToArray();
            var blockCount = blockAxes.Aggregate(1L, (count, axis) => checked(count * hierarchy[axis]));
            var stateBarrier = "tl.debug_barrier()";
            if (blockCount > 1)
            {
                stateBarrier = "tle.distributed_barrier(PYNTT_GRID_MESH)";
            }
            ...
            {
                throw new InvalidOperationException(
                    "PyNTT GatedDeltaNet recurrent_core requires the selected A/B projection transfer pipeline.");
            }

            var bWeightDescriptor = RegisterHostTensorDescriptor(
                bWeight,
                $"{helperName}__b_weight_descriptor");
            var aWeightDescriptor = RegisterHostTensorDescriptor(
                aWeight,
                $"{helperName}__a_weight_descriptor");
            var templateModel = new PyNTTGatedDeltaNetRecurrentCoreTemplateModel(
                    ...
                    new PyNTTBufferPointerTemplateModel(
                        GetGatedDeltaNetStateFieldArgument(
                            ...
                            stateConfig)),
                    GetBufferScalarPointer(qkv),
                    GetBufferScalarPointer(z),
                    zPartial,
                    GetBufferScalarPointer(projectionInput),
                    ...
                    checked((int)localValueCapacity),
                    activeLocalValueCapacity.TritonExpression,
                    qkvLaneCount,
                    zLaneCount,
                    projectionInputLaneCount,
                    ...
                    microKernel,
                    $"({qkv.Name}, {z.Name}, {projectionInput.Name}, {bWeight.Name}, {aWeight.Name}, gated_delta_net_state) -> {gatedOutput.Name}")
            {
                BWeightDescriptorName = bWeightDescriptor.Name,
                BWeightDescriptorOriginElements = bWeightDescriptor.OriginElements,
                BWeightDescriptorOwnerIndexed = bWeightDescriptor.OwnerIndexed,
                AWeightDescriptorName = aWeightDescriptor.Name,
                AWeightDescriptorOriginElements = aWeightDescriptor.OriginElements,
                AWeightDescriptorOwnerIndexed = aWeightDescriptor.OwnerIndexed,
            };
            if (blockCount > 1)
            {
                _attrs["requires_grid_barrier"] = true;
        ...
        private int GetOpaqueInputIndex(IVar parameter)
        {
            if (!_parameterNames.TryGetValue(parameter, out var inputName))
            {
                throw new NotSupportedException(
                    $"PyNTT opaque kernel input {parameter.Name} must be a direct function parameter.");
            }
        ...
        private string GetPipelineChannelPointerArgument(BaseExpr expr)
        {
            expr = ResolveBoundExpression(expr);
            if (!IsPipelineChannelExpression(expr))
            {
                throw new NotSupportedException(
                    $"PyNTT pipeline channel pointer expects Reference<PipelineChannel>, got {expr.CheckedType}.");
            }

            if (TryGetFormalObjectBaseName(expr, out var formalBaseName))
            {
                return formalBaseName;
            }

            if (expr is IVar parameter)
            {
                return $"input{GetOpaqueInputIndex(parameter).ToString(CultureInfo.InvariantCulture)}";
            }

            throw new NotSupportedException(
                $"PyNTT pipeline channel pointer must be a direct or nested PrimFunction parameter, got {expr.GetType().Name}.");
        }
        ...
        private bool IsObjectKernelInput(string name)
        {
            foreach (var pair in _parameterNames)
            {
                ...
                {
                    return IsObjectExpression(expr) && !IsPipelineChannelExpression(expr);
                }
            }
        ...
        private static bool IsPipelineChannelExpression(BaseExpr expr)
            => expr.CheckedDataType is ReferenceType
            {
                ElemType: Nncase.IR.Heterogeneous.PipelineChannelType,
            };
        ...
        private HostTensorDescriptorBinding RegisterHostTensorDescriptor(TIR.Buffer buffer, string name)
        {
            if (!ReferenceEquals(_currentFunction, _function))
            {
                ...
                return new(name, originElementsName, false);
            }

            var descriptor = CreateHostTensorDescriptorBacking(buffer, name);
            AddHostTensorDescriptorBacking(descriptor);
            return new(descriptor.Name, "0", descriptor.OwnerIndexing is not null);
        }
            ...
            string resourceKey)
        {
            if (!ReferenceEquals(_currentFunction, _function))
            {
                return RegisterHostTensorDescriptor(buffer, resourceKey);
            }
            ...
            int scalarElementSizeBytes)
        {
            ...
            foreach (var resourceIndex in resourceIndexes)
            {
                ...
                return new(
                    ...
                    resource.OwnerIndexing is not null);
            }
            ...
            return new(candidate.Name, "0", candidate.OwnerIndexing is not null);
        }
            ...
            string name)
        {
            ...
            name = SanitizeBoundedPythonIdentifier(name);
            HostTensorDescriptorOwnerIndexingMetadata? ownerIndexing = buffer.MemSpan.Buffer.Location switch
            {
                MemoryLocation.Data when buffer.DistributedType is not null &&
                    buffer.DistributedStorageKind != DistributedBufferStorageKind.CanonicalGlobal =>
                    new HostTensorDescriptorOwnerIndexingMetadata("source_pool", 0),
                _ => null,
            };
            if (buffer.MemSpan.Buffer.Location == MemoryLocation.BlockLocalRdata)
            {
                var ownerStrideBytes = PyNTTRDataUtility.GetLocalRDataTableStrideBytes(
                    _currentFunction.SchedResult.BlockLocalRdatas,
                    _currentFunction.SchedResult.BlockLocalRDataMaterializations,
                    _targetOptions,
                    "b");
                ownerIndexing = ownerStrideBytes switch
                {
                    < 0 => throw new InvalidOperationException(
                        $"PyNTT block-local rdata owner stride must be non-negative, got {ownerStrideBytes}."),
                    0 => null,
                    _ => new HostTensorDescriptorOwnerIndexingMetadata("fixed", ownerStrideBytes),
                };
            }
        ...
        private static bool CanReuseHostTensorDescriptor(
            ...
            => resource.Source == candidate.Source &&
               Equals(resource.OwnerIndexing, candidate.OwnerIndexing) &&
        ...
        private void ValidateOperandScaleBuffer(string context, TIR.Buffer scale)
        {
            var scalarType = GetScalarDataType(scale.ElemType);
            if (scalarType is not PrimType || !scalarType.IsFloat())
            {
                throw new NotSupportedException(
                    $"{context} scale must use scalar floating-point storage, got {scale.ElemType}.");
            }

            var elementCount = Product(GetBufferActiveShape(scale));
            if (elementCount.MinValue != 1 || elementCount.MaxValue != 1)
            {
                throw new NotSupportedException(
                    $"{context} scale must contain exactly one active element, got {elementCount}.");
            }
        }
        ...
        private void WriteExplicitBarrier(TIR.NTT.Barrier barrier)
        {
            if (barrier.Scope == TIR.NTT.BarrierScope.Chip)
            {
                ...
                if (axes.Length != 0)
                {
                    var key = AddGridBarrierAxisGroup(axes, shape: null);
        ...
        private void AddGridBarrierAxisGroups(IEnumerable<PyNTTGridBarrierAxisGroup> groups)
        {
            foreach (var group in groups)
            {
                AddGridBarrierAxisGroup(group.Axes, group.Shape);
            }
        }

        private string AddGridBarrierAxisGroup(int[] rawAxes, int[]? shape)
        {
            var axes = NormalizeGridBarrierAxisGroupAxes(new IRArray<int>(rawAxes));
            if (axes.Length == 0)
            {
                throw new InvalidOperationException(
                    "A full-mesh grid barrier must use PYNTT_GRID_MESH instead of an axis-group descriptor.");
            }

            var hierarchy = GetBlockHierarchy(_targetOptions);
            var groupShape = shape ?? axes.Select(axis => hierarchy[axis]).ToArray();
            if (groupShape.Length != axes.Length)
            {
                throw new ArgumentException(
                    $"Grid barrier group shape rank {groupShape.Length} does not match axes rank {axes.Length}.",
                    nameof(shape));
            }

            for (var index = 0; index < axes.Length; index++)
            {
                var extent = groupShape[index];
                var domainExtent = hierarchy[axes[index]];
                if (extent <= 0 || domainExtent % extent != 0)
                {
                    throw new ArgumentException(
                        $"Grid barrier group extent {extent} must be positive and divide " +
                        $"hierarchy axis {axes[index]} extent {domainExtent}.",
                        nameof(shape));
                }
            }
            ...
            var key = string.Join(
                "_",
                axes.Zip(groupShape, static (axis, extent) =>
                    $"{axis.ToString(CultureInfo.InvariantCulture)}x{extent.ToString(CultureInfo.InvariantCulture)}"));
            var descriptor = new PyNTTGridBarrierAxisGroup(axes, groupShape);
            if (_gridBarrierAxisGroups.TryGetValue(key, out var existing) &&
                (!existing.Axes.SequenceEqual(axes) || !existing.Shape.SequenceEqual(groupShape)))
            {
                throw new InvalidOperationException(
                    $"Conflicting PyNTT grid barrier axis-group descriptors share key {key}.");
            }

            _gridBarrierAxisGroups[key] = descriptor;
            return key;
        }
        ...
        private sealed class PipelineRegionCodegenState
        {
            ...
            private readonly List<PipelineHandoffCodegenState> _handoffOrder;
            private readonly IReadOnlyDictionary<string, PipelineStageCodegenState> _stages;
            private readonly Dictionary<string, PipelineHandoffCodegenState> _handoffs;
            private readonly IReadOnlySet<string> _semanticHandoffIds;
            private readonly IReadOnlySet<string> _drainedStageIds;
            private readonly List<string> _primaryAuxiliaryConsumerEndpointNames = new();
            private readonly List<string> _auxiliaryConsumerBody = new();
            private readonly List<string> _auxiliaryConsumerEndpointNames = new();
            private readonly HashSet<string> _auxiliaryConsumerEndpointNameSet = new(StringComparer.Ordinal);
                ...
                IReadOnlySet<string> drainedStageIds)
            {
                ...
                _handoffOrder = handoffs.ToList();
                _stages = ToUniqueDictionary(
                    _stageOrder,
                    stage => stage.StageId,
                    "pipeline stage");
                _handoffs = new Dictionary<string, PipelineHandoffCodegenState>(
                    ToUniqueDictionary(
                    _handoffOrder,
                    handoff => handoff.HandoffId,
                    "pipeline handoff"), StringComparer.Ordinal);
                _semanticHandoffIds = _handoffs.Keys.ToHashSet(StringComparer.Ordinal);
            ...
            public IReadOnlyList<string> ConsumerEndpointNames
                => PipeStages.Select(stage => stage.ReaderName)
                    ...
                    .Concat(_handoffOrder
                        .Where(handoff => _semanticHandoffIds.Contains(handoff.HandoffId))
                        .Select(handoff => handoff.WriterName))
                    .Concat(_primaryAuxiliaryConsumerEndpointNames)
                    .ToArray();
            ...
            public IReadOnlyList<string> ProducerEndpointNames
                => PipeStages.Select(stage => stage.WriterName)
                    .Concat(_handoffOrder
                        .Where(handoff => _semanticHandoffIds.Contains(handoff.HandoffId))
                        .Select(handoff => handoff.ReaderName))
                    .ToArray();

            public bool HasAuxiliaryConsumer => _auxiliaryConsumerBody.Count != 0;

            public string AuxiliaryConsumerBodySource
                => string.Join(Environment.NewLine, _auxiliaryConsumerBody);

            public IReadOnlyList<string> AuxiliaryConsumerEndpointNames
                => _auxiliaryConsumerEndpointNames;
            ...
            public AuxiliaryConsumerBinding BindAuxiliaryConsumer(
                PipelineStageCodegenState stage,
                PyNTTAuxiliaryConsumerTemplateModel contract)
            {
                var selectedChannels = contract.TransferPipelineChannelNames
                    .Select(name => stage.PipeStages.SingleOrDefault(pipeStage =>
                        string.Equals(pipeStage.ChannelName, name, StringComparison.Ordinal))
                        ?? throw new InvalidOperationException(
                            $"Pipeline stage {stage.StageId} auxiliary consumer references unknown " +
                            $"transfer channel {name}."))
                    .ToArray();
                var consumerWorkspaces = stage.PipeStages
                    .SelectMany(pipeStage => pipeStage.ConsumerSharedWorkspaces)
                    .ToDictionary(workspace => workspace.Name, StringComparer.Ordinal);
                var selectedWorkspaces = contract.SharedWorkspaceNames
                    .Select(name => consumerWorkspaces.TryGetValue(name, out var workspace)
                        ? workspace
                        : throw new InvalidOperationException(
                            $"Pipeline stage {stage.StageId} auxiliary consumer references unknown " +
                            $"consumer Shared workspace {name}."))
                    .ToArray();

                var start = AddAuxiliaryHandoff(stage, "start");
                var done = AddAuxiliaryHandoff(stage, "done");
                var auxiliaryEndpoints = selectedChannels
                    .Select(channel => channel.ReaderName)
                    .Concat(new[] { start.ReaderName, done.WriterName })
                    .Concat(selectedWorkspaces.Select(workspace => workspace.VariableName))
                    .ToArray();
                _primaryAuxiliaryConsumerEndpointNames.Add(start.WriterName);
                _primaryAuxiliaryConsumerEndpointNames.Add(done.ReaderName);
                foreach (var endpoint in auxiliaryEndpoints)
                {
                    if (_auxiliaryConsumerEndpointNameSet.Add(endpoint))
                    {
                        _auxiliaryConsumerEndpointNames.Add(endpoint);
                    }
                }

                var selectedReaderNames = selectedChannels
                    .Select(channel => channel.ReaderName)
                    .ToArray();
                var drainEndpointNames = _drainedStageIds.Contains(stage.StageId)
                    ? selectedReaderNames
                    : Array.Empty<string>();
                if (drainEndpointNames.Length != 0)
                {
                    stage.BindAuxiliaryConsumerDrain(drainEndpointNames);
                }

                return new(
                    new[] { start.WriterName, done.ReaderName },
                    auxiliaryEndpoints,
                    drainEndpointNames);
            }

            public void AppendAuxiliaryConsumerLine(string source)
                => _auxiliaryConsumerBody.Add(source);

            private PipelineHandoffCodegenState AddAuxiliaryHandoff(
                PipelineStageCodegenState stage,
                string role)
            {
                var handoff = PipelineHandoffCodegenState.CreateAuxiliary(
                    stage.StageId,
                    stage.StageName,
                    role);
                if (!_handoffs.TryAdd(handoff.HandoffId, handoff))
                {
                    throw new InvalidOperationException(
                        $"Pipeline stage {stage.StageId} emitted duplicate auxiliary " +
                        $"consumer handoff {handoff.HandoffId}.");
                }

                _handoffOrder.Add(handoff);
                return handoff;
            }
        ...
        private sealed class PipelineStageCodegenState
        {
            ...
            private IReadOnlySet<string> _auxiliaryConsumerDrainEndpointNames =
                new HashSet<string>(StringComparer.Ordinal);
            ...
            public IReadOnlySet<string> AuxiliaryConsumerDrainEndpointNames
                => _auxiliaryConsumerDrainEndpointNames;
            ...
            public void BindAuxiliaryConsumerDrain(IEnumerable<string> endpointNames)
            {
                var names = endpointNames.ToHashSet(StringComparer.Ordinal);
                if (names.Count == 0 ||
                    names.Any(name => !_consumerDrainEndpointNames.Contains(name, StringComparer.Ordinal)))
                {
                    throw new InvalidOperationException(
                        $"Pipeline stage {StageId} auxiliary consumer must own one or more " +
                        "consumer drain endpoints from the selected transfer channels.");
                }

                _auxiliaryConsumerDrainEndpointNames = names;
            }
            ...
            string WriterName)
        {
            ...
            public static PipelineHandoffCodegenState CreateAuxiliary(
                string stageId,
                string stageName,
                string role)
            {
                var handoffId = $"{stageId}/auxiliary/{role}";
                var handoffName = SanitizeBoundedPythonIdentifier(
                    $"{stageName}__auxiliary_{role}");
                return new(
                    handoffId,
                    handoffName,
                    $"{handoffName}__pipe",
        ...
        private sealed record AuxiliaryConsumerBinding(
            IReadOnlyList<string> PrimaryConsumerEndpointNames,
            IReadOnlyList<string> AuxiliaryConsumerEndpointNames,
            IReadOnlyList<string> DrainEndpointNames);
        ...
        public sealed record DeviceFunctionRelocationBinding(
            string AnchorName,
            long RelativeOffsetBytes);

        private sealed record DeviceFunctionRelocationCandidate(
            int Index,
            long FirstOffsetBytes,
            string TranslationSignature);

        private sealed record DeviceFunctionRelocationLocation(
            string BaseName,
            long OffsetBytes);
    ...
    private static void ValidateRoPEShape(string context, IReadOnlyList<PyNTTDimExpression> inputShape, IReadOnlyList<PyNTTDimExpression> cosShape, IReadOnlyList<PyNTTDimExpression> sinShape, IReadOnlyList<PyNTTDimExpression> outputShape, int rotaryAxis, int laneCount, int sinCosVectorPackFactor)
    {
        ...
        ValidateSameShape($"{context} sin/cos", cosShape, sinShape);
        if (cosShape.Count != outputShape.Count)
        {
            throw new NotSupportedException(
                $"{context} sin/cos tensors must retain the output rank, got " +
                $"sin/cos=[{ShapeText(cosShape)}], output=[{ShapeText(outputShape)}].");
        }

        for (var axis = 0; axis < cosShape.Count; axis++)
        {
            if (axis != rotaryAxis && !cosShape[axis].IsFixedOne && !SameDim(cosShape[axis], outputShape[axis]))
            {
                throw new NotSupportedException(
                    $"{context} sin/cos axis {axis} must be broadcastable to the output, got " +
                    $"sin/cos=[{ShapeText(cosShape)}], output=[{ShapeText(outputShape)}].");
            }
        }

        var outputPhysicalExtent = RequireFixedDim(outputShape[rotaryAxis], $"{context} output rotary extent");
        var sinCosPhysicalExtent = RequireFixedDim(cosShape[rotaryAxis], $"{context} sin/cos rotary extent");
        var rotaryPhysicalExtent = checked(sinCosPhysicalExtent * sinCosVectorPackFactor);
        if (rotaryPhysicalExtent <= 0 || rotaryPhysicalExtent > outputPhysicalExtent)
        {
            throw new NotSupportedException(
                $"{context} packed rotary extent must be positive and no larger than the output extent, got " +
                $"rotary={rotaryPhysicalExtent}, output={outputPhysicalExtent}, " +
                $"sin/cos=[{ShapeText(cosShape)}], lanes={laneCount}, pack={sinCosVectorPackFactor}.");
        }

        var rotaryDim = checked(rotaryPhysicalExtent * laneCount);
        if (rotaryDim % 2 != 0)
        {
            throw new NotSupportedException(
                $"{context} requires an even logical rotary dimension, got {rotaryDim} from " +
                $"sin/cos=[{ShapeText(cosShape)}], lanes={laneCount}, pack={sinCosVectorPackFactor}.");
        }
    }
    ...
    private static string GetTritonDType(string dtypeName)
    {
        return dtypeName switch
        {
            ...
            "float8e4m3fn" => "tl.float8e4nv",
            "float8e5m2" => "tl.float8e5",
            "float32" => "tl.float32",
            "float64" => "tl.float64",
...
internal sealed record DeviceFunctionBuildResult(
    ...
    IReadOnlyList<PyNTTGridBarrierAxisGroup> GridBarrierAxisGroups,
...
internal sealed record PyNTTGridBarrierAxisGroup(
    [property: JsonPropertyName("axes")]
    int[] Axes,
...
internal sealed record HostTensorDescriptorBackingMetadata(
    ...
    long OffsetBytes,
    [property: JsonPropertyName("owner_indexing")]
    HostTensorDescriptorOwnerIndexingMetadata? OwnerIndexing,
...
internal sealed record HostTensorDescriptorOwnerIndexingMetadata(
    [property: JsonPropertyName("kind")]
    string Kind,
    [property: JsonPropertyName("stride_bytes")]
    long StrideBytes);
